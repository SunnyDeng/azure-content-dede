...
**Grundlegende Dienste** – [Die Komponenten von Azure](#components) – [Verwaltungsportal](#portal) – [Compute](#compute) – [Datenverwaltung](#data) – [Netzwerkbetrieb](#networking) – [Dienste für Entwickler](#DevService)

**Zusätzliche Dienste** – [Identität und Zugriff](#identity) – [Mobile Services](#mobile) – [Backup](#backup) – [Messaging und Integration](#messaging) – [Compute Assistance](#ComputeAssist) – [Leistung](#Performance) – [Big Compute und Data](#BigStuff) – [Media Services](#media) – [Commerce](#commerce)
...
Azure gruppiert Dienste im Verwaltungsportal unter verschiedenen Kategorien und bietet verschiedene visuelle Hilfsmittel, wie beispielsweise die [Infografik "Was ist Microsoft Azure?"](http://azure.microsoft.com/documentation/infographics/azure/ "Infografik "Was ist Microsoft Azure?""). Im Verwaltungsportal verwalten Sie die meisten (aber nicht alle) Dienste in Azure.
...
![Azure-Komponenten](./media/intro-to-azure/AzureComponentsIntroNew780.png) **Abbildung: Azure bietet über das Internet zugängliche Anwendungsdienste, die in Azure-Datencentern ausgeführt werden. **

<h2><a id="portal"></a>Verwaltungsportal</h2>
...
Eine der grundlegendsten Aufgaben einer Cloudplattform ist die Ausführung von Anwendungen. Azure bietet die folgenden Optionen:
...
![Azure Virtual Machines](./media/intro-to-azure/VirtualMachinesIntroNew.png) **Azure Virtual Machines bietet Ihnen vollständige Kontrolle über die Instanzen virtueller Computer in der Cloud.**
...
![Azure Websites](./media/intro-to-azure/AzureWebsitesIntroNew.png) **Azure Websites führt eine Websiteanwendung in der Cloud aus, ohne dass der zugrunde liegende Webserver verwaltet werden muss.**
...
###Cloud Services###
![Azure Cloud Service](./media/intro-to-azure/CloudServicesIntroNew.png) **Azure Cloud Services ermöglicht die Ausführung von hoch skalierbarem benutzerdefinierten Code in einer PaaS-Umgebung (Platform-as-a-Service).**
...
Die Seite "Azure Websites, Cloud Services and Virtual Machines im Vergleich" (http://azure.microsoft.com/documentation/articles/choose-web-site-cloud-service-vm/) enthält ausführlichere Informationen zur Auswahl eines Computemodells.
...
###Azure SQL-Datenbank###
![Azure Storage und SQL-Datenbank](./media/intro-to-azure/StorageAzureSQLDatabaseIntroNew.png) **Abbildung: Azure SQL-Datenbank stellt einen verwalteten relationalen Datenbankdienst in der Cloud bereit.**
...
###Tabellen###
![Azure-Speichertabellen](./media/intro-to-azure/StorageTablesIntroNew.png) **Abbildung: Azure-Tabellen bietet eine flache NoSQL-Methode zum Speichern von Daten.**
...
![Azure-Speicherblobs](./media/intro-to-azure/StorageBlobsIntroNew.png) **Abbildung: Azure-Blobs enthält unstrukturierte binäre Daten.**
...
###Azure-Dateidienst###
![Azure File Service](./media/intro-to-azure/FileServiceIntroNew.png) **Abbildung: Azure File Services stellt "\\server\share"-SMB-Pfade für in der Cloud ausgeführte Anwendungen bereit.**
...
![ExpressRoute](./media/intro-to-azure/ExpressRouteIntroNew.png) **Abbildung: ExpressRoute verwendet ein virtuelles Azure-Netzwerk, verwendet für Verbindungen jedoch schnellere dedizierte Leitungen anstelle des öffentlichen Internets.**
...
![Traffic Manager](./media/intro-to-azure/TrafficManagerIntroNew.png) **Abbildung: Mit Azure Traffic Manager können Sie globalen Datenverkehr mithilfe intelligenter Regeln zu Ihrem Dienst lenken.**
...
<h2><a id="DevService"></a>Entwicklerdienste</h2>
...
<h2><a id="identity"></a>Identität und Zugriff</h2>
...
![Azure Multi-Factor Authentication](./media/intro-to-azure/MFAIntroNew.png) **Abbildung: Multi-Factor Authentication bietet Ihrer Anwendung die Funktionalität, mehr als eine Form der Identifikation zu überprüfen.**
...
![Notification Hubs](./media/intro-to-azure/NotificationHubsIntroNew.png) **Abbildung: Notification Hubs stellt Funktionen bereit, die häufig von Anwendungen mit einer Schnittstelle für mobile Geräte benötigt werden.**
...
**Notification Hub-Szenarien** – Wenn Sie ein mobiles Spiel schreiben, in dem sich die Spieler abwechseln, müssen Sie Spieler 2 eventuell darüber benachrichtigen, dass Spieler 1 seine Runde beendet hat. Wenn dies schon alles ist, können Sie auch einfach Mobile Services verwenden. Wenn ihr Spiel jedoch von 100.000 Benutzern gespielt wird und Sie jedem ein zeitkritisches kostenloses Angebot senden möchten, ist Notification Hubs die bessere Wahl.
...
Die Lösung überwacht ständig den Zustand Ihrer Dienste und automatisiert die geordnete Wiederherstellung von Diensten bei Ausfall eines Standorts im primären Datencenter. Virtuelle Computer können systematisch online geschaltet werden, um Anwendungen selbst bei komplexen mehrschichtigen Workloads schnell wiederherzustellen.

Site Recovery nutzt vorhandene Technologien wie Hyper-V Replica, System Center und SQL Server AlwaysOn. Ausführlichere Informationen finden Sie unter [Übersicht über Azure Site Recovery](hyper-v-recovery-manager-overview.md).

###Azure Backup###
![Azure Backup](./media/intro-to-azure/AzureBackupIntroNew.png) **Abbildung: Azure Backup sichert Daten von lokalen Windows-Servern in der Cloud.**
...
<h2><a id="messaging"></a>Messaging und Integration</h2>
...
###Warteschlangen###
![Azure Service Bus Relay](./media/intro-to-azure/QueuesIntroNew.png) **Abbildung: Warteschlangen ermöglichen eine lose Kopplung von Elementen einer Anwendung und erleichtern die Skalierung.**
...
![Azure Service Bus Relay](./media/intro-to-azure/ServiceBusRelayIntroNew.png) **Abbildung: Service Bus Relay ermöglicht die Kommunikation zwischen Anwendungen auf unterschiedlichen Seiten der Firewall.**
...
![Azure Service Bus-Themen](./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png) **Abbildung: Service Bus-Themen bietet mehreren Apps die Möglichkeit, Nachrichten zu senden und anderen Anwendungen, über ein Abonnement Nachrichten zu empfangen, die bestimmte Kriterien erfüllen.**
...
**Service Bus Relay-Szenarien** In allen Situationen, in denen viele Nachrichten vorkommen, die zwar sämtlich wichtig sind, von den verschiedenen nachgelagerten Systemen jedoch nur teilweise überwacht werden müssen, sind Service Bus-Themen und -Abonnements eine gute Option.
  

###BizTalk Services###
![BizTalk Services](./media/intro-to-azure/BizTalkServicesIntroNew.png) **Abbildung: BizTalk Services ermöglicht die Umwandlung von XML-Nachrichtenformaten in der Cloud.**
...
<h2><a id="Performance"></a>Leistung</h2>
...
###Azure Caching###
![Azure Caching](./media/intro-to-azure/AzureCacheIntroNew.png) **Abbildung: Eine Azure-Anwendung kann Daten im Arbeitsspeicher zwischenspeichern und die Daten sogar auf viele Workerrollen verteilen.**
...
###CDN###
![Azure CDN](./media/intro-to-azure/CDNIntroNew.png) **Abbildung: Kopien eines Blobs können auf Standorten auf der ganzen Welt zwischengespeichert werden.**
...
<h2><a id="BigStuff"></a>Big Data und Big Compute</h2>

###HDInsight (Hadoop)###
![HDInsight](./media/intro-to-azure/HDInsightIntroNew.png) **Abbildung: HDInsight bietet Unterstützung bei der Massenverarbeitung großer Datenmengen.**
...
![Azure Media Services](./media/intro-to-azure/MediaServicesIntroNew.png) **Abbildung: Media Services ist eine Plattform für Anwendungen, die Kunden rund um die Welt Videos und andere Medien zur Verfügung stellen.**
