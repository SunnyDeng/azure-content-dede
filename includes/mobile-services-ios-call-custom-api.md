...
Aktualisieren der App Aktualisieren der App zum Aufruf der benutzerdefinierten API
----------------------------------------------------------------------------------
...
3.  Nun verbinden wir dieses Objekt mit dem View Controller-Quellcode. **Klicken Sie mit gedrückter Strg-Taste** auf die neue Schaltfläche **"Alle"** und ziehen Sie mit der Maus vor die `@end`-Zeile in **QSTodoListViewController.h**. Verbinden Sie das Objekt mit einer neuen **Aktion** namens `onCompleteAll` in **QSTodoListViewController**. Xcode fügt automatisch die folgende Zeile vor der `@end`-Zeile ein:

            - (IBAction)onCompleteAll:(id)sender;

4.  Ziel dieser `onCompleteAll`-Methode ist die Verarbeitung des Click-Ereignisses für die neue Schaltfläche. Sie ruft eine neue `completeAll`-Methode auf, die wir zu unserer benutzerdefinierten Klasse hinzufügen werden, die wiederum eine POST-Anfrage an die neue benutzerdefinierte API sendet. Das von der benutzerdefinierten API zurückgegebene Ergebnis wird in einem Meldungsdialogfeld angezeigt, ebenso wie jegliche Fehler. Bearbeiten Sie **QSTodoListViewController.m**, indem Sie die folgende Implementierung vor der `@end`-Zeile einfügen:
...
5.  Beachten Sie, dass sich der obige Code auf eine neue Methode `completeAll` bezieht, die noch nicht in **QSTodoService** definiert worden ist. Bearbeiten Sie **QSTodoService.h**, und fügen Sie die folgende Zeile vor der `@end`-Zeile ein:

         - (void) completeAll:(MSAPIBlock)completion;

6.  Fügen Sie die entsprechende Implementierung von `completeAll` vor der `@end`-Zeile in **QSTodoService.m** ein. Wie JavaScript unterstützt iOS JSON-Serialisierung beliebiger Typen nicht. Daher hat es auch eine ziemlich einfache API zum Aufrufen benutzerdefinierter APIs beruhend auf der `invokeAPI`-Methode.
