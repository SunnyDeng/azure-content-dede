...
3.  Öffnen Sie die Projektdatei "MainPage.xaml.cs", und ersetzen Sie die Methodenüberschreibung **OnNavigatedTo** durch Folgendes:
    ...
    In diesem Code müssen Sie den generierten Pushklassennamen (`todolistPush`) durch den Namen der Klasse ersetzen, die vom Assistenten generiert wurde, meist im Format `mobile_servicePush`.
