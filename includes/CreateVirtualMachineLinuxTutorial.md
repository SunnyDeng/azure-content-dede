...
**Hinweis**: In diesem Tutorial wird ein virtueller Computer erstellt,
...
Ein virtueller Computer in Azure ist ein Server in der Cloud, den Sie steuern und verwalten können. Nachdem Sie einen virtuellen Computer in Azure erstellt haben, können Sie ihn jederzeit löschen und neu erstellen, und Sie können auf den virtuellen Computer zugreifen, wie auf einen Server in Ihrem Büro. Zum Erstellen eines virtuellen Computers werden Dateien für virtuelle Festplatten (Virtual Hard Disk File, VHD) verwendet. Die folgenden Typen von VHDs werden verwendet:

* **Abbild** -- Eine VHD, die als Vorlage zum Erstellen eines neuen
  virtuellen Computers verwendet wird. Ein Abbild ist eine Vorlage, da
  es nicht über spezifische Einstellungen verfügt wie ein ausgeführter
  virtueller Computer. Beispiele für diese Einstellungen sind
  Computername und Benutzerkontoeinstellungen. Wenn Sie einen virtuellen
  Computer mithilfe eines Abbilds erstellen, wird automatisch eine
  Betriebssystemfestplatte für den neuen virtuellen Computer erstellt.
* **Festplatte** -- Eine Festplatte ist eine VHD, die Sie als
  ausgeführte Version eines Betriebssystems starten und einbinden
  können. Sobald ein Abbild bereitgestellt wurde, wird es zu einer
  Festplatte. Es wird immer eine Festplatte erstellt, wenn Sie ein
  Abbild zum Erstellen eines virtuellen Computers verwenden. Jede VHD,
  die an virtualisierte Hardware angeschlossen ist und die im Rahmen
  eines Service ausgeführt wird, ist eine Festplatte.

Folgende Optionen sind verfügbar, um virtuelle Computer mithilfe von Abbildern zu erstellen:

* Erstellen Sie einen virtuellen Computer unter Verwendung eines
  Abbilds, das in der Bildergalerie des Azure Verwaltungsportals
  bereitgestellt wurde.
* Erstellen Sie eine VHD-Datei, die ein Abbild zu Azure enthält, und
  laden Sie diese hoch. Erstellen Sie dann einen virtuellen Computer
...
1.  Melden Sie sich beim Azure [Verwaltungsportal][3] an. Klicken Sie in der Befehlsleiste auf **Neu**.
...
* Wenn Sie einen Computer mit einem Windows-Betriebssystem verwenden, können Sie beispielsweise einen SSH-Client wie PuTTY verwenden. Weitere Informationen erhalten Sie unter [PuTTY Download][6] (in
  englischer Sprache).
* Wenn Sie einen Computer mit einem Linux-Betriebssystem verwenden, können Sie beispielsweise einen SSH-Client wie OpenSSH verwenden. Weitere Informationen finden Sie unter [OpenSSH][7].
...
1.  Navigieren Sie im Verwaltungsportal zum **Hostnamen** und den **Portinformationen**. Sie finden die erforderlichen Informationen im Dashboard des virtuellen Computers. Klicken Sie in der **Schnelleinsicht** des Dashboards auf den Namen des virtuellen Computers, und navigieren Sie zu den **SSH-Details**.
    ...
    ![Rufen Sie die SSH-Details
    auf.](./media/CreateVirtualMachineLinuxTutorial/SSHdetails.png)
...
3.  Geben Sie den **Hostnamen** und die **Portinformationen** aus dem Dashboard ein, und klicken Sie dann auf **Öffnen**.
    ...
    ![Geben Sie den Hostnamen und die Portinformationen
    ein.](./media/CreateVirtualMachineLinuxTutorial/putty.png)

4.  Melden Sie sich mit dem Konto "NeuerBenutzer1" an, das Sie beim Erstellen des virtuellen Computers angegeben haben.
    
    ![Melden Sie sich bei dem neuen virtuellen Computer
    an.](./media/CreateVirtualMachineLinuxTutorial/sshlogin.png)
...
**Hinweis: Datenträger im Vergleich zu Ressourcendatenträgern** Datenträger auf Azure Storage können zum dauerhaften Speichern von Dateien und Anwendungsdaten verwendet werden.
...
Auf Linux wird der Ressourcendatenträger in der Regel vom Azure Linux Agent verwaltet und automatisch in **/mnt/resource** (oder **/mnt** auf Ubuntu-Abbildern) eingebunden. Weitere Informationen erhalten Sie im
[Azure Linux Agent User Guide][8] (Benutzerhandbuch für Azure Linux, in
englischer Sprache).
...
3.  Klicken Sie in der Befehlsleiste auf **Anschließen**, und klicken Sie dann auf **Leeren Datenträger anschließen**.
    ...
    ![Definieren Sie die
    Datenträgerdetails.](./media/CreateVirtualMachineLinuxTutorial/attachnewdisklinux.png)
...
6.  Sie können überprüfen, ob der Datenträger erfolgreich an den virtuellen Computer angeschlossen wurde, indem Sie sich das Dashboard anzeigen lassen. Klicken Sie auf den Namen des virtuellen Computers, um das Dashboard anzuzeigen.
    ...
    angeschlossen](./media/CreateVirtualMachineLinuxTutorial/attachemptysuccess.png)
...
2.  Geben Sie in das SSH-Fenster folgenden Befehl ein, und geben Sie dann das Kontokennwort ein:
    ...
    ![Bezeichnen Sie den
    Datenträger.](./media/CreateVirtualMachineLinuxTutorial/diskmessages.png)
...
4.  Geben Sie **n** ein, um eine neue Partition zu erstellen.
    ...
    ![Neues Gerät
    erstellen](./media/CreateVirtualMachineLinuxTutorial/diskpartition.png)
...
8.  Sie müssen das Dateisystem auf der neuen Partition erstellen. Geben Sie z. B. den folgenden Befehl ein, um das Dateisystem zu erstellen, und geben Sie anschließend das Kontokennwort ein:
    ...
    ![Dateisystem
    erstellen](./media/CreateVirtualMachineLinuxTutorial/diskfilesystem.png)
...
* [Introduction to Linux on Azure (Einführung in Linux auf Azure, in
  englischer Sprache)][9]

* [Verwenden der Azure-Befehlszeilentools für Mac und Linux][10]
