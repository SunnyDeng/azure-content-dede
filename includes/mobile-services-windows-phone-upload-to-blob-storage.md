...
In diesem Abschnitt aktualisieren Sie das Projekt aus dem Lernprogramm [Erste Schritte mit Mobile Services][Erste Schritte mit Mobile Services], um Fotos aufzunehmen und diese in den Azure-BLOB-Speicher hochzuladen. Zum Aufnehmen des Bilds wird in diesem Lernprogramm [CameraCaptureTask][CameraCaptureTask] aus dem `Microsoft.Phone.Tasks`-Namespace verwendet. Diese Klasse startet die Kamera-Benutzeroberfläche auf dem Windows Phone-Gerät, um das Foto aufzunehmen, und speichert das Bild automatisch in Eigene Aufnahmen auf dem Windows Phone-Gerät. Falls Sie die Bilder nicht in "Eigene Aufnahmen" speichern möchten, verwenden Sie stattdessen die [PhotoCamera][PhotoCamera]-Klasse im `Microsoft.Devices`-Namespace.
...
2.  Öffnen Sie die Datei MainPage.xaml file und ersetzen Sie das **Grid**-Element namens **ContentPanel** durch den folgenden Code:
    ...
    Dadurch wird eine neue Schaltfläche zum Starten von [CameraCaptureTask][CameraCaptureTask] hinzugefügt. Außerdem wird **ItemTemplate** ein Bild hinzugefügt und dessen Bindungsquelle als URI des hochgeladenen Bilds im BLOB-Speicherdienst festgelegt.
...
5.  Aktualisieren Sie die MainPage-Klasse in der Projektdatei MainPage.xaml.cs. Fügen Sie den folgenden Code hinzu, um [CameraCaptureTask][CameraCaptureTask] und ein Datenstromobjekt zum Verweisen auf das erfasste Bild zu deklarieren:
...
4.  Klicken Sie auf **zulassen**, um das Bild anzunehmen und die Kamera-Benutzeroberfläche zu verlassen.
