<properties 
	pageTitle="Gewusst wie: Einrichten von Tomcat7 auf einem virtuellen Linux-Computer mit Microsoft Azure" 
...
#Gewusst wie. Einrichten von Tomcat7 auf einem virtuellen Linux-Computer mit Microsoft Azure 
...
-	Gewusst wie: Erstellen eines virtuellen Computers in Azure.
...
3.	Klicken Sie auf **Generieren**, um die Schlüssel zu generieren. Während des Vorgangs können Sie die Zufälligkeit durch Bewegen der Maus über den leeren Bereich im Fenster erhöhen.  
![][1]
4.	Nach dem Generierungsvorgang zeigt "Puttygen.exe" den generierten Schlüssel an. Beispiel:  
![][2]
5.	Wählen und kopieren Sie den öffentlichen Schlüssel in **Schlüssel**, und speichern Sie ihn in einer Datei namens "publicKey.pem". Klicken Sie nicht auf **Öffentlichen Schlüssel speichern**, weil sich das Dateiformat des gespeicherten öffentlichen Schlüssels vom gewünschten öffentlichen Schlüssel unterscheidet.
...
1.	Klicken Sie im Azure Preview-Portal auf **Durchsuchen** -> **Virtueller Computer**, und klicken Sie dann auf den virtuellen Computer, den Sie erstellt haben.  
![][5]
...
Um zu überprüfen, ob Tomcat7 erfolgreich installiert wurde, navigieren Sie zum DNS-Namen Ihres Tomcat-Servers: (http://tomcatexample.cloudapp.net/ is the example URL in this article). Wenn sinngemäß die folgende Seite angezeigt wird, haben Sie Tomcat7 korrekt installiert.
![][16] 
...
>AZURE.HINWEIS: Erstellen Sie ein sicheres Kennwort für den Admin-Benutzername.  
...
-	**Lösung** 
	...
	2.	Wenn das Problem durch die Firewall/IP-Tabellen verursacht wird, fügen Sie "/etc/sysconfig/iptables" die folgenden Zeilen hinzu:  
		...
		Führen Sie den folgenden Befehl aus, um die IP-Tabellen erneut zu laden:  
...
-	**Lösung**  
Sie müssen Berechtigungen aus dem Konto "root" abrufen. Sie können den Besitzer dieses Ordners aus "root" in den Benutzernamen ändern, die Sie bei der Bereitstellung des Computers verwendet haben. Das folgende Beispiel zeigt den Kontonamen "azureuser":  
