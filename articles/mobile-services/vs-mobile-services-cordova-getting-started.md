<properties 
	pageTitle="" 
	description="Beschreibt die ersten Schritte für die Arbeit mit Azure Mobile Services in einem Cordova-Projekt." 
...
todoTable.update(todoItem).done(function (item) { // Aktualisieren einer lokalen Sammlung von Elementen. items.splice(items.indexOf(todoItem), 1, item); });

#####Löschen eines Tabellenelements

Löschen Sie eine Zeile aus einer Datentabelle mithilfe der Methode **del**. Rufen Sie die `done()`-Methode für das zurückgegebene [Promise](https://msdn.microsoft.com/library/dn802826.aspx)-Objekt auf, um eine Kopie des eingefügten Objekts abzurufen und ggf. Fehler zu behandeln.
