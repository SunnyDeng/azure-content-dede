<properties
	pageTitle="Arbeiten mit der verwalteten Clientbibliothek von Mobile Services (Windows | Xamarin) | Microsoft Azure"
	description="Erfahren Sie, wie Sie einen .NET Client für Azure Mobile Services mit Windows- und Xamarin-Apps verwenden."
...
Xamarin-Apps erfordern zusätzlichen Code zum Registrieren einer Xamarin-App unter iOS oder Android beim Apple Push Notification Service (APNS) und den Google Cloud Messaging-Diensten (GCM). Weitere Informationen finden Sie unter **Hinzufügen von Pushbenachrichtigungen zur App** ([Xamarin.iOS](partner-xamarin-mobile-services-ios-get-started-push.md#add-push) | [Xamarin.Android](partner-xamarin-mobile-services-android-get-started-push.md#add-push)).
...
Damit Sie Benutzer authentifizieren können, müssen Sie Ihre App beim Microsoft-Konto für das Developer Center registrieren. Sie müssen dann diese Registrierung mit Ihrem mobilen Dienst verbinden. Führen Sie die Schritte unter [Registrieren Ihrer App für die Verwendung einer Microsoft-Kontoanmeldung](mobile-services-how-to-register-microsoft-authentication.md) aus, um eine Microsoft-Kontoregistrierung zu erstellen und diese mit Ihrem mobilen Dienst zu verbinden. Wenn Sie sowohl eine Windows Store- als auch eine Windows Phone 8-/Silverlight-Version Ihrer App haben, registrieren Sie zuerst die Windows Store-Version.

Im folgenden Code erfolgt eine Authentifizierung mit Live SDK und wird das zurückgegebene Token verwendet, um eine Anmeldung bei Ihrem mobilen Dienst vorzunehmen.
