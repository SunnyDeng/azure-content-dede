<properties urlDisplayName="Azure Import/Export Service" pageTitle="Verwenden von Import/Export zum Übertragen von Daten in den Blob-Speicher | Microsoft Azure" metaKeywords="" description="Erfahren Sie, wie Sie Import- und Exportaufträge im Azure-Verwaltungsportal erstellen, um Daten in den Blob-Speicher zu übertragen." metaCanonical="" disqusComments="1" umbracoNaviHide="0" title="Using the Azure Import/Export Service to Transfer Data to Blob Storage" authors="tamram" manager="adinah" />
...
Dieser Artikel bietet eine Übersicht über den Import-/Export-Dienst und beschreibt die Verwendung des Verwaltungsportals für den Import-/Export-Dienst. In der [REST-API-Referenz zum Azure-Import-/Exportdienst](http://go.microsoft.com/fwlink/?LinkID=329099)finden Sie weitere Informationen zur REST-API.

## Übersicht über den Import-/Exportdienst
...
Zum Vorbereiten Ihres Laufwerks auf den Versand für einen Importauftrag führen Sie das **Microsoft Azure-Import-/Exporttool** aus. Dabei werden Ihre Daten auf das Laufwerk kopiert, die Daten auf dem Laufwerk mit BitLocker verschlüsselt und die Journaldateien für das Laufwerk erstellt, die weiter unten besprochen werden.

<div class="dev-callout">
<strong>Hinweis</strong>
<p>Die Daten auf dem Laufwerk müssen mit BitLocker-Laufwerkverschlüsselung verschlüsselt sein. So sind Ihre Daten beim Transport geschützt. Bei einem Exportauftrag verschlüsselt der Import-/Export-Dienst Ihre Daten, bevor das Laufwerk an Sie zurückgesendet wird.</p>
</div>
...
1.	**Abonnement und Speicherkonten:** Sie müssen über ein Azure-Abonnement und ein oder mehrere Speicherkonten verfügen, um den Import-/Export-Dienst nutzen zu können. Bei jedem Auftrag können lediglich Daten auf ein oder von einem Speicherkonto übertragen werden. In anderen Worten: Ein Auftrag kann nicht mehrere Speicherkonten umfassen. Unter [Erstellen eines Speicherkontos](../storage-create-storage-account/)finden Sie weitere Informationen zum Erstellen eines neuen Speicherkontos.
...
Bevor Sie einen Importauftrag erstellen, müssen Sie Ihre Laufwerke mit dem Microsoft Azure Import-/Exporttool vorbereiten. In der [Referenz zum Microsoft Azure-Import-/Exporttool](http://go.microsoft.com/fwlink/?LinkId=329032)finden Sie weitere Informationen zur Verwendung des Microsoft Azure-Import-/Exporttools. Sie können das [Microsoft Azure-Import-/Exporttool](http://go.microsoft.com/fwlink/?LinkID=301900&clcid=0x409) als eigenständiges Paket herunterladen.
...
3.	Geben Sie im zweiten Schritt die Kontaktinformationen des Ansprechpartners für diesen Importauftrag an. Wenn Sie ausführliche Protokolldaten für den Importauftrag speichern möchten, sollten Sie die Option zum <strong>Speichern des ausführlichen Protokolls im Blob-Container "waimportexport"</strong>aktivieren.
...
6. 	Wählen Sie in Schritt 5 Ihren Rücktransporteur aus und geben Sie Ihr Kundenkonto beim Transportunternehmen an. Microsoft verwendet diese Kontodaten, um Ihre Laufwerke nach Abschluss des Importauftrags an Sie zurückzuschicken.
	...
	Wenn Sie noch keine Nachverfolgungsnummer haben, wählen Sie **Ich stelle meine Versandinformationen für diesen Importauftrag zur Verfügung, nachdem ich mein Paket versendet habe** aus, und schließen Sie dann den Importvorgang ab.

7. Wenn Sie Ihre Nachverfolgungsnummer eingeben möchten, nach Sie Ihr Paket versendet haben, kehren Sie zur Seite **Import/Export** für Ihr Speicherkonto im Verwaltungsportal zurück, wählen Sie Ihren Auftrag aus der Liste aus, und wählen Sie **Versandinformationen** aus. Navigieren Sie durch den Assistenten und geben Sie Ihre Nachverfolgungsnummer in Schritt 2 ein.
...
1. 	Navigieren Sie zum Speicherkonto im Verwaltungsportal, und zeigen Sie das Dashboard an, um einen Exportauftrag zu erstellen. 	Klicken Sie unter <strong>Auf einen Blick</strong>auf <strong>Exportauftrag erstellen,</strong>und fahren Sie mit den Schritten im Assistenten fort.

2. 	Geben Sie im zweiten Schritt die Kontaktinformationen des Ansprechpartners für diesen Exportauftrag an. Wenn Sie ausführliche Protokolldaten für den Exportauftrag speichern möchten, sollten Sie die Option zum <strong>Speichern des ausführlichen Protokolls im Blob-Container "waimportexport"</strong>aktivieren.
...
3.	Legen Sie im dritten Schritt fest, welche Blob-Daten Sie von Ihrem Speicherkonto auf Ihr leeres Laufwerk oder Ihre Laufwerke exportieren möchten. Sie können alle Blob-Daten des Speicherkontos exportieren, oder Sie legen fest, welche Blobs oder Blob-Sätze exportiert werden sollen.
	...
	- Verwenden Sie die Auswahl **Entspricht**, und geben Sie den relativen Pfad zu dem Blob an, beginnend mit dem Containernamen, um den zu exportierenden Blob festzulegen. Verwenden Sie $root, um den Stammcontainer festzulegen.
	- Verwenden Sie die Auswahl **Beginnt mit**, und legen Sie das Präfix beginnend mit einem Schrägstrich "/" fest, um alle Blobs festzulegen, die mit einem Präfix beginnen. Bei dem Präfix kann es sich um das Präfix des Containernamens, den vollständigen Containernamen oder den vollständigen Containernamen gefolgt vom Präfix des Blob-Namens handeln.
	...
	<table border="1" cellspacing="0" cellpadding="5" style="border: 1px solid #000000;">
		<tbody>
			...
			<tr>
				<td>Beginnt mit</td>
				<td>/music/love</td>
				<td>Exportiert alle Blobs im Container <strong>music,</strong> die mit dem Präfix <strong>love beginnen</strong></td>
			</tr>
			<tr>
				<td>Entspricht</td>
				<td>$root/logo.bmp</td>
				<td>Exportiert den Blob <strong>logo.bmp</strong> in den Stammcontainer</td>
			</tr>
			<tr>
				<td>Entspricht</td>
				<td>videos/story.mp4</td>
				<td>Exportiert den Blob <strong>story.mp4</strong> in den Container <strong>videos</strong></td>
			</tr>
		</tbody>
	</table>
...
5. 	Wählen Sie in Schritt 5 Ihren Rücktransporteur aus und geben Sie Ihr Kundenkonto beim Transportunternehmen an. Microsoft verwendet diese Kontodaten, um Ihre Laufwerke nach Abschluss des Exportauftrags an Sie zurückzuschicken.
	...
	Wenn Sie noch keine Nachverfolgungsnummer haben, wählen Sie <strong>Ich stelle meine Versandinformationen für diesen Exportauftrag zur Verfügung, nachdem ich mein Paket versendet habe</strong>aus, und schließen Sie dann den Exportvorgang ab.

6. Wenn Sie Ihre Nachverfolgungsnummer eingeben möchten, nach Sie Ihr Paket versendet haben, kehren Sie zur Seite **Import/Export** für Ihr Speicherkonto im Verwaltungsportal zurück, wählen Sie Ihren Auftrag aus der Liste aus, und wählen Sie **Versandinformationen** aus. Navigieren Sie durch den Assistenten und geben Sie Ihre Nachverfolgungsnummer in Schritt 2 ein.

	Falls der Auftrag den Status "Erstellung", "Versand" oder "Übertragung" hat, können Sie außerdem Ihre Kontonummer beim Transportunternehmen in Schritt 2 des Assistenten eingeben. Falls der Auftrag den Status "Verpackung" hat, können Sie Ihre Kontonummer beim Transportunternehmen nicht mehr ändern. 


## Nachverfolgen des Auftragsstatus im Verwaltungsportal##
...
- Preisinformationen stehen auf der [Preisseite](http://go.microsoft.com/fwlink/?LinkId=329033) zur Verfügung.
...
- Für Regionen in den USA und Europa wird nur [Federal Express](http://www.fedex.com/us/oadr/) (FedEx) unterstützt. Alle Pakete werden per FedEx Ground oder FedEx International Economy zurückgeschickt.

- Für Regionen in Asien wird nur [DHL](http://www.dhl-welcome.com/Tutorial/) unterstützt. Alle Pakete werden per DHL Express Worldwide zurückgeschickt.

	<div class="dev-callout">
	<strong>Wichtig:</strong>
	<p>Sie müssen Ihre Tracking-Nummer an den Azure Import-/Export-Dienst übertragen, ansonsten kann Ihr Auftrag nicht verarbeitet werden.</p>
	</div>

**Entstehen Kosten für den Rückversand?**

- Microsoft verwendet die Kontonummer für das Transportunternehmen, die Sie bei der Auftragserstellung angegeben haben, um die Laufwerke vom Rechenzentrum an Ihre Absenderadresse zu schicken. Geben Sie daher unbedingt eine Kontonummer für ein Transportunternehmen an, das in der Region des Rechenzentrums unterstützt wird. Sie können ein Konto bei [FedEx](http://www.fedex.com/us/oadr/) (für die USA und Europa) oder bei [DHL](http://www.dhl-welcome.com/Tutorial/) (Asien) erstellen, falls Sie nicht über ein solches Konto verfügen.
