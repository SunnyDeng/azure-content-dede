...
-   [Python auf HDInsight][Python auf HDInsight]
-   [Hive und Python][Hive und Python]
-   [Pig und Python][Pig und Python]
...
2.  Die Anweisung **SELECT TRANSFORM ... USING 'D:\\Python27\\python.exe streaming.py'** wählt Daten aus **hivesampletable** aus und übergibt clientid, devicemake und devicemodel an das Skript **streaming.py**.
    ...
    > [WACOM.NOTE] Die **USING**-Klausel legt den vollständigen Pfad zu python.exe fest, da das nicht im Pfad ist.

3.  Die **AS**-Klausel beschreibt die Felder, die von **streaming.py** zurückgegeben werden.

<a name="streamingpy"></a>
Hier ist die Datei **streaming.py**, die vom HiveQL-Beispiel verwendet wird.
...
2.  Das Zeilenumbruchzeichen am Ende wird mit `string.strip(line, "\n ")` entfernt, da wir nur die Textdaten möchten und keinen Indikator für das Zeilenende.

3.  Bei der Streamverarbeitung enthält eine einzelne Zeile alle Werte, jeweils getrennt durch ein Tabulatorzeichen. Deshalb kann `string.split(line, "\t")` zum Teilen der Eingabe bei jedem Tab verwendet werden, sodass nur diese Felder zurückgeben werden.

4.  Wenn die Verarbeitung abgeschlossen ist, muss die Ausgabe als eine Zeile nach STDOUT geschrieben werden mit einem Tabulator zwischen jedem Feld. Das wird erreicht mithilfe von `print "\t".join([clientid, phone_label, hashlib.md5(phone_label).hexdigest()])`.

5.  All das ereignet sich in einer `while`-Schleife, die sich wiederholt, bis keine `line` mehr gelesen wird; dann verlässt `break` die Schleife und das Skript wird beendet.
...
<a name="jythonpy"></a>
Das ist die **jython.py**-Datei, die im Pig-Beispiel verwendet wird:
...
Diese Schritte verwenden Microsoft Azure PowerShell. Wen es nicht bereits auf Ihrem Entwicklungsrechner installiert und konfiguriert ist, sehen Sie sich bitte [Installieren und Konfigurieren von Azure PowerShell][Installieren und Konfigurieren von Azure PowerShell] an und folgen Sie den Schritten dort.

1.  Erstellen Sie mithilfe der Python-Beispiele [streaming.py][streaming.py] und [jython.py][jython.py] lokale Kopien der Dateien auf Ihrem Entwicklungscomputer.
...
2.  Verwenden Sie das folgende PowerShell-Skript zum Hochladen der Dateien **streaming.py** und **jython.py** auf den Server. Ersetzen Sie den Namen Ihres HDInsight-Clusters und den Pfad zu den Dateien **streaming.py** und **jython.py** in den ersten drei Skriptzeilen.
    ...
    > [WACOM.NOTE] Andere Methoden zum Hochladen der Skripts finden sich im Dokument [Hochladen von Daten für Hadoop-Jobs in HDInsight][Hochladen von Daten für Hadoop-Jobs in HDInsight].
...
3.  Klicken Sie auf die Schaltfläche **Senden**, um den Job abzusenden. Je nach Version des HDInsight-Clusters werden Sie möglicherweise auf die Seite „Job-Details“ weitergeleitet. Wenn nicht, wählen Sie **Details anzeigen** im Bereich **Job-Sitzung** unten auf der Seite.

4.  Die Seite **Job-Details** wird neu geladen, bis der Job abgeschlossen ist. Nachdem das der Fall ist, werden Informationen über den Job sowie die Ausgabe angezeigt.
...
**So führen Sie den Hive-Job aus**
...
<td>
Sehen Sie sich diese Dateien im BLOB-Container an
...
Wenn Sie Python-Module laden müssen, die standardmäßig nicht bereitgestellt werden, sehen Sie sich [Bereitstellen eines Moduls für Azure HDInsight][Bereitstellen eines Moduls für Azure HDInsight], wo Sie ein Beispiel finden, wie das geht.

Wenn Sie Jobs remote auf HDInsight ohne PowerShell ausführen möchten, finden Sie unter [Einsatz von Azure HDInsight unter Linux][Einsatz von Azure HDInsight unter Linux] ein Beispiel, wie Sie mit Python Jobs über die WebHCat REST API ausführen.

  [Python auf HDInsight]: #python
  ...
  [jython.py]: #jythonpy
  [Hochladen von Daten für Hadoop-Jobs in HDInsight]: /de-de/documentation/articles/hdinsight-upload-data/
  [Bereitstellen eines Moduls für Azure HDInsight]: http://blogs.msdn.com/b/benjguin/archive/2014/03/03/how-to-deploy-a-python-module-to-windows-azure-hdinsight.aspx
  [Einsatz von Azure HDInsight unter Linux]: http://blogs.msdn.com/b/benjguin/archive/2014/02/18/how-to-use-hdinsight-from-linux.aspx
