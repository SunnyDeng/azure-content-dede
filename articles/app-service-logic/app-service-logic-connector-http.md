...
Logik-Apps können basierend auf einer Vielzahl von Datenquellen ausgelöst werden und Connectors anbieten, um Daten als Teil des Datenflusses abzurufen und zu verarbeiten. Es gibt jedoch einige Szenarien, in denen Sie möglicherweise mit direkten HTTP-Verbindungen arbeiten müssen, einschließlich:
...
2. Suchen Sie nach "HTTP", wählen Sie den HTTP-Listener, und wählen Sie dann **Erstellen**.
...
7. Abschließend legen Sie die Sicherheitseinstellungen der API-App auf "Öffentlich (Anonym)" fest, um externen Clients den Zugriff auf den Endpunkt zu erlauben. Diese Einstellung steht unter "Alle Einstellungen > Anwendungseinstellungen" der HTTP-Listener-API-App zur Verfügung: ![][10]
