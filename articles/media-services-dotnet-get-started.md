<properties urlDisplayName="Get Started with Media Services" pageTitle="Erste Schritte mit Media Services | Azure" metaKeywords="Azure media services" description="Eine Einf&uuml;hrung in Media Services mit Azure." metaCanonical="" services="media-services" documentationCenter="" title="Erste Schritte mit Mediendiensten" authors="juliako" solutions="" manager="dwrede" editor="" />
...
> [WACOM.NOTE]
> Sie benötigen ein Azure-Konto, um dieses Lernprogramm durchführen zu können. Wenn Sie noch kein Konto haben, können Sie in nur wenigen Minuten ein kostenloses Testkonto erstellen. Einzelheiten finden Sie unter [Kostenlose Azure-Testversion][Kostenlose Azure-Testversion].
...
5.  Erstellen Sie einen Ordner auf Ihrem lokalen Computer mit dem Namen supportFiles (in diesem Beispiel befindet sich supportFiles unterhalb des MediaServicesGettingStarted-Projektordners.) Das zugehörige [Projekt][Download] zu diesem Lernprogramm enthält den supportFiles-Ordner. Sie können den Inhalt dieses Ordners in Ihren supportFiles-Ordner kopieren.
...
## <span id="Step2"></span></a>Abrufen des Media Services-Kontexts
...
## <span id="Step3"></span></a>Erstellen eines Medienobjekts und Hochladen einer Datei
...
Fügen Sie einen Aufruf an die Methode nach der Zeile \*\*\_context = new CloudMediaContext(\_accountName, \_accountKey);\*\* in Ihrer Main-Methode hinzu.
...
1.  Fügen Sie die folgende **CreateEncodingJob**-Methode zu Ihrer Klasse hinzu. Diese Methode demonstriert die Ausführung verschiedener Aufgaben für einen Codierungsauftrag:
    ...
    -   Definieren eines Medienprozessors für die Verarbeitung des Auftrags. Der Medienprozessor ist für Codierung, Verschlüsselung, Formatumwandlungen und andere verwandte Verarbeitungsaufträge zuständig. Insgesamt sind mehrere verschiedene Arten von Medienprozessoren verfügbar (Sie können die komplette Liste mit \_context.MediaProcessors durchlaufen.) Die später in dieser Anleitung gezeigte Methode GetLatestMediaProcessorByName gibt den Azure Media Encoder-Prozessor zurück.
    -   Definieren einer neuen Aufgabe. Jeder Auftrag enthält eine oder mehrere Aufgaben. Sie übergeben mit der Aufgabe einen lesbaren Namen, eine Medienprozessor-Instanz, eine Aufgabenkonfigurations-Zeichenfolge und Optionen für die Aufgabenerstellung. Die Konfigurations-Zeichenfolge gibt die Codierungseinstellungen vor. Dieses Beispiel verwendet die Einstellung **H264 Broadband 720p**. Diese Voreinstellung produziert eine einzige MP4-Datei. Weitere Informationen zu voreingestellten Codierungswerten finden Sie unter [Systemvoreinstellungen für den Azure Media Encoder][1].
...
3.  Fügen Sie die folgenden Helfermethoden zur Klasse hinzu. Diese Methoden unterstützten die **CreateEncodingJob**-Methode. Es folgt eine Zusammenfassung der Helfermethoden.
    ...
    -   Die **DownloadAssetToLocal**-Methode lädt die einzelnen Dateien des Medienobjekts in einen lokalen Ordner herunter. In diesem Beispiel wurde das Medienobjekt mit einer Eingabe-Mediendatei erstellt, und die Sammlung der Ausgabe-Medienobjekte enthält zwei Dateien: die codierte Mediendatei (im MP4-Format) und eine XML-Datei mit Metadaten des Medienobjekts. Die Methode lädt beide Dateien herunter.
...
> [WACOM.NOTE]
> Im Media Services-Objektmodell bezeichnet ein Medienobjekt eine Sammlung von Media Services-Inhalten, die eine oder mehrere Dateien enthält. Der Locatorpfad liefert eine Azure-Blob-URL, die als Basispfad zu diesem Medienobjekt im Azure-Speicher dient. Fügen Sie einen Dateinamen zum Locator-Basispfad hinzu, um eine einzelne Datei im Medienobjekt abzurufen.
...
<!-- Anchors. -->
  ...
  [Kostenlose Azure-Testversion]: http://www.windowsazure.com/de-de/pricing/free-trial/?WT.mc_id=A8A8397B5
  ...
  [Verwenden von Media Services]: http://azure.microsoft.com/de-de/develop/media-services/resources/
