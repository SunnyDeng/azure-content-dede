<properties 
	pageTitle="Debuggen von Hadoop in HDInsight: Fehlermeldungen | Azure" 
...
Die Fehler, auf die ein Benutzer in Azure PowerShell oder auf dem Azure-Portal stoßen kann, werden im Abschnitt [HDInsight-Fehlern]alphabetisch nach Name aufgeführt.(#hdinsight-error-messages) Sie sind mit dem entsprechenden Eintrag im Abschnitt mit der [Beschreibung und Lösung der Fehler](#discription-mitigation-errors) verknüpft, der die folgenden Informationen zu dem Fehler enthält:
...
- **Beschreibung**: Es konnte kein Cluster in der Region *nameOfYourRegion* erstellt werden. Verwenden Sie eine gültige HDInsight-Region, und wiederholen Sie die Anforderung.   
...
- **Beschreibung**: Der Cluster-DNS-Name *yourDnsName* ist ungültig. Stellen Sie sicher, dass der Name mit einem alphanumerischen Zeichen beginnt und endet. Er darf nur das Sonderzeichen "-" enthalten.  
...
- **Beschreibung**: Der Clustername *yourClusterName* ist ungültig. Wählen Sie einen anderen Namen.  
...
- **Beschreibung**: Der Cluster-DNS-Name *yourDnsClusterName* ist ungültig. Stellen Sie sicher, dass der Name mit einem alphanumerischen Zeichen beginnt und endet. Er darf nur das Sonderzeichen "-" enthalten.  
...
- **Beschreibung**: Der Containername in URI *yourcontainerURI* und der DNS-Name *yourDnsName* im Anforderungstext müssen identisch sein.  
...
- **Beschreibung**: Es wurde zweimal versucht, den Clustercontainer zu erstellen. Für *nameOfYourContainer* ist ein Datensatz vorhanden, die ETags stimmen jedoch nicht überein.   
...
- **Beschreibung**: Der gehostete Dienst *nameOfYourHostedService* enthält bereits einen Cluster. Ein gehosteter Dienst kann nicht mehrere Cluster enthalten.  
...
- **Beschreibung**: Der Cluster *yourClusterName* wurde im Rahmen von Wartungsarbeiten gelöscht. Erstellen Sie den Cluster neu.     
...
- **Beschreibung**: Der gehostete Dienst *nameOfYourHostedService* konnte nicht erstellt werden. Wiederholen Sie die Anforderung.  
...
- **Beschreibung**: Der gehostete Dienst *nameOfYourHostedService* enthält bereits eine Produktionsbereitstellung. Ein gehosteter Dienst kann nicht mehrere Produktionsbereitstellungen enthalten. Wiederholen Sie die Anforderung mit einem anderen Clusternamen.   
...
- **Beschreibung**: Der gehostete Dienst *nameOfYourHostedService* konnte für den Cluster nicht gefunden werden.  
...
- **Beschreibung**: Dem gehosteten Dienst *nameOfYourHostedService* wurde keine Bereitstellung zugeordnet.  
- **Lösung**: Wenn der Cluster im Fehlerzustand ist, löschen Sie ihn, und versuchen Sie es erneut. 

<h3><a id="InsufficientResourcesCores"></a>InsufficientResourcesCores</h3>
- **Beschreibung**: Die Abonnement-ID *yourSubscriptionId* hat keine Kerne mehr übrig, um den Cluster *yourClusterName* zu erstellen. Erforderlich: *resourcesRequired*, Verfügbar: *resourcesAvailable*.  
...
- **Beschreibung**: Die Abonnement-ID *yourSubscriptionId* verfügt für einen neuen gehosteten Dienst über kein Kontingent, um den Cluster *yourClusterName* zu erstellen.  
- **Lösung**: Geben Sie Ressourcen in Ihrem Abonnement frei, oder erhöhen Sie die für das Abonnement verfügbaren Ressourcen, und versuchen Sie erneut, den Cluster zu erstellen.
...
- **Beschreibung**: Der Ort für den Azure-Speicher *dataRegionName* ist kein gültiger Standort. Stellen Sie sicher, dass die Region korrekt ist, und wiederholen Sie die Anforderung.   
...
- **Beschreibung**: Die verwendete Abonnement-ID *yourSubscriptionId* verfügt nicht über ausreichende Berechtigungen, um den Löschvorgang für den Cluster *yourClusterName* auszuführen.  
...
- **Beschreibung**: Der Blob-Containername *yourContainerName* des externen Speicherkontos ist ungültig. Stellen Sie sicher, dass der Name mit einem Buchstaben beginnt und nur Kleinbuchstaben, Ziffern und Bindestriche enthält.  
...
- **Beschreibung**: Die Konfiguration für das externe Speicherkonto *yourStorageAccountName* ist erforderlich, damit Details zum geheimen Schlüssel festgelegt werden können.  
...
- **Beschreibung**: Der Versionsheader *yourVersionHeader* hat nicht das gültige Format JJJJ-MM-TT.  
...
- **Beschreibung**: Der Parameter *yourParameterName* darf nicht null oder leer sein.  
...
- **Beschreibung**: Die Regionsfunktion ist für Region *yourRegionName* und Abonnement-ID *yourSubscriptionId* nicht verfügbar.  
...
- **Beschreibung**: Speicherkonto *yourStorageAccountName* ist in Region *currentRegionName*. Sie sollte identisch mit der Clusterregion *yourClusterRegionName* sein.  
...
- **Beschreibung**: Die Abonnement-ID *yourSubscriptionId* wurde nicht gefunden.  
...
- **Beschreibung**: DNS *yourDnsUrl* konnte nicht aufgelöst werden. Stellen Sie sicher, dass die vollqualifizierte URL für den Blob-Endpunkt bereitgestellt wird.  
...
- **Beschreibung**: Der Speicherort für die Ressource *yourDnsUrl* konnte nicht überprüft werden. Stellen Sie sicher, dass die vollqualifizierte URL für den Blob-Endpunkt bereitgestellt wird.  
- **Lösung**: Geben Sie eine gültige Blob-URL an. Die URL MUSS uneingeschränkt gültig sein, das heißt, sie muss mit *http://* beginnen und mit *.com* enden. Die vollqualifizierte URL befindet sich in der Regel auf der Speicher-Registerkarte des Portals unter manage.windowsazure.com. 

<h3><a id="VersionCapabilityNotAvailable"></a>VersionCapabilityNotAvailable</h3>
- **Beschreibung**: Die Versionsfunktion ist für Version *specifiedVersion* und Abonnement-ID *yourSubscriptionId* nicht verfügbar.  
...
- **Beschreibung**: Version *specifiedVersion* nicht verfügbar in Azure-Region *specifiedRegion*.  
