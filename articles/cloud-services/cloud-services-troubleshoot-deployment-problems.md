...
Wenn Sie ein Anwendungspaket für einen Clouddienst in Azure bereitstellen, können Sie im Bereich **Eigenschaften** des Verwaltungsportals Informationen zu der Bereitstellung abrufen. Mithilfe der Informationen in diesem Bereich können Sie Probleme mit dem Clouddienst behandeln, und Sie können dem Azure-Support diese Informationen bereitstellen, wenn Sie eine neue Supportanfrage eröffnen.

> [AZURE.NOTE]Sie können den Inhalt des Eigenschaftenbereichs in die Zwischenablage kopieren, indem Sie auf das Symbol in der oberen rechten Ecke des Bereichs klicken.
...
Der im Portal angezeigte Website-URL enthält den Port nicht. Der Standardport für Websites lautet 80. Falls Ihre Anwendung jedoch so konfiguriert ist, dass sie einen anderen Port verwendet, müssen Sie der URL den richtigen Port hinzufügen, wenn Sie auf die Website zugreifen.

1. Klicken Sie im Verwaltungsportal auf die Bereitstellung Ihres Clouddiensts.
...
3. Falls der Port nicht *80* lautet, fügen Sie der URL den richtigen Portwert hinzu, wenn Sie auf die Anwendung zugreifen. Um einen nicht standardmäßigen Port anzugeben, geben Sie die URL gefolgt von einem Doppelpunkt (:) und der Portnummer ohne Leerzeichen ein.
...
1. Klicken Sie im Verwaltungsportal auf die Bereitstellung Ihres Clouddiensts.
2. Überprüfen Sie die Informationen im Bereich **Eigenschaften** des Verwaltungsportals, und stellen Sie fest, ob während der Zeitraums, in dem Sie den Neustart der Rollen beobachtet haben, eine Dienstreparatur erfolgt ist.

Rollen werden darüber hinaus etwa einmal pro Monat im Rahmen von Upgrades der Host- und Gastbetriebssysteme neu gestartet. Weitere Informationen finden Sie im Blogpost [Role Instance Restarts Due to OS Upgrades](http://blogs.msdn.com/b/jarrettr/archive/2012/09/19/role-instance-restarts-due-to-os-upgrades.aspx).
...
1. Klicken Sie im Verwaltungsportal auf die Bereitstellung Ihres Clouddiensts.
2. Sehen Sie sich im Bereich **Eigenschaften** des Verwaltungsportals den Wert des **Status** an. Lautet der Wert **Bereit**, überprüfen Sie den Wert **Letzter Vorgang**, um festzustellen, ob kürzlich ein Vorgang ausgeführt wurde, der den VIP-Austausch möglicherweise verhindert hat.
...
Diese Bedingung kann auf ein Problem mit dem Anwendungscode, dem Anwendungspaket oder der Konfigurationsdatei hinweisen. Wenn diese Bedingung zutrifft, sehen Sie, dass der Status sich alle paar Minuten ändert und im Azure-Verwaltungsportal Meldungen wie **Recycling**, **Ausgelastet** oder **Wird initialisiert** angezeigt werden. Dies weist darauf hin, dass ein Fehler in der Anwendung vorliegt, der verhindert, dass die Rolleninstanz ausgeführt wird.

Weitere Informationen zum Behandeln dieses Problems finden Sie in den Blogposts [Azure PaaS Compute Diagnostics Data] und [Common Issues Which Cause Roles to Recycle](cloud-services-troubleshoot-common-issues-which-cause-roles-recycle.md).

## Problem: Meine Anwendung funktioniert nicht mehr.

1. Klicken Sie im Verwaltungsportal auf die Rolleninstanz.
2. Sehen Sie sich im Bereich **Eigenschaften** des Verwaltungsportals die folgenden Bedingungen an, die Ihnen bei der Problembehebung helfen können:
   ...
   * Weitere Informationen erhalten Sie in den Diagnosedaten und Problembehandlungsszenarien im Blogpost [Azure PaaS Compute Diagnostics Data].
