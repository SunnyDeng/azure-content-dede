...
Bei der Entscheidung, ob Sie Azure-Tabellenspeicher verwenden möchten, müssen Sie Kosten und Leistung zu berücksichtigen. Die Verwendung von Tabellenspeicher ist viel wirtschaftlicher als die Speicherung der Daten in einer SQL-Datenbank. Jedoch müssen Sie sorgfältig berücksichtigen, inwieweit die Daten relationale SQL-Funktionen wie z. B. Verknüpfungen, Filter, Abfragen usw. verwenden dürfen. Wenn die Daten diese Funktionen wenig nutzen, ist dies ein guter Kandidat für die Speicherung in einer Azure-Tabelle.
...
* [Anleitung zu Leistung und Flexibilität von SQL-Datenbanken](http://go.microsoft.com/fwlink/?LinkId=221876) 
...
Planen Sie, ausreichend Cachingkapazitäten bereitzustellen, damit sie Objekte zwischenspeichern können, auf die häufig zugegriffen wird. In SQL-Datenbank werden häufig Referenztabellen verwendet, um numerische Codes in längere beschreibende Zeichenfolgen zu konvertieren. Diese Tabellen enthalten häufig Daten wie z. B. die Namen von Ländern und Städten, gültige Postleitzahlen, Namen von Abteilungen in Ihrem Unternehmen usw.. Bei kleineren Tabellen kann es sinnvoll sein, die gesamte Tabelle im Cache zu speichern, bei anderen speichern Sie möglicherweise nur die am häufigsten genutzten Werte. Der Leistungsgewinn resultiert aus Abfragen mit mehreren Verknüpfungen, die an diesen Daten ausgeführt werden: bei jedem Wert, der im Cache gefunden wird, werden einige Festplattenzugriffe gespeichert. Eine gute Einführung und Diskussion zum Thema Leistung und Caching in Azure finden Sie unter [Introducing the Azure Caching Service](http://go.microsoft.com/fwlink/?LinkId=252680). Einen neueren Blogbeitrag zu diesem Thema finden Sie unter [Windows #Azure Caching Performance Considerations](http://go.microsoft.com/fwlink/?LinkId=252681).
...
Azure verfügt über zwei alternative Warteschlangentechnologien: Azure Storage Queues und Service Bus.
...
* [Azure HDInsight-Dienst](/documentation/services/hdinsight/)
...
* [Überwachen der SQL-Datenbank mit dynamischen Verwaltungssichten](http://go.microsoft.com/fwlink/?LinkId=236195) 
