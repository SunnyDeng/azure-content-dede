...
DocumentDB bietet ein einfaches und offenes REST-konformes Programmiermodell über HTTP. In der Vorschauversion stellt DocumentDB Client-SDKs für .NET, Python, Node.js und JavaScript bereit, die alle über die zugrundeliegenden REST APIs einfache Wrapper darstellen. In zukünftigen Versionen werden auch SDKs für C++ und Java bereitgestellt. Schreiben Sie eigene SDKs für Ihre spezifische Programmierumgebung und teilen Sie sie mit der Community, da wir unsere SDKs erweitern.
...
Die vom DocumentDB verwalteten Entitäten werden als **Ressourcen** bezeichnet, die durch ihren logischen URI eindeutig identifiziert werden. Entwickler können unter Verwendung von Standard HTTP-Verben, Anforderungs-/Antwortheader und Statuscodes mit den Ressourcen interagieren. Wie in der folgenden Abbildung dargestellt, besteht das **Ressourcenmodell** von DocumentDB aus Ressourcensätzen in einem Datenbankkonto, wobei jeder Satz über einen logischen und stabilen URI aufrufbar ist. Ein Ressourcensatz wird in diesem Dokument als **Feed** bezeichnet.

![][0]

## Hierarchisches Ressourcenmodell für ein Datenbankkonto

Als Kunde von DocumentDB können Sie mithilfe Ihres Azure-Abonnements ein DocumentDB-**Datenbankkonto** bereitstellen. Ein Datenbankkonto besteht aus einem Satz von **Datenbanken**, die jeweils mehrere **Sammlungen** beinhalten, welche wiederum **gespeicherte Prozeduren, Trigger, UDFs, Dokumente** und verwandte **Anhänge** beinhalten. Eine Datenbank hat ebenfalls zugehörige **Benutzer** mit dem jeweiligen Satz an **Berechtigungen** für den Zugriff auf zahlreiche andere Sammlungen, gespeicherte Prozeduren, Trigger, UDFs, Dokumente und Anhänge. Während Datenbanken, Benutzer, Berechtigungen und Sammlungen vom System definierte Ressourcen mit bekannten Schemata darstellen, sind in Dokumenten und Anhängen beliebige, benutzerdefinierte JSON-Inhalte enthalten.

| Ressource             | Beschreibung                                                                                                                                                                                                                                                                     |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Datenbankkonto        | Ein Datenbankkonto ist mit einer oder mehreren Kapazitätseinheiten verknüpft, die bereitgestellten Dokumentspeicher und -durchsatz, ein Satz an Datenbanken und Blobspeicher darstellen. Sie können ein oder mehrere Datenbankkonten mithilfe Ihres Azure-Abonnements erstellen. |
| Datenbank             | Eine Datenbank ist ein logischer Datenspeichercontainer, der von allen Sammlungen geteilt wird. Sie ist auch ein Benutzercontainer.                                                                                                                                              |
| Benutzer              | Logischer Namespace für Bereichs-/geteilte Berechtigungen.                                                                                                                                                                                                                       |
| Berechtigung          | Ein mit einem Benutzer verknüpftes Autorisierungstoken für autorisierten Zugriff auf eine bestimmte Ressource.                                                                                                                                                                   |
| Sammlung              | Eine Sammlung ist ein JSON-Dokumentcontainer, der mit der JavaScript-Anwendungslogik verknüpft ist.                                                                                                                                                                              |
...
### Systemdefinierte vs. benutzerdefinierte Ressourcen

Ressourcen wie Datenbankkonten, Datenbanken, Sammlungen, Benutzer, Berechtigungen, gespeicherte Prozeduren, Trigger und UDFs haben alle ein festgelegtes Schema und werden al *Systemressourcen* bezeichnet. Im Gegensatz dazu verfügen Ressourcen wie Dokumente und Anhänge über keine Schemaeinschränkungen und stellen Beispiele *benutzerdefinierter Ressourcen* dar. In DocumentDB werden sowohl systemdefinierte als auch benutzerdefinierte Ressourcen als standardkonforme JSON dargestellt und verwaltet. Alle Ressourcen, egal ob system- oder benutzerdefiniert, haben folgende Eigenschaften gemeinsam.
...
<tbody>
<tr class="odd">
<td align="left"><p><strong>Eigenschaft</strong></p></td>
<td align="left"><p><strong>Vom Benutzer einstellbar oder systemgeneriert?</strong></p></td>
<td align="left"><p><strong>Zweck</strong></p></td>
</tr>
<tr class="even">
<td align="left"><p>_rid</p></td>
<td align="left"><p>Systemgeneriert</p></td>
<td align="left"><p>Systemgeneriert, eindeutige und hierarchische ID der Ressource</p></td>
</tr>
<tr class="odd">
<td align="left"><p>_etag</p></td>
<td align="left"><p>Systemgeneriert</p></td>
<td align="left"><p>ETag der Ressource, das für die Steuerung optimistischer Parallelität erforderlich ist</p></td>
</tr>
<tr class="even">
<td align="left"><p>_ts</p></td>
<td align="left"><p>Systemgeneriert</p></td>
<td align="left"><p>Zuletzt aktualisierter Zeitstempel der Ressource</p></td>
</tr>
<tr class="odd">
<td align="left"><p>_self</p></td>
<td align="left"><p>Systemgeneriert</p></td>
<td align="left"><p>Eindeutiger aufrufbarer URI der Ressource</p></td>
</tr>
<tr class="even">
<td align="left"><p>id</p></td>
<td align="left"><p>Vom Benutzer einstellbar</p></td>
<td align="left"><p>Benutzerdefinierter eindeutiger Name der Ressource</p></td>
</tr>
</tbody>
</table>
...
Zum Ausführen der gespeicherten Prozedur aus dem oben genannten Beispiel muss POST mit dem URI der gespeicherten Prozedurresssource ausgeführt werden (/dbs/\_rid-db/colls/\_rid-coll/sprocs/sproc1). Dies ist im Folgenden dargestellt:
...
<tbody>
<tr class="odd">
<td align="left"><p><strong>Konnektivitätsmodus</strong></p></td>
<td align="left"><p><strong>Protokoll</strong></p></td>
<td align="left"><p><strong>Details</strong></p></td>
<td align="left"><p><strong>DocumentDB SDKs</strong></p></td>
</tr>
<tr class="even">
<td align="left"><p>Gateway</p></td>
<td align="left"><p>HTTPS</p></td>
<td align="left"><p>Anwendungen können unter Verwendung logischer URIs eine direkte Verbindung mit den Egdeknoten herstellen. Dabei fällt ein zusätzlicher Netzwerkhop an.</p></td>
<td align="left"><p>REST-APIs</p>
<p>.NET, JavaScript, Node.js, Python</p></td>
</tr>
<tr class="odd">
<td align="left"><p>Direkte Konnektivität</p></td>
<td align="left"><p>HTTPS und</p>
<p>TCP</p></td>
<td align="left"><p>Die Anwendungen können direkt auf die Routingtabelle zugreifen und die direkte Verbindung mit den Replikaten durch das clientseitige Routing herstellen.</p></td>
...
-   JSON-Spezifikation [][2]<http://-www.ietf.org/rfc/rfc4627.txt></a>
-   HTTP-Spezifikation [][3]<http://www.w3.org/Protocols/rfc2616/rfc2616.html></a>
-   Entitätstags [][4]<http://en.wikipedia.org/wiki/HTTP_ETag></a>
-   [Abfragen von DocumentDB][Abfragen von DocumentDB]
-   [DocumentDB SQL-Referenz][DocumentDB SQL-Referenz]
-   [DocumentDB-Programmierung: Gespeicherte Prozeduren, Trigger und UDFs][DocumentDB-Programmierung: Gespeicherte Prozeduren, Trigger und UDFs]
-   [DocumentDB-Referenzdokumentation][DocumentDB-Referenzdokumentation]
