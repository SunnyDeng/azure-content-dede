...
| [Reagieren auf eine Azure-Warnung](automation-webhooks.md) | <li>Starten eines Runbooks als Reaktion auf eine Azure-Warnung.<br> <li>Konfigurieren eines Webhooks für Runbook und Link zum Warnen.<br> <li>Authentifizierung über Sicherheitstoken in der URL.<br> <li>Unterstützt derzeit nur Warnungen für Metriken. |
...
Die folgende Abbildung veranschaulicht detailliert die Prozessschritte im Lebenszyklus eines Runbooks. Dies umfasst verschiedene Arten, auf die ein Runbook in Azure Automation gestartet werden kann, sowie Komponenten, die erforderlich sind, damit ein lokaler Computer Azure Automation-Runbooks ausführen und Interaktionen zwischen verschiedenen Komponenten unterstützen kann. Weitere Informationen zum Ausführen von Automation-Runbooks in Ihrem Rechenzentrum finden Sie unter [Hybrid Runbook Worker](automation-hybrid-runbook-worker.md).
...
-	Die Runbook-Architektur im aktuellen Artikel bietet eine allgemeine Beschreibung von Hybrid-Runbooks. Weitere Informationen finden Sie unter [Untergeordnete Runbooks in Azure Automation](automation-child-runbooks.md).
