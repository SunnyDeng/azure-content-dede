...
Ein Automation-Anmeldeinformationsobjekt enthält ein [PSCredential](http://msdn.microsoft.com/library/system.management.automation.pscredential)-Objekt, das Sicherheitsanmeldeinformationen wie beispielsweise einen Benutzernamen und ein Kennwort umfasst. Runbooks können Cmdlets verwenden, die zur Authentifizierung ein PSCredential-Objekt akzeptieren, oder das Runbook extrahiert den Benutzernamen und das Kennwort aus dem PSCredential-Objekt, um diese Informationen für eine Anwendung oder einen Dienst bereitzustellen, die bzw. der eine Authentifizierung erfordert. Die Eigenschaften für ein Anmeldeinformationsobjekt werden sicher in Azure Automation gespeichert und können im Runbook über die Aktivität [Get-AutomationPSCredential](http://msdn.microsoft.com/library/system.management.automation.pscredential.aspx) abgerufen werden.
...
Die Cmdlets in der folgenden Tabelle werden zum Erstellen und Verwalten von Automation-Anmeldeinformationsobjekten mit Windows PowerShell verwendet. Sie gehören zum Lieferumfang des [Azure PowerShell-Moduls](../powershell-install-configure.md), das zur Verwendung in Automation-Runbooks verfügbar ist.
...
Die Aktivitäten in der folgenden Tabelle werden für den Zugriff auf Anmeldeinformationen in einem Runbook verwendet.

|Aktivitäten|Beschreibung|
|:---|:---|
|Get-AutomationPSCredential|Ruft ein Anmeldeinformationsobjekt zur Verwendung in einem Runbook auf. Gibt ein [System.Management.Automation.PSCredential](http://msdn.microsoft.com/library/system.management.automation.pscredential)-Objekt zurück.|

>[AZURE.NOTE]Vermeiden Sie die Verwendung von Variablen im Parameter "–Name" von "Get-AutomationPSCredential" in einem Runbook, da dies die Ermittlung von Abhängigkeiten zwischen Runbooks und Verbindungsobjekten zur Entwurfszeit erschweren kann.
