<properties
	...
	description="Erste Schritte mit Azure-BLOB-Speicher in einem ASP.NET 5-Projekt in Visual Studio"
	services="storage"
	documentationCenter=""
	authors="patshea123"
	manager="douge"
...
Dieser Artikel beschreibt, wie die ersten Schritte des Verwendens des Azure-BLOB-Speichers in Visual Studio aussehen, nachdem Sie über das Visual Studio-Dialogfeld „Verbundene Dienste hinzufügen“ in einem ASP.NET 5-Projekt ein Azure-Speicherkonto erstellt oder auf ein solches Konto verwiesen haben.
...
3. Verwenden Sie ein `CloudBlobClient`-Objekt, um eine `CloudBlobContainer`-Referenz auf einen vorhandenen Container in Ihrem Speicherkonto zu erhalten.
...
Sie können auch den `CloudBlobClient` zum Erstellen eines Containers in Ihrem Speicherkonto verwenden. Dazu müssen Sie lediglich wie im folgenden Code einen Aufruf von `CreateIfNotExistsAsync` hinzufügen:
...
Wenn Sie einen BLOB herunterladen möchten, rufen Sie zuerst einen Verweis auf den BLOB ab, und rufen Sie dann die Methode `DownloadToStreamAsync` auf. Im folgenden Beispiel wird die `DownloadToStreamAsync`-Methode verwendet, um den BLOB-Inhalt auf ein Datenstromobjekt zu übertragen, das danach in einer lokalen Datei gespeichert werden kann.
...
##Löschen eines BLOBs
Wenn Sie einen BLOB löschen möchten, rufen Sie zuerst einen Verweis auf den BLOB ab, und rufen Sie dann die Methode `DeleteAsync` für den Verweis auf.
