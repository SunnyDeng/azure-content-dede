...
* Abschluss des Lernprogramms [Erstellen einer Windows-App].
...
>[AZURE.NOTE] Sie benötigen ein Azure-Konto, um dieses Lernprogramm auszuführen. Wenn Sie noch kein Konto haben, können Sie in nur wenigen Minuten ein kostenloses Testkonto erstellen. Einzelheiten finden Sie unter <a href="http://www.windowsazure.com/pricing/free-trial/?WT.mc_id=AE564AB28" target="_blank">1 Monat kostenlose Testversion</a>.

## <a name="enable-offline-app"></a>Aktualisieren der App zur Unterstützung von Offlinefunktionen

Offlinefunktionen der mobilen Azure-App ermöglichen Ihnen die Interaktion mit einer lokalen Datenbank, wenn Sie den Mobile Service offline verwenden. Um diese Funktionen in der App zu verwenden, initialisieren Sie einen `MobileServiceClient.SyncContext` in einem lokalen Speicher. Verweisen Sie anschließend über die `IMobileServiceSyncTable`-Schnittstelle auf Ihre Tabelle. In diesem Lernprogramm verwenden wir SQLite als lokalen Speicher.
...
10. Fügen Sie Ausnahmehandler in der `SyncAsync`-Methode hinzu:
    ...
    Beachten Sie, dass `MobileServicePushFailedException` sowohl für einen Push- als auch für einen Pullvorgang verwendet werden kann. Sie kann für einen Pullvorgang auftreten, weil dieser intern einen Pushvorgang ausführt, um sicherzustellen, dass alle Tabellen und Beziehungen konsistent sind.
...
4. Drücken Sie **F5**, um die Anwendung zu erstellen und auszuführen. Geben Sie neue "todoitems" ein, und klicken Sie für jedes auf **Speichern**. Die neuen Todo-Elemente existieren nur im lokalen Speicher, bis sie per Push zum mobilen App-Back-End übertragen werden können. Die Client-App verhält sich so, als ob eine Verbindung zum mobilen App-Back-End vorhanden wäre, und unterstützt alle Erstellungs-, Lese-, Aktualisierungs- und Löschaktionen (CRUD).
...
2. Drücken Sie die Taste **F5**, um die App erneut zu erstellen und auszuführen. Beachten Sie, dass die Daten genau wie im Offlineszenario aussehen, obwohl die App jetzt mit dem mobilen App-Back-End verbunden ist. Dies liegt daran, dass die App immer mit der `IMobileServiceSyncTable` arbeitet, die auf den lokalen Speicher verweist.
...
*  Um Änderungen an den Server zu übertragen, wurde `IMobileServiceSyncContext.PushAsync()` aufgerufen. Diese Methode ist ein Element von `IMobileServicesSyncContext` anstatt der Synchronisierungstabelle, da sie Änderungen auf alle Tabellen überträgt.
...
<!-- URLs. -->
[TodoList Offline Sample]: http://go.microsoft.com/fwlink/?LinkId=394777 (in englischer Sprache)
...
[SQLite für Windows 8.1]: http://go.microsoft.com/fwlink/?LinkId=394776 (in englischer Sprache)
[SQLite für Windows Phone 8.1]: http://go.microsoft.com/fwlink/?LinkId=397953 (in englischer Sprache)
