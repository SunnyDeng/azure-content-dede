...
Microsoft ist zusammen mit einigen anderen wichtigen Anbietern ein aktiver Förderer von DASH und CENC. Microsoft Azure Media Services bietet Unterstützung für DASH und CENC. Aktuelle Ankündigungen finden Sie in den Blogs von Mingfei: [Announcing Google Widevine license delivery services public preview in Azure Media Services](http://azure.microsoft.com/blog/announcing-google-widevine-license-delivery-services-public-preview-in-azure-media-services/) und [Azure Media Services adds Google Widevine packaging for delivering multi-DRM stream](http://azure.microsoft.com/blog/azure-media-services-adds-google-widevine-packaging-for-delivering-multi-drm-stream/).
...
**DRM-System**|**Systemeigenes Betriebssystem/Systemeigene App**|**Unterstützte Desktopbrowser**|**Unterstützte mobile Browser**|**Unterstützte Streamingformate**
----|------|----|-----|----
**PlayReady**|Windows, Windows Phone, Xbox|IE11 unter Windows 8.1 und höher, Microsoft Edge|Windows Phone 8.1 und höher|DASH, Smooth Streaming
**Widevine**|Android|Chrome 34 und höher auf allen PCs und Macs|Android 4.3 und höher|DASH
...
n:1|Für jedes Asset sind mehrere Inhaltsschlüssel erforderlich. <br/><br/>Wenn Sie z. B. dynamischen CENC-Schutz mit mehreren DRM-Systemen für MPEG-DASH und dynamische AES-128-Verschlüsselung für HLS aktivieren müssen, benötigen Sie zwei separate Inhaltsschlüssel mit jeweils eigenem „ContentKeyType“-Element. (Für den Inhaltsschlüssel, der für dynamischen CENC-Schutz genutzt wird, muss „ContentKeyType.CommonEncryption“ verwendet werden, während für den Inhaltsschlüssel, der für die dynamische AES-128-Verschlüsselung genutzt wird, „ContentKeyType.EnvelopeEncryption“ erforderlich ist.)<br/><br/>Bei einem weiteren Beispiel des CENC-Schutzes von DASH-Inhalten kann theoretisch ein Inhaltsschlüssel zum Schützen des Videostreams und ein anderer Inhaltsschlüssel zum Schützen des Audiostreams eingesetzt werden. 
...
**DRM-Lizenzbereitstellung**|1\. Azure Media Services-Lizenzbereitstellung (PlayReady, Widevine, FairPlay),<br/>2. Axinom-Lizenzserver oder<br/>3. Benutzerdefinierter PlayReady-Lizenzserver
...
1. Erstellen eines Players mithilfe der [Azure Media Player-API](http://amp.azure.net/libs/amp/latest/docs/). Die [ProtectionInfo-API von Azure Media Player](http://amp.azure.net/libs/amp/latest/docs/) erlaubt Ihnen die Angabe, welche DRM-Technologie auf unterschiedlichen DRM-Plattform verwendet werden soll.
1. Testübersicht:
...
George Trifonov aus dem Azure Media Services-Team hat einen Blog mit detaillierten Schritten zur Einrichtung von Azure Active Directory für eine ASP.NET MVC-Player-App geschrieben: [Integrate Azure Media Services OWIN MVC based app with Azure Active Directory and restrict content key delivery based on JWT claims](http://gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/).
...
1. Die URL zu **Issuer** muss mit **„/“** enden.  
	...
	**Audience** muss die Client-ID der Player-Anwendung enthalten. Zudem sollten Sie auch **„/“** an das Ende der URL von „Issuer“ hinzufügen.
...
##Weitere Aspekte in unserem Entwurf und unserer Implementierung

Als Nächstes erörtern wir weitere Aspekte in unserem Entwurf und unserer Implementierung
...
1. DRM-Lizenzerwerb durch den Client, der unter HTTPS erfolgen muss
...
**Microsoft-Konto (MSA)**: In diesem Fall sehen Sie die Anmeldeseite von Microsoft-Konto für Endkunden.
...
###Verwenden von Encrypted Media Extensions (EME) für PlayReady

In einem modernen Browser mit EME/PlayReady-Unterstützung, z. B. Internet Explorer 11 unter Windows 8.1 und höher und Microsoft Edge unter Windows 10, ist PlayReady das zugrunde liegende DRM-System für EME.
...
![Nicht berechtigte Benutzer](./media/media-services-cenc-with-multidrm-access-control/media-services-unentitledusers.png.png)

###Ausführen eines benutzerdefinierten Secure Token Service (STS)

Bei einem benutzerdefinierten STS-Szenario wird das JWT-Token vom benutzerdefinierten STS ausgestellt, entweder mithilfe eines symmetrischen oder asymmetrischen Schlüssels.
