...
1.  [Bereitstellen der Datenbank](#provision): Das The Azure Store-Add-On [MongoLab](http://mongolab.com) stellt Ihnen eine MongoDB-Datenbank bereit, die in der Azure-Cloud gehostet und durch die Clouddatenbankplattform von MongoLab verwaltet wird.
...
9.  Führen Sie `npm install mongoose` aus, um M ongoose zu erhalten, einen MongoDB-Node-Treiber.
...
In diesem Abschnitt richten Sie Ihre Entwicklungsumgebung ein und erstellen den Code für eine grundlegende Aufgabenlisten-Webanwendung mithilfe von Node.js, Express und MongoDB. [Express](http://expressjs.com) stellt ein Controlleranzeigeframework für den Knoten bereit, während [Mongoose](http://mongoosejs.com) ein Treiber für die Kommunikation mit MongoDB in Node ist.
...
2.  Speichern Sie die Datei **app.js**.
...
### Erstellen einer neuen Website und Durchführen eines Pushvorgangs für Ihren Code
...
2.  Wählen Sie **Berechnen \> Website \> Schnellerfassung** aus. ![CreateSite](./media/store-mongolab-web-sites-nodejs-store-data-mongodb/screen-mongolab-newwebsite.png)
...
7.  Wenn Sie Ihre Website mit den oben genannten Schritten erstellt haben, wird der Prozess durch den folgenden Befehl abgeschlossen. Wenn Sie allerdings bereits mehr als eine Azure-Website haben, können Sie die oben genannten Schritte überspringen und eine neue Website mithilfe des gleichen Befehls erstellen. Im Projektverzeichnis **tasklist**:

         azure site create eindeutigersitename --git  

    Ersetzen Sie "eindeutigersitename" durch den eindeutigen Sitenamen für Ihre Website. Wenn die Website als Teil dieses Befehls erstellt wird, werden Sie nach dem Rechenzentrum gefragt, in dem sich die Website befinden wird. Wählen Sie das Rechenzentrum aus, das sich geographisch in der Nähe Ihrer MongoLab-Datenbank befindet.

    Der `--git`-Parameter erstellt Folgendes:
    A. ein lokales Git-Repository im Ordner **tasklist**, falls keins vorhanden ist.
    A. einen [Git remote](http://git-scm.com/docs/git-remote) namens "azure", der verwendet wird, um die Anwendung auf Azure zu veröffentlichen.
    A. eine [iisnode.yml](https://github.com/WindowsAzure/iisnode/blob/master/src/samples/configuration/iisnode.yml)-Datei, die Einstellungen enthält, die von Azure verwendet werden, um Node-Anwendungen zu hosten.
...
Glückwunsch! Sie haben gerade eine "Node.js"-Anwendung mit einer von MongoLab gehosteten MongoDB-Datenbank gestartet! Das Sie jetzt eine MongoLab-Datenbank haben, können Sie sich mit allen Fragen oder Bedenken bezüglich Ihrer Datenbank sowie mit Hilfeanfragen bezüglich MongoDB oder dem Node-Treiber selbst an <support@mongolab.com> wenden. Viel Glück!
