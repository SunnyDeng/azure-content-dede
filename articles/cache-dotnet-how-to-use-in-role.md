...
-	**Zusammengestelltes Rollencaching**: Der Cache teilt sich die VM-Ressourcen (Bandbreite, CPU und Speicher) mit der Anwendung.
...
Zum Konfigurieren eines **Zusammengestellten Rollencacheclusters** müssen Sie die Rolle auswählen, in der Sie den Cachecluster hosten möchten. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf die Rolleneigenschaften, und wählen Sie **Eigenschaften** aus.

![RoleCache1][RoleCache1]

Gehen Sie zur Registerkarte **Caching**, aktivieren Sie das Kontrollkästchen **Caching aktivieren**, und geben Sie die gewünschten Cachingoptionen an. Wenn das Caching in einer **Workerrolle** oder einer **ASP.NET- Webrolle** aktiviert ist, lautet die Standardeinstellung wie folgt: **Zusammengestelltes Rollencaching**, wobei 30 % des Speichers für die Rolleninstanzen dem Caching zugewiesen sind. Es wird automatisch ein Standardcache konfiguriert. Wenn gewünscht können weitere Caches mit anderen Namen erstellt werden, die ebenfalls den zugewiesenen Speicher teilen.
...
Klicken Sie zum Konfigurieren der Größe des virtuellen Computers und der Anzahl der Instanzen mit der rechten Maustaste im **Projektmappen-Explorer** auf "Rolleneigenschaften" und wählen Sie **Eigenschaften**.
...
Diese neuen Abschnitte umfassen Verweise auf ein **dataCacheClients**-Element und ein **cacheDiagnostics**-Element. Diese Elemente werden außerdem zum **configuration**-Element hinzugefügt.
...
Ersetzen Sie nach Hinzufügen der Konfiguration **[cache cluster role name]** mit dem Namen der Rolle, die den Cachecluster hostet.
...
Das NuGet-Paket fügt auch eine **ClientDiagnosticLevel**-Einstellung zu **ConfigurationSettings** der Cacheclientrolle in "ServiceConfiguration.cscfg" hinzu. Das folgende Beispiel ist der Abschnitt **WebRole1** einer "ServiceConfiguration.cscfg"-Datei mit einem **ClientDiagnosticLevel** von 1, was der Standardeinstellung für **ClientDiagnosticLevel** entspricht.
...
In-Role Cache verwenden möchten:

    using Microsoft.ApplicationServer.Caching;

>Sollte Visual Studio die Typen in der "Using"-Anweisung selbst nach der Installation des Cache NuGet-Pakets, das die nötigen Referenzen hinzufügt, nicht erkennen, stellen Sie sicher, dass das Zielprofil für das Projekt ".NET Framework 4.0" oder höher ist, und wählen Sie eines der Profile, das kein **Clientprofil** angibt. Anweisungen zum Konfigurieren von Cacheclients finden Sie unter [Konfigurieren der Cacheclients][].
...
> ErrorCode:SubStatus: Es wird versucht, ein Objekt zu erstellen, dessen Schlüssel bereits im Cache vorhanden ist. Das Caching akzeptiert nur eindeutige Schlüsselwerte für Objekte.
...
Um das verbleibende Zeitüberschreitungsintervall eines Elements im Cache anzuzeigen, kann mit der **GetCacheItem**-Methode ein **DataCacheItem**-Objekt abgerufen werden, das Informationen zu dem Element im Cache (einschließlich des verbleibenden Zeitüberschreitungsintervalls) enthält.
...
Weitere Informationen zur Verwendung des Sitzungszustandsanbieter des Caching-Services finden Sie unter [Sitzungszustandsanbieter für In-Role Cache][].
...
Weitere Informationen zur Verwendung des Ausgabecacheanbieters für In-Role Cache finden Sie unter [Ausgabecacheanbieter für In-Role Cache][].
...
-   Erfahren Sie, wie Sie die Migration auf den In-Role Cache durchführen: [Migrieren zum In-Role Cache][]
...
[Was ist In-Role Cache?]: #what-is
[Erstellen eines Azure-Caches]: #create-cache
[Welche Art von Cache ist richtig für mich?]: #choosing-cache
[Erste Schritte mit dem In-Role Cache-Dienst:]: #getting-started-cache-service
[Vorbereiten Ihres Visual Studio-Projekts zur Verwendung des In-Role Caches]: #prepare-vs
...
[Konfigurieren der gewünschten Cachegröße]: #cache-size
