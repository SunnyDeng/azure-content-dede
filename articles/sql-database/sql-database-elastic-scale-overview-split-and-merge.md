...
Azure SQL-Datenbank bietet eine Vielzahl von Möglichkeiten zum Skalieren der Datenebene einer Anwendung bei sich ändernden geschäftlichen Anforderungen. Beispielsituationen umfassen die Handhabung von Anwendungen, die stark anwachsen oder in denen bestimmte Mandanten eine Datenbank bis an ihre Grenzen bringen. Optionen für die Skalierung umfassen Folgendes:

* Elastische Datenbankpools ermöglichen Datenbanken innerhalb eines Pools verbrauchte Ressourcen nach Bedarf einzeln zu vergrößern und zu verkleinern, während gleichzeitig ein vorhersagbarer Gesamtpreis für den Pool insgesamt bewahrt bleibt. Datenbanken können auch hinzugefügt oder aus dem Pool entfernt werden, um neue oder ausscheidende Mandanten zu unterstützen. Weitere Informationen finden Sie unter [Elastische Pools in Azure SQL-Datenbank (Vorschau)](sql-database-elastic-pool.md). 

* Explizites Erhöhen oder Verringern der Ressourcen einer Datenbank durch manuelles oder richtlinienbasiertes Ändern von Editionen oder Leistungsebenen (siehe [Shard-Elastizität](sql-database-elastic-scale-elasticity.md))

* Ändern der Verteilung von Daten zwischen Shards – oft in Verbindung mit Vergrößern oder Verkleinern der Gesamtzahl von Datenbanken für eine Shard-Gruppe. Dies wird als Split-Merge (Aufteilung/Zusammenführung) bezeichnet, und ist häufig erforderlich, wenn mehrere Endkunden (Mandanten) innerhalb der gleichen Shard verwaltet werden.

Für dieses letzte Szenario steht das **Split-Merge-Tool für elastische Datenbanken** bereit. Dies ist wichtig, wenn die einfacheren Alternativen der vertikalen Skalierung oder einfachen Hinzufügung einer neuen Datenbank für einen neuen Mandanten nicht ausreichen. Mit dem Split-Merge-Tool wird das Hoch- und Herunterskalieren verwaltet. Sie können Datenbanken zur Shard-Gruppe hinzufügen oder daraus entfernen und das Split-Merge-Tool verwenden, um die Verteilung der Shardlets zwischen ihnen auszugleichen. (Begriffsdefinitionen finden Sie im [Elastic Scale-Glossar](sql-database-elastic-scale-glossary.md).)

Bei den aktuell verfügbaren Azure SQL-Datenbankebenen kann die Kapazität auch durch Hoch- oder Herunterskalieren der Kapazität einer einzelnen Azure SQL-DB-Datenbank reguliert werden. Das Hoch-/Herunterskalieren der elastischen Kapazitätsverwaltung wird vom Split-Merge-Dienst nicht abgedeckt – siehe stattdessen [Shard-Elastizität](sql-database-elastic-scale-elasticity.md)).
...
Nachfolgend werden alle Verarbeitungsvorgänge im Dienst im Zusammenhang mit diesen Fähigkeiten als **Split/Merge/Move**-Anforderungen bezeichnet.
