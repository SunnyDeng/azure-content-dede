...
# Erstellen und Verwalten eines elastischen SQL-Datenbankpools mit PowerShell
...
In diesem Artikel erfahren Sie, wie Sie alle erforderlichen Elemente mit Ausnahme des Azure-Abonnements erhalten, die Sie zum Erstellen und Konfigurieren eines Pools für elastische Datenbanken benötigen. Wenn Sie ein Azure-Abonnement benötigen, müssen Sie lediglich oben auf dieser Seite auf den Link **Kostenlose Testversion** klicken. Lesen Sie anschließend den Artikel weiter.
...
Um einen Pool für elastische Datenbanken mit PowerShell zu erstellen, muss Azure PowerShell installiert sein und ausgeführt werden. Wechseln Sie in den Ressourcen-Manager-Modus, um auf die PowerShell-Cmdlets für den Azure-Ressourcen-Manager zuzugreifen.
...
Nachdem Sie nun Zugriff auf Cmdlets für Ihr Azure-Abonnement haben, können Sie im nächsten Schritt die Ressourcengruppe einrichten, die den Server für den Pool enthält. Sie können den nächsten Befehl für einen beliebigen gültigen Speicherort anpassen. Führen Sie **\(Get-AzureLocation \| where-object {$\_.Name -eq "Microsoft.Sql/servers" }\).Locations** aus, um eine Liste der gültigen Speicherorte abzurufen.
...
Der im vorherigen Schritt erstellte Pool ist leer und enthält keine Datenbanken. Die folgenden Abschnitte zeigen, wie neue Datenbanken innerhalb des Pools erstellt und vorhandene Datenbanken zum Pool hinzugefügt werden.


### Erstellen einer neuen elastischen Datenbank in einem Pool für elastische Datenbanken

Verwenden Sie zum direkten Erstellen einer neuen Datenbank in einem Pool das Cmdlet **New-AzureSqlDatabase**, und legen Sie den **ElasticPoolName**-Parameter fest.
...
Verwenden Sie zum Verschieben einer vorhandenen Datenbank in einen Pool das Cmdlet **Set-AzurSqlDatabase**, und legen Sie den **ElasticPoolName**-Parameter fest.
...
Verschieben Sie die vorhandene Datenbank in den Pool.
...
Sie können den Status der Poolvorgänge, einschließlich Erstellung und Updates, verfolgen.
...
Metriken, die als Prozentsatz des Poolgrenzwerts abgerufen werden können:
...
* Durchschnittliche DTU-Auslastung \(als Maximalwert der CPU-, E/A- und Protokoll-Auslastung\) – DTU\_percent 
...
* Gesamtspeichergröße für den Pool – storage\_in\_megabytes 
...
Nach dem Erstellen eines Pools für elastische Datenbanken können Sie elastische Datenbanken im Pool mit elastischen Aufträgen verwalten. Elastische Aufträge erleichtern die Ausführung von T-SQL-Skripts für eine beliebige Anzahl von elastischen Datenbanken im Pool.
