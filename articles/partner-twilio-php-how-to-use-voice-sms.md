...
Sobald Sie Ihre PHP-Seite mit den TwiML-Antworten eingerichtet haben, können Sie die URL der PHP-Seite an die Methode `Services_Twilio->account->calls->create` übergeben. Beispiel: Ihre Webanwendung **MyTwiML** wird in einem unter Azure gehosteten Dienst bereitgestellt, und der Name der PHP-Seite lautet **mytwiml.php**. In diesem Fall kann die URL, wie im folgenden Beispiel gezeigt, an  **Services_Twilio->account->calls->create** übergeben werden:
