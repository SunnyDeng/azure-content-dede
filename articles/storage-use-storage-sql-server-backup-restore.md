<properties urlDisplayName="Storage for SQL Server backups" pageTitle="Gewusst wie: Verwenden von Azure Storage f&uuml;r die SQL Server-Sicherung und -Wiederherstellung | Azure" metaKeywords="" description="" metaCanonical="" services="storage" documentationCenter="" title="Verwenden von Azure-Speicher f&uuml;r SQL Server-Sicherung und -Wiederherstellung" authors="karaman" solutions="" manager="jeffreyg" editor="tysonn" />
...
-   Kostenvorteile: Sie zahlen nur für den tatsächlich genutzten Dienst. Die Option kann genauso kosteneffizient wie eine Offsitesicherungs-/-archivierungslösung sein. Weitere Informationen finden Sie im [Azure-Kostenrechner][Azure-Kostenrechner] und in der [Azure-Preisübersicht][Azure-Preisübersicht].

Weitere Details finden Sie unter [SQL Server-Sicherung und -Wiederherstellung mit dem Windows Azure-BLOB-Speicherdienst][SQL Server-Sicherung und -Wiederherstellung mit dem Windows Azure-BLOB-Speicherdienst].
...
Eine ausführliche Anleitung zum Erstellen eines Speicherkontos und Durchführen einer einfachen Wiederherstellung finden Sie unter [Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Windows Azure-BLOB-Speicherdienst][Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Windows Azure-BLOB-Speicherdienst]
...
-   Speicherkonto: Das Speicherkonto ist der Ausgangspunkt für alle Speicherdienste. Um auf einen Azure-Blob-Speicherdienst zuzugreifen, erstellen Sie als Erstes ein Azure-Speicherkonto. Der Name und Zugriffsschlüssel des Speicherkontos werden für die Authentifizierung beim Azure-Blob-Speicherdienst und dessen Komponenten benötigt.
    Weitere Informationen zum Azure-Blob-Speicherdienst finden Sie unter [Verwenden des Azure-Blob-Speicherdiensts][Verwenden des Azure-Blob-Speicherdiensts].
...
-   BLOB: Eine Datei von beliebiger Art und Größe. Es gibt zwei Arten von Blobs, die im Azure-Blob-Speicherdienst gespeichert werden können: Block- und Seiten-BLOBs. Für die SQL Server-Sicherung werden Seiten-Blobs verwendet. Blobs sind über das folgende URL-Format adressierbar: `https://<storage account>.blob.core.windows.net/<container>/<blob>`
    Weitere Informationen zu Seiten-Blobs finden Sie unter [Grundlegendes zu Block-BLOBs und Seiten-BLOBs][Grundlegendes zu Block-BLOBs und Seiten-BLOBs].

## SQL Server-Komponenten

-   URL: Eine URL gibt einen Uniform Resource Identifier (URI) zu einer eindeutigen Sicherungsdatei an. Die URL dient zur Angabe des Speicherorts und Namens der SQL Server-Sicherungsdatei. In dieser Implementierung ist lediglich eine URL gültig, die auf einen Seiten-Blob in einem Azure-Speicherkonto verweist. Die URL darf nicht auf ein Blob, sondern lediglich auf einen Container verweisen. Wenn das Blob nicht vorhanden ist, wird es erstellt. Bei Angabe eines vorhandenen Blobs schlägt BACKUP fehl, es sei denn, die Option \> WITH FORMAT wird angegeben.
    Nachfolgend sehen Sie ein Beispiel für die URL zur Angabe im BACKUP-Befehl:
...
-   Anmeldeinformationen: Die Informationen, die zum Herstellen einer Verbindung zum Azure-Blob-Speicherdienst und zum Authentifizieren bei diesem erforderlich sind, werden als Anmeldeinformationen gespeichert. Damit SQL Server Sicherungen in einen Azure-Blob schreiben oder daraus wiederherstellen kann, müssen SQL Server-Anmeldeinformationen erstellt werden. In den Anmeldeinformationen werden der Name des Speicherkontos sowie der zugehörige Zugriffsschlüssel gespeichert. Nach Erstellung der Anmeldeinformationen müssen diese bei Ausgabe der BACKUP/RESTORE-Anweisungen in der Option WITH CREDENTIAL angegeben werden. Weitere Informationen zum Anzeigen, Kopieren oder erneuten Generieren von Zugriffsschlüsseln für Speicherkonten finden Sie unter [Speicherzugriffsschlüssel für ein Azure-Speicherkonto][Speicherzugriffsschlüssel für ein Azure-Speicherkonto].
    Schritt-für-Schritt-Anweisungen zum Erstellen von SQL Server-Anmeldeinformationen finden Sie unter [Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Azure-BLOB-Speicherdienst][Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Windows Azure-BLOB-Speicherdienst].

## SQL Server-Sicherung und -Wiederherstellung mit Azure-Blobs – Konzepte und Aufgaben:

**Konzepte, Überlegungen und Codebeispiele:**

[SQL Server-Sicherung und -Wiederherstellung mit dem Windows Azure-BLOB-Speicherdienst][SQL Server-Sicherung und -Wiederherstellung mit dem Windows Azure-BLOB-Speicherdienst]

**Erste-Schritte-Lernprogramm:**

[Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Windows Azure-BLOB-Speicherdienst][1]

**Bewährte Methoden, Problembehandlung:**

[Bewährte Methoden für die Sicherung und Wiederherstellung (Windows Azure-BLOB-Speicherdienst)][Bewährte Methoden für die Sicherung und Wiederherstellung (Windows Azure-BLOB-Speicherdienst)]

  [Azure-Kostenrechner]: http://go.microsoft.com/fwlink/?LinkId=277060 "Kostenrechner"
  [Azure-Preisübersicht]: http://go.microsoft.com/fwlink/?LinkId=277059 "Preisübersicht"
  ...
  [Erste Schritte mit der SQL Server-Sicherung und -Wiederherstellung im Windows Azure-BLOB-Speicherdienst]: http://go.microsoft.com/fwlink/?LinkId=271615
  [Verwenden des Azure-Blob-Speicherdiensts]: http://www.windowsazure.com/de-de/develop/net/how-to-guides/blob-storage/
  [Grundlegendes zu Block-BLOBs und Seiten-BLOBs]: http://msdn.microsoft.com/de-de/library/windowsazure/ee691964.aspx
  [Speicherzugriffsschlüssel für ein Azure-Speicherkonto]: http://msdn.microsoft.com/de-de/library/windowsazure/hh531566.aspx
  [1]: http://go.microsoft.com/fwlink/?LinkID=271615 "Lernprogramm"
