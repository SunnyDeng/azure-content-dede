...
4.  Führen Sie in der Shell den Befehl '`zypper lr`' aus. Wenn dieser Befehl Folgendes zurückgibt
    ...
    Wenn der Befehl "No repositories defined. Use the 'zypper addrepo' command to add one or more repositories" zurückgibt, müssen die Repositorys erneut aktiviert werden:
...
7.  Ändern Sie die Kernel-Boot-Zeile in grub, um die folgenden Parameter einzubinden. Dadurch wird zudem sichergestellt, dass alle Konsolennachrichten zum ersten seriellen Port gesendet werden. Dieser kann Azure bei der Behebung von Fehlern unterstützen:
...
1.  Es wird empfohlen, dass Sie /etc/sysconfig/network/dhcp oder ein Äquivalent von DHCLIENT\_SET\_HOSTNAME="yes" auf DHCLIENT\_SET\_HOSTNAME="no" einstellen.

2.  Kommentieren Sie in /etc/sudoers die folgende Zeile aus, sofern diese vorhanden ist:

        Defaults targetpw

3.  Stellen Sie sicher, dass der SSH-Server installiert und konfiguriert ist, damit er beim Booten hochfährt.

4.  Richten Sie keinen SWAP-Raum auf dem BS-Datenträger ein.
...
6.  Stellen Sie sicher, dass der Azure Linux Agent beim Start ausgeführt wird:

        systemctl enable waagent.service

7.  Klicken Sie im Hyper-V-Manager auf **Shutdown**.
...
1.  Sie müssen sicherstellen, dass Sie einen Kernel ausführen, der entweder die neuesten LIS-Treiber für Hyper V enthält oder mit dem Sie diese erfolgreich kompiliert haben (sie wurden als Open Source zur Verfügung gestellt). Die Treiber sind [unter diesem Speicherort](http://go.microsoft.com/fwlink/p/?LinkID=254263&clcid=0x409) zu finden.
