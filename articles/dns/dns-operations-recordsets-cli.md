...
Datensatzgruppen werden mit dem `azure network record-set create`-Befehl erstellt. Sie müssen den Namen der Datensatzgruppe, die Zone, die Gültigkeitsdauer (TTL) und den Eintragstyp angeben.
...
Sie können alle Datensätze in einer DNS-Zone mit dem `azure network dns-record-set list`-Befehl auflisten:
...
Fügen Sie nach dem Erstellen der A-Datensatzgruppe die IPv4 Adresse zur Datensatzgruppe mit `azure network dns-record-set add-record` hinzu:
