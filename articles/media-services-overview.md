...
## Gängige Entwicklungs-Workflows: VoD (Video on Demand) und Livestreaming

### VoD: Bereitstellen von Streaminginhalten 
...
1. (Optional) Konfigurieren Sie Inhaltsschutz und eine Autorisierungsrichtlinie für Inhaltsschlüssel (wird zur dynamischen Verschlüsselung verwendet).
1. Veröffentlichen Sie das Medienobjekt.
1. Streamen Sie die veröffentlichten Inhalte. 

### VoD: Indizieren von Inhalten

1. Laden Sie eine Zwischendatei (Mezzanine File) hoher Qualität in ein Medienobjekt hoch.
1. Indizieren Sie die Inhalte.
1. Nutzen Sie die indizierten Inhalte.

### VoD: Bereitstellen eines progressiven Downloads 
...
1. Starten und konfigurieren Sie den gewünschten Livetranscoder.
1. Rufen Sie den Vorschauendpunkt des Kanals ab, und überprüfen Sie, ob der Kanal den Livestream ordnungsgemäß empfängt.
1. Erstellen Sie ein Programm und ein Medienobjekt. 
2. Konfigurieren Sie eine Übermittlungsrichtlinie für das Medienobjekt (wird zur dynamischen Paketierung verwendet).
3. (Optional) Konfigurieren Sie Inhaltsschutz und eine Autorisierungsrichtlinie für Inhaltsschlüssel (wird zur dynamischen Verschlüsselung verwendet).
1. Um den Stream für Benutzer verfügbar zu machen, erstellen Sie einen Streaminglocator, der für den Zugriff auf das dem Programm zugeordnete Medienobjekt erforderlich ist.
