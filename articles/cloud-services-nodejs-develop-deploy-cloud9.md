...
4.  Wählen Sie **File** und dann **New File** aus, um eine neue Node-Anwendung zu erstellen.
...
6.  Wählen Sie **File** und dann **Save as** aus, um den Code zu speichern. Geben Sie im Dialogfeld **Save As** als Dateinamen **server.js** ein, und klicken Sie dann auf **Save**.
...
Um die Anwendung in Microsoft Azure bereitzustellen, benötigen Sie ein Konto. Wenn Sie kein Azure-Konto haben, können Sie sich wie folgt für eine kostenlose Testversion anmelden:
...
1.  Um eine neue Bereitstellung zu erstellen, wählen Sie **Deploy** aus, und klicken Sie dann auf **+**, um einen Bereitstellungsserver zu erstellen.

    ![create a new deployment][create a new deployment]

2.  Geben Sie im Dialogfeld **Add a deploy target** einen Bereitstellungsnamen ein, und wählen Sie dann **Azure** in der Liste **Choose type** aus. Der Bereitstellungsname, den Sie angeben, wird zur Identifizierung der Bereitstellung in Cloud9 verwendet und entspricht nicht dem Bereitstellungsnamen in Azure.
...
3.  Wenn Sie erstmalig eine Cloud9-Umgebung erstellt haben, die Azure verwendet, müssen Sie Ihre Azure-Veröffentlichungseinstellungen konfigurieren. Führen Sie die folgenden Schritte aus, um diese Einstellungen herunterzuladen und in Cloud9 zu installieren:
    ...
    3.  Wählen Sie im Dialogfeld **Add a deploy target** die Option **Choose File** und dann die Datei aus, die Sie zuvor heruntergeladen haben.

    4.  Klicken Sie nach Auswahl der Datei auf **Upload**.

4.  Klicken Sie auf **+ Create new**, um einen neuen gehosteten Dienst zu erstellen. Ein *hosted service* ist der Container, in dem die Anwendung gehostet wird, wenn diese in Azure bereitgestellt wird. Weitere Informationen erhalten Sie unter [Übersicht zum Erstellen eines gehosteten Diensts für Azure][].
...
2.  Wählen Sie **Deploy to Production environment** aus.

3.  Klicken Sie auf **Deploy**, um mit der Bereitstellung zu beginnen.

4.  Wenn Sie dieses Projekt erstmalig zu Azure bereitstellen, erhalten Sie eine Fehlermeldung **'No web.config found'**. Wählen Sie **Yes** aus, um die Datei zu erstellen. Dadurch wird eine  'Web.cloud.config'-Datei zu Ihrem Projekt hinzugefügt.
	
	![no web.config file found message](./media/cloud-services-nodejs-develop-deploy-cloud9/cloud9_no_web_config.png)

5.  Wenn Sie dieses Projekt erstmalig in Azure bereitstellen, erhalten Sie eine Fehlermeldung **'No 'csdef' file present'**. Wählen Sie **Yes** aus, um die .csdef-Datei zu erstellen. Dadurch wird eine 'ServiceDefinition.csdef'-Datei zu Ihrem Projekt hinzugefügt.    "ServiceDefinition.csdef" ist eine Azure-spezifische Datei, die für die Veröffentlichung Ihrer Anwendung erforderlich ist. Weitere Informationen erhalten Sie unter [Übersicht zum Erstellen eines gehosteten Diensts für Azure][].

6.  Sie werden aufgefordert, die Instanzengröße für diese Anwendung auszuwählen. Wählen Sie **Small** aus, und klicken Sie dann auf **Create**. Weitere Details zu Azure-VM-Größen finden Sie unter [Konfigurieren der Größe von virtuellen Computern][].

	![specify csdef file values](./media/cloud-services-nodejs-develop-deploy-cloud9/cloud9_createcsdef.png)

7.  Der Bereitstellungseintrag zeigt den Status des Bereitstellungsprozess an. Nach der Fertigstellung wird die Bereitstellung als **Active** angezeigt.
...
2.  Wählen Sie **File** und dann **Save** aus, um den Code zu speichern.

## Bereitstellen der Aktualisierung zur Azure-Stagingumgebung

1.  Wählen Sie **Deploy to Staging** aus.
...
3.  Wählen Sie links auf der Webseite **Gehostete Dienste, Speicherkonten und CDN** aus, und klicken Sie dann auf **Gehostete Dienste**.

	![Azure Management Portal][Azure Management Portal]

	Das Ergebnisfenster zeigt den gehosteten Dienst mit dem Namen an, den Sie in Cloud9 angegeben haben, sowie zwei Bereitstellungen: eine mit dem    **Umgebung**-Wert **Staging**, die zweite mit **Produktion**.

4.  Für den VIP-Austausch wählen Sie den gehosteten Dienst aus, und klicken Sie dann im Menüband auf **VIP austauschen**.
...
4.  Wählen Sie die Stagingbereitstellung aus (angezeigt durch den **Umgebung**-Wert). Klicken Sie im Menüband auf **Löschen**, um die Anwendung zu löschen.

	![delete the deployment](./media/cloud-services-nodejs-develop-deploy-cloud9/cloud9_deletedeployment.png)

5.  Wählen Sie die Produktionsbereitstellung aus, und klicken Sie auf **Löschen**, um diese Anwendung ebenfalls zu löschen.
...
-   [Cloud9-Dokumentation][]
  ...
  [Konfigurieren der Größe von virtuellen Computern]: http://msdn.microsoft.com/library/windowsazure/ee814754.aspx
  [Übersicht über das Verwalten von Bereitstellungen in Azure]: http://msdn.microsoft.com/library/windowsazure/gg433027.aspx
