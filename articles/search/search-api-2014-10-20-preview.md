<properties 
	pageTitle="Azure Search-Dienst-REST-API: 2014-10-20-Preview" 
	description="Azure Search-Dienst-REST-API: 2014-10-20-Preview" 
	services="search" 
	documentationCenter="" 
	authors="HeidiSteen" 
	manager="mblythe"/>
...
# Azure Search-Dienst-REST-API: Version 2014-10-20-Preview #

Das vorliegende Dokument beschreibt die Version **2014-10-20-Preview** der Azure Search-Dienst-REST-API. Diese Version ist mittlerweile veraltet. Falls Sie diese API im Produktionscode verwenden, migrieren Sie baldmöglichst zur allgemein verfügbaren Version. Eine entsprechende Anleitung finden Sie unter [Migrieren von Vorschau-APIs in Azure Search](search-transition-from-preview.md).

Weitere API-Inhalte für diese Version umfassen Folgendes:

- [Bewertungsprofile (Azure Search-Dienst-REST-API: 2014-10-20-Preview)](search-api-scoring-profiles-2014-10-20-preview.md)

Eine Dokumentation für die Veröffentlichungsversion der Azure Search-Dienst-REST-API finden Sie bei MSDN. Weitere Informationen finden Sie unter [Azure Search-Dienst-REST-API](http://msdn.microsoft.com/library/azure/dn798935.aspx).
...
Der Endpunkt für Dienstvorgänge ist die URL des von Ihnen bereitgestellten Azure Search-Diensts: https://*your-service-name*.search.windows.net.


### Versionen ###

Es gibt verschiedene API-Versionen für Azure Search. Wenn Sie den Einsatz von Azure Search in einer Produktionsanwendung prüfen möchten, empfehlen wir `api-version=2014-07-31-Preview`. Dies ist momentan die einzige gesperrte Version. Ausführliche Informationen finden Sie unter [Versionsverwaltung für den Suchdienst](http://msdn.microsoft.com/library/azure/dn864560.aspx).


<a name="Authentication"></a>
### Authentifizierung und Zugriffssteuerung###

Für die Authentifizierung bei einem Azure Search-Dienst sind zwei Angaben erforderlich: die URL des Suchdiensts und ein `api-key`. Die `api-keys` werden bei der Erstellung des Diensts generiert und können nach der Bereitstellung des Diensts bei Bedarf neu generiert werden. Ein `api-key` ist entweder ein Administratorschlüssel, der Zugriff auf alle Vorgänge erteilt, oder ein Abfrageschlüssel, der ausschließlich Abfrageanforderungen authentifiziert. Pro Dienst werden 2 Administratorschlüssel und bis zu 50 Abfrageschlüssel verwendet.
...
Sie können einen neuen Index innerhalb des Azure Search-Diensts mithilfe einer HTTP POST-Anforderung erstellen. Der Text der Anforderung ist ein JSON-Dokument, das Indexname, Felder, Attribute zum Steuern des Abfrageverhaltens, eine Bewertung der Ergebnisse, Vorschläge und CORS-Optionen angibt.
...
`api-version` ist erforderlich. Gültige Werte sind `2014-07-31-Preview` oder `2014-10-20-Preview`. Sie können angeben, welche Version für die einzelnen Anforderungen verwendet wird, um ein versionsspezifisches Verhalten zu nutzen. Im Allgemeinen empfiehlt es sich aber, im gesamten Code die gleiche Version zu verwenden. Ausführliche Informationen zu den API-Versionen finden Sie unter [Versionsverwaltung für den Suchdienst](http://msdn.microsoft.com/library/azure/dn864560.aspx). Unter [Sprachunterstützung](#LanguageSupport) finden Sie ausführliche Informationen zu den Sprachanalysemodulen.
...
Beim Erstellen eines Indexes können die folgenden Attribute festgelegt werden. Ausführliche Informationen zur Bewertung und zu Bewertungsprofilen finden Sie unter [Bewertungsprofile (Azure Search-Dienst-REST-API: 2014-10-20-Preview)](search-api-scoring-profiles-2014-10-20-preview.md).
...
`suggestions`: Legt fest, ob das Feld für die automatische Vervollständigung bei der Eingabe verwendet werden kann. Diese kann nur für Felder vom Typ `Edm.String` oder `Collection(Edm.String)` festgelegt werden. `suggestions` ist standardmäßig `false`, da hierfür zusätzlicher Platz im Index erforderlich ist. **Hinweis**: Verwenden Sie anstelle dieser Option ggf. die in 2014-10-20-Preview eingeführte Eigenschaft `suggesters`. In einer zukünftigen Version wird die Eigenschaft `suggestions` zugunsten einer separaten Spezifikation vom Typ `suggesters` ausgemustert.
...
Vorschläge ermöglichen eine automatische Vervollständigung bei Suchvorgängen. In der Regel werden bei der Eingabe unvollständige Suchzeichenfolgen an die Vorschlags-API gesendet. Diese gibt daraufhin eine Reihe von Vorschlägen zurück.

Azure Search wechselt zu einer neuen Vorschlags-API. In 2014-07-31-Preview wurde für Vorschläge eine enger gefasste API verwendet. Dadurch konnte ein Feld mit `"suggestions": true` markiert werden, was Präfixvorschläge für kurze Zeichenfolgen (zwischen drei und 25 Zeichen) ermöglichte. Ab 2014-10-20-Preview verfügt Azure Search über eine leistungsfähigere Vorschlagskomponente, wie in diesem Abschnitt beschrieben. Diese neue Implementierung ermöglicht sowohl einen Präfix- als auch einen Infixabgleich und bietet eine bessere Fehlertoleranz für Suchzeichenfolgen. Ab 2014-10-20-Preview wird dringend die Verwendung der neuen Vorschlags-API empfohlen.

Die aktuelle Vorschlagsunterstützung funktioniert am besten, wenn sie für spezifische Dokumentvorschläge (und nicht für zusammenhanglose Begriffe oder Ausdrücke) verwendet wird. Geeignete Quellfelder für diese Art von Vorschlägen sind Titel, Namen und andere relativ kurze Ausdrücke, die ein Element identifizieren. Weniger effektiv sind dagegen Felder mit stark repetitivem Charakter (etwa Kategorien oder Tags) sowie besonders lange Felder (etwa für Beschreibungen oder Kommentare).

Sie können der Sammlung `suggesters` im Rahmen der Indexdefinition einen einzelnen Vorschlag hinzufügen. Ein Vorschlag wird durch folgende Eigenschaften definiert:
...
- `sourceFields`: Eine Liste mit einem oder mehreren Feldern, die als Quelle für den Inhalt von Vorschlägen dienen. Als Vorschlagsquellen sind nur Felder vom Typ `Edm.String` und `Collection(Edm.String)` zulässig. In der aktuellen Vorschlagsvorschauversion können nur Felder verwendet werden, für die keine benutzerdefinierte Analyse festgelegt ist.

In der aktuellen Version der API kann die Vorschlagssammlung derzeit nur einen einzelnen Vorschlag enthalten.
...
Sie können einen vorhandenen Index in Azure Search mit einer HTTP PUT-Anforderung aktualisieren. In der öffentlichen Vorschauversion können Aktualisierungen im Hinzufügen neuer Felder zum vorhandenen Schema, im Ändern der CORS-Optionen und im Verändern von Bewertungsprofilen bestehen (siehe [Bewertungsprofile (Azure Search-Dienst-REST-API: 2014-10-20-Preview)](search-api-scoring-profiles-2014-10-20-preview.md)). Geben Sie den Namen des Index an, der mit dem Anforderungs-URI aktualisiert werden soll:

    PUT https://[search service url]/indexes/[index name]?api-version=[api-version]
    Content-Type: application/json
    api-key: [admin key]

**Wichtig:** Die Unterstützung für Aktualisierungen des Indexschemas ist auf Vorgänge beschränkt, die keine Neuerstellung des Suchindex erfordern. Nicht unterstützt werden Schema-Aktualisierungen, die eine erneute Indizierung nach sich ziehen (z. B. Änderungen der Feldtypen). Während sich bestehende Felder nicht ändern oder löschen lassen, können Sie jederzeit neue Felder hinzufügen. Gleiches gilt für Vorschläge: Einem Vorschlag können beim Hinzufügen von Feldern neue Felder hinzugefügt werden. Es ist jedoch weder möglich, Felder aus Vorschlägen zu entfernen, noch können vorhandene Felder zu Vorschlägen hinzugefügt werden.
...
Sie benötigen außerdem den Dienstnamen, um die URL der Anforderung zu erstellen. Sie finden den Dienstnamen und den `api-key` im Dashboard zu Ihrem Dienst im Azure-Vorschauportal. Hilfe bei der Seitennavigation finden Sie unter [Erste Schritte mit Azure Search](search-get-started.md).
...
`highlight=[string]` (optional): Ein Satz kommagetrennter Feldnamen für wichtige Treffer. Für wichtige Treffer können nur Felder mit dem Attribut `searchable` verwendet werden.
  ...
  `highlightPreTag=[string]` (optional): Ein Zeichenfolgetag, das wichtigen Treffern vorangestellt wird. Es muss mit `highlightPostTag` festgelegt werden. In URLs reservierte Zeichen müssen mit dem Prozentzeichen codiert werden (z. B. %23 anstatt nur der Zahl).

  `highlightPostTag=[string]` (optional): Ein Zeichenfolgetag, das an wichtige Treffer angefügt wird. Es muss mit `highlightPreTag` festgelegt werden. In URLs reservierte Zeichen müssen mit dem Prozentzeichen codiert werden (z. B. %23 anstatt nur der Zahl).
...
Vorschlagsanforderungen dienen dazu, Zieldokumente vorzuschlagen. Der vorgeschlagene Text kann daher wiederholt werden, wenn mehrere infrage kommende Dokumente der gleichen Sucheingabe entsprechen. Sie können mit `$select` weitere Dokumentfelder (einschließlich des Dokumentschlüssels) abrufen, um festzustellen, aus welchem Dokument die einzelnen Vorschläge stammen.
...
`highlightPreTag=[string]` (optional; Standardwert: `) - a string tag that prepends to search hits. Must be set with `highlightPostTag. In URLs reservierte Zeichen müssen mit dem Prozentzeichen codiert werden (z. B. %23 anstatt nur der Zahl).

`highlightPostTag=[string]` (optional; Standardwert: `) - a string tag that appends to search hits. Must be set with `highlightPreTag. In URLs reservierte Zeichen müssen mit dem Prozentzeichen codiert werden (z. B. %23 anstatt nur der Zahl).

`suggesterName=[string]` (optional): Der Name des Vorschlags, der in der Sammlung `suggesters` angegeben wurde, die Teil der Indexdefinition ist. Wird diese Option nicht verwendet, basieren die Vorschläge auf der Implementierung der Vorgängerversion, die mit `"suggestions": true` markierte Felder heranzieht und nur den Präfixabgleich unterstützt.
