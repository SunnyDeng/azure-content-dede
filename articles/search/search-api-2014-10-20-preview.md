<properties pageTitle="Azure Search-Dienst-REST-API: Version 2014-10-20-Preview" description="Azure Search-Dienst-REST-API: Version 2014-10-20-Preview" services="search" solutions="" documentationCenter="" authors="HeidiSteen" manager="mblythe" editor="" />
...
Sie können der `suggesters`-Sammlung im Rahmen der Indexdefinition einen einzelnen Vorschlag hinzufügen. Ein Vorschlag wird durch folgende Eigenschaften definiert:
...
`moreLikeThis=[key]` (optional): Der Parameter sucht nach Dokumenten, die mit dem vom Dokumentschlüssel angegebenen Dokument übereinstimmen. Inhalte in Feldern mit dem Attribut `searchable` werden standardmäßig durchsucht, es sei denn, es wurde `searchFields` angegeben. Diese Option kann nicht in Abfragen verwendet werden, die den Textsuchparameter „search=[string]“ enthalten.
