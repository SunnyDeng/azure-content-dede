...
1.  Geben Sie den Anforderungsheader an, und ersetzen Sie host und api-key (Kleinbuchstaben) durch die entsprechenden Werte für Ihren Dienst.
...
Falls Sie eine "HTTP 504"-Antwort erhalten, prüfen Sie, ob die URL mit HTTPS beginnt. Falls Sie eine "HTTP 404"-Antwort erhalten, prüfen Sie Ihre Syntax. "HTTP 400" deutet auf ein Problem mit dem api-key hin (entweder ein ungültiger Schlüssel oder ein Syntaxproblem bei der Angabe des API-Schlüssels).
