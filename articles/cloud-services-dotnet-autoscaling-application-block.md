<properties urlDisplayName="Autoscaling" pageTitle="Verwenden des Anwendungsblocks (.NET) für die automatische Skalierung - Azure" metaKeywords="Azure autoscaling, Azure autoscaling C#, Azure autoscaling .NET" description="Hier erfahren Sie, wie Sie die Anwendung für die automatische Skalierung verwenden. Die Codebeispiele sind in C# geschrieben und verwenden die .NET API." metaCanonical="" services="cloud-services" documentationCenter=".NET" title="How to Use the Autoscaling Application Block" authors="rasquill" solutions="" manager="timlt" editor="" />
...
[Was ist der Anwendungsblock für die automatische Skalierung?][Was ist der Anwendungsblock für die automatische Skalierung?]   
 ...
 [Vorgehensweise: Instanziieren und Ausführen der automatischen Skalierung][Vorgehensweise: Instanziieren und Ausführen der automatischen Skalierung] [Gewusst wie: Definieren des Dienstmodells][Gewusst wie: Definieren des Dienstmodells]   
...
1.  Öffnen Sie das Fenster der **NuGet-Paket-Manager-Konsole**. Wählen Sie im Menü **Extras** den Eintrag **Bibliothekspaket-Manager** und danach **Paket-Manager-Konsole** aus.
...
Ihr Projekt muss auf .NET Framework 4 verweisen. So ändern oder verifizieren Sie das Zielframework:
...
## <a id="Instantiate"> </a>Gewusst wie: Instanziieren und Ausführen der automatischen Skalierung

Verwenden Sie die Methode **IServiceLocator.GetInstance**, um die
automatische Skalierung zu skalieren, und rufen Sie dann die Methode **Autoscaler.Start** auf, um die
**automatische Skalierung** auszuführen.

    Autoscaler scaler =
        EnterpriseLibraryContainer.Current.GetInstance<Autoscaler>();
    scaler.Start();

## <a id="DefineServiceModel"> </a>Gewusst wie: Definieren des Dienstmodells
...
Ersetzen Sie die Werte in eckigen Klammern durch spezifische Werte für
Ihre Umgebung und Zielanwendung. Um zahlreiche dieser Werte zu finden,
müssen Sie sich beim [Azure-Verwaltungsportal][Azure-Verwaltungsportal] anmelden.
...
-   **[subscriptionid]:** Die eindeutige ID des Azure-Abonnements, das die Anwendung enthält, in der Sie die automatische Skalierung verwenden möchten.
    ...
    2.  Klicken Sie in der Liste der Cloud-Dienste auf den Dienst, der die Anwendung hostet, in der Sie die automatische Skalierung verwenden möchten. Im Bereich "Auf einen Blick" auf der rechten Seite wird die **Abonnement-ID** angezeigt.
    ...
    1.  Klicken Sie im Azure-Verwaltungsportal auf **Cloud-Dienste**. 
    2.  Suchen Sie in der Liste der Cloud-Dienste den Dienst, der die Anwendung hostet, in der Sie die automatische Skalierung verwenden möchten. Der Name des Cloud-Diensts ist das **DNS-Präfix**.
    ...
    2.  Klicken Sie in der Liste der Cloud-Dienste auf den Dienst, der die Anwendung hostet, in der Sie die automatische Skalierung verwenden möchten. Klicken Sie anschließend auf **Instanzen**. In der Spalte **Rolle* wird der Name der Zielrolle angezeigt.
    ...
    3.  Klicken Sie unten auf dem Bildschirm auf die Schaltfläche **Schlüssel verwalten**, um den primären Zugriffsschlüssel zu erhalten.
...
Weitere Informationen zum Inhalt der Dienstmodelldatei finden Sie unter [Speichern der Dienstinformationsdaten][Speichern der Dienstinformationsdaten].

## <a id="DefineAutoscalingRules"> </a>Gewusst wie: Definieren der Regeln für die automatische Skalierung
...
Dieses Beispiel enthält drei Regeln für die automatische Skalierung (eine **Einschränkungsregel** und zwei **reaktive Regeln**), die ein Ziel namens **AutoscalingApplicationRole** bearbeiten. Dabei handelt es sich um den Alias einer Rolle, die im **Dienstmodell** definiert ist:
...
## <a id="Configure"> </a>Gewusst wie: Konfigurieren des Anwendungsblocks für die automatische Skalierung
...
2.  Klicken Sie im Menü **Blöcke** auf **Add Autoscaling Settings**:  
	![image](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling10.png)
  
3.  Erweitern Sie **Autoscaling Settings**, und klicken Sie anschließend auf die Auslassungszeichen (...)     neben **Data Points Store Storage Account**, fügen Sie den **Kontoname** und den **Kontoschlüssel** des Azure-Speicherkontos hinzu, in dem der Block die gesammelten Datenpunkte speichert (siehe     [Gewusst wie: Definieren des Dienstmodells][Gewusst wie: Definieren des Dienstmodells], wenn Sie nicht wissen, wo Sie diese Werte finden), und klicken Sie dann auf **OK**:  

	![image](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling11.png)

4.  Erweitern Sie den Abschnitt **Autoscaling Settings**, sodass die Abschnitte **Rules Store** und **Service Information Store** angezeigt werden. Standardmäßig sind sie für die Verwendung des Azure-Blob-Speichers konfiguriert:  
	![image](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling12.png)


5.  Klicken Sie auf das Pluszeichen (+) neben **Rules Store**, zeigen Sie auf **Set Rules Store**, und klicken Sie dann auf **Use Local File Rules Store** und anschließend auf **Yes**.

6.  Geben Sie in das Feld **File Name** den Dateinamen **rules.xml** ein. Dies ist der Name der Datei, welche die Regeln für die automatische Skalierung enthält:  
	![image](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling13.png)


7.  Klicken Sie auf das Pluszeichen (+) neben **Service Information Store**, zeigen Sie auf **Set Service Information Store**, und klicken Sie dann auf **Use Local File Service Information Store** und anschließend auf **Yes**.

8.  Geben Sie in das Feld **File Name** den Dateinamen **services.xml** ein. Dies ist der Name der Datei, welche die Regeln für die automatische Skalierung enthält:  
	![image](./media/cloud-services-dotnet-autoscaling-application-block/autoscaling14.png)


9.  Klicken Sie im Fenster "Enterprise Library Configuration" im Menü **File** auf **Save**, um die Änderungen an der Konfiguration zu speichern. Klicken Sie danach im Fenster "Enterprise Library Configuration" im Menü **File** auf **Exit**.
...
-   [Verringern der TechNet- und MSDN-Hostingkosten und der Umweltbelastung mit automatischer Skalierung in Azure][Verringern der TechNet- und MSDN-Hostingkosten und der Umweltbelastung mit automatischer Skalierung in Azure]
  ...
  [Vorgehensweise: Instanziieren und Ausführen der automatischen Skalierung]: #Instantiate
