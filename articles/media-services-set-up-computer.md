<properties urlDisplayName="Set Up Computer for Media Services" pageTitle="Einrichten des Computers für Media Services - Azure" metaKeywords="" description="Lernen Sie die Voraussetzungen für Media Services bei Verwendung des Media Services SDK für .NET kennen. Erfahren Sie zudem, wie Sie eine Visual Studio-App erstellen." metaCanonical="" services="media-services" documentationCenter="" title="Setting up your computer for Media Services development" authors="juliako" solutions="" manager="dwrede" editor="" />
...
-   Ein Mediendienstekonto in einem neuen oder existierenden Azure-Abonnement. Weitere Informationen finden Sie unter [Vorgehensweise: Erstellen eines Mediendienstekontos](../media-services-create-account/).
...
-   Verwenden Sie das [windowsazure.mediaservices Nuget](http://nuget.org/packages/windowsazure.mediaservices) -Paket zum Installieren des Azure-SDKs für .NET. Das folgende Beispiel zeigt, wie Sie das Azure-SDK mit **Nuget** installieren können.
...
Dieser Abschnitt beschreibt, wie Sie ein Projekt in Visual Studio erstellen und für die Mediendienste-Entwicklung einrichten.  Es handelt sich um eine C# Windows-Konsolenanwendung, allerdings gelten für andere Typen von Projekten für Mediendienst-Anwendungen (z. B. eine Windows Forms-Anwendung oder ASP.NET-Webanwendung) dieselben Schritte.
   ...
   1. Erstellen Sie eine neue C#-**Konsolenanwendung** in Visual Studio 2013, Visual Studio 2012 oder Visual Studio 2010 SP1. Geben Sie **Name**, **Ort** und **Lösungsname** ein und klicken Sie dann auf **OK**.
   ...
   3. Fügen Sie einen Verweis auf die Assembly **System.Configuration** hinzu. Um einen Verweis auf **System.Configuration** im **Projektmappen-Explorer** hinzuzufügen, klicken Sie mit der rechten Maustaste auf den Knoten **Verweise** und wählen **Verweis hinzufügen...** aus. Wählen Sie im Dialogfeld **Verweise verwalten** den Eintrag **System.Configuration** aus und klicken Sie auf **OK**.
   4. Verwenden Sie das [windowsazure.mediaservices Nuget](http://nuget.org/packages/windowsazure.mediaservices) -Paket, um Verweise zum **Azure SDK für .NET hinzuzufügen.** (alle anderen abhängigen Assemblys werden ebenfalls installiert).  

	Um Verweise mit Nuget hinzuzufügen, gehen Sie wie folgt vor. Klicken Sie im **Hauptmenü** auf **EXTRAS** -> **Bibliothekspaket-Manager** -> **Paket-Manager-Konsole**. Geben Sie im Konsolenfenster **Install-Package windowsazure.mediaservices** ein und drücken Sie die **EINGABETASTE**.
...
Sie haben Ihren Computer eingerichtet und eine Visual Studio-Lösung für die Programmierung von Mediendiensten erstellt und können nun das Thema [Erstellen eines verschlüsselten Medienobjekts und Hochladen in den Speicher][].
