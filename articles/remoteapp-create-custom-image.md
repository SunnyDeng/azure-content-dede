...
## **Erstellen eines Vorlagenimages**##
...
2.	Erstellen Sie mithilfe der Datenträgerverwaltung eine VHD-Datei. 
	...
	2.	Erstellen Sie eine dynamisch expandierende VHD mit 40 GB oder mehr. (Schätzen Sie den Speicherplatz, den Sie für Windows sowie für Ihre Anwendungen und Anpassungen benötigen. Windows Server mit der RDSH-Rolle und dem Desktopdarstellung-Feature benötigen etwa 10 GB.)
		...
		2.	Geben Sie Speicherort, Größe und VHD-Format an. Wählen Sie **Dynamisch expandierend** aus, und klicken Sie anschließend auf **OK**.
		...
		- Klicken Sie mit der rechten Maustaste auf den Datenträger (nicht auf den nicht zugewiesenen Speicherplatz), und klicken Sie dann auf **Datenträger initialisieren**. Wählen Sie als Partitionsstil **MBR** (Master Boot Record) aus, und klicken Sie auf **OK**.
...
1. Installieren von Windows Server 2012 R2:
	1. Erstellen Sie einen neuen virtuellen Computer. Verwenden Sie den Assistenten für neue virtuelle Computer im Hyper-V Manager oder Hyper-V für Clients. 
		...
		2. Wählen Sie auf der Seite "Virtuelle Festplatte anschließen" die Option **Vorhandene virtuelle Festplatte verwenden** aus, und gehen Sie zu der im vorherigen Schritt erstellten VHD.
		2. Wählen Sie auf der Seite "Installationsoptionen" die Option **Betriebssystem von Start-CD/DVD_ROM installieren** aus, und wählen Sie dann den Speicherort der Windows Server 2012 R2-Installationsmedien aus.
	...
	2.  Bearbeiten Sie nach Abschluss des Assistenten die Einstellungen des virtuellen Computers, und nehmen Sie alle für die Installation von Windows und Ihrer Programme erforderlichen Änderungen (z. B. die Anzahl der virtuellen Prozessoren) vor. Klicken Sie dann auf **OK**.
...
1. Installieren Sie die RDSH-Rolle (Remote Desktop Session Host) und das Desktopdarstellung-Feature:
	...
	4. Wählen Sie **Rollenbasierte oder featurebasierte Installation** aus, und klicken Sie auf **Weiter**.
	5. Wählen Sie den lokalen Computer aus der Liste aus, und klicken Sie auf **Weiter**.
	6. Wählen Sie **Remotedesktopdienste** aus, und klicken Sie dann auf **Weiter**.
	7. Erweitern Sie **Benutzeroberflächen und Infrastruktur**, und wählen Sie **Desktopdarstellung** aus.
