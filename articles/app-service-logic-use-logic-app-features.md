...
Im [vorherigen Thema][Erstellen einer neuen Logik-App] haben Sie Ihre erste Logik-App erstellt. Nun erfahren Sie, wie einen vollständigeren Prozess mithilfe von App Services-Logik-Apps erstellen können. In diesem Thema werden die folgenden neuen Logik-App-Konzepte vorgestellt:
...
2. Führen Sie einen Bildlauf zur Aktion  `twitterconnector` durch, suchen Sie den Abfragewert, und ersetzen Sie ihn durch `@concat('#', parameters('topic'))`.
	Die **Concat**-Funktion verknüpft zwei oder mehr Zeichenfolgen. 
...
Über diesen Rückruf können Sie eine Logik-App in Ihrer benutzerdefinierten Anwendung aufrufen. Sie müssen die **Standardauthentifizierung** verwenden. Der Benutzername  `default` wird für Sie erstellt, und das Kennwort ist das Feld **Primärer Zugriffsschlüssel ** auf dem Blatt **Eigenschaften**. Beispiel: 
