<properties 
	pageTitle="Konfigurieren eines benutzerdefinierten Domänennamens für eine Azure-Website" 
	description="Erfahren Sie, wie Sie einen benutzerdefinierten Domänennamen mit Azure-Websites verwenden" 
	services="web-sites" 
	documentationCenter="" 
	authors="blackmist" 
	manager="wpickett" 
...
<div class="dev-center-tutorial-subselector"><a href="/de-de/documentation/articles/web-sites-custom-domain-name/" title="Websites" class="current">Website</a> | <a href="/de-de/documentation/articles/web-sites-traffic-manager-custom-domain-name/" title="Website using Traffic Manager">Website mit Traffic Manager</a></div>

[AZURE.INCLUDE [websites-cloud-services-css-guided-walkthrough](../includes/websites-cloud-services-css-guided-walkthrough.md)]

Wenn Sie eine Website erstellen, weist Azure eine Unterdomäne von *.azurewebsites.NET zu. Wenn Ihre Website beispielsweise **contoso**, heißt, lautet die URL **contoso.azurewebsites.net**. Darüber hinaus weist Azure eine virtuelle IP-Adresse zu.

![contoso.azurewebsites.net-Unterdomäne][Unterdomäne]
...
> [AZURE.NOTE] Die IP-Adresse kann sich ändern, wenn Sie Ihre Website löschen und neu erstellen bzw. den Websitemodus wieder in den Modus "Kostenlos" ändern.
...
Auf der Seite werden A-Einträge und CNAME-Einträge separat aufgeführt, oder es steht eine Dropdownliste zur Auswahl eines Eintragstyps zur Verfügung. Möglicherweise werden auch andere Namen für die Eintragstypen verwendet, wie z. B. **IP-Adresseintrag** anstelle von A-Eintrag oder **Aliaseintrag** anstelle von CNAME-Eintrag.  Normalerweise erstellt die Registrierungsstelle einige Einträge für Sie, sodass Einträge für die Stammdomäne oder allgemeine Unterdomänen wie **www** bereits vorhanden sein können.

Wenn Sie einen Eintrag erstellen oder bearbeiten, können Sie die Felder verwenden, um Ihren Domänennamen einer IP-Adresse (für A-Einträge) oder einer anderen Domäne (für CNAME-Einträge) zuzuordnen. Bei einem CNAME-Eintrag erfolgt die Zuordnung *from* (von) Ihrer benutzerdefinierten Domäne *to* (zu) Unterdomäne mit dem Namen "azurewebsites.net". 
...
Wenn Sie einen A-Eintrag erstellen, fordert Azure-Websites auch einen speziellen CNAME-Eintrag an. Über diesen wird sichergestellt, dass Sie der Besitzer der Domäne sind, die Sie verwenden möchten. Der CNAME-Eintrag muss das folgende Format aufweisen. 

- *Wenn der A-Eintrag der Stammdomäne oder einer Platzhalterdomäne zugeordnet ist:* Erstellen Sie einen CNAME-Eintrag der eine Zuordnung von **awverify.&lt;IhreDomäne&gt;** zu **awverify.&lt;IhreWebsite&gt;.azurewebsites.net** durchführt.  Beispiel: Wenn der A-Eintrag für **contoso.com** erstellt wurde, erstellen Sie einen CNAME-Eintrag für **awverify.contoso.com**.
- *Wenn der A-Eintrag einer spezifischen Unterdomäne zugeordnet ist:* Erstellen Sie einen CNAME-Eintrag der eine Zuordnung von **awverify.&lt;Unterdomäne&gt;** zu **awverify.&lt;IhreWebsite&gt;.azurewebsites.net** durchführt.  Beispiel: Wenn der A-Eintrag für **blogs.contoso.com** erstellt wurde, erstellen Sie einen CNAME-Eintrag für **awverify.blogs.contoso.com**.
...
<!-- Images -->
[Unterdomäne]: media/web-sites-custom-domain-name/azurewebsites-subdomain.png
