<properties urlDisplayName="Get Started with Data" pageTitle="Erste Schritte mit Daten (Windows Store) | Mobile Dev Center" metaKeywords="" description="Erfahren Sie mehr über die ersten Schritte bei der Verwendung von Mobile Services zur Nutzung von Daten in Ihrer Windows Store-App." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Get started with data in Mobile Services" authors="wesmc" solutions="" manager="dwrede" editor="" />
...
In diesem Thema wird gezeigt, wie Sie Azure Mobile Services als Back-End-Datenquelle für eine Windows Store-App nutzen können. In diesem Lernprogramm laden Sie ein Visual Studio 2013-Projekt für eine App herunter, die Daten im Arbeitsspeicher speichert, erstellen einen neuen mobilen Dienst, integrieren den mobilen Dienst in die App und ziegen dann die Datenänderungen an, die während der Ausführung der App vorgenommen werden.
...
6. [Veröffentlichen des mobilen Diensts in Azure]
7. [Testen der Windows Store-App mit dem in Azure gehosteten Dienst]

Für dieses Lernprogramm benötigen Sie Folgendes:

* Ein aktives Azure-Konto. Wenn Sie noch kein Konto haben, können Sie in nur wenigen Minuten ein kostenloses Testkonto erstellen. Ausführliche Informationen finden Sie unter [Kostenlose Azure-Testversion](http://azure.microsoft.com/de-de/pricing/free-trial/?WT.mc_id=A0E0E5C02&returnurl=http%3A%2F%2Fazure.microsoft.com%2Fde-de%2Fdocumentation%2Farticles%2Fmobile-services-dotnet-backend-windows-store-dotnet-get-started-data%2F).
...
Dieses Lernprogramm basiert auf der [GetStartedWithMobileServices app][Developer Code Samples site], bei der es sich um ein Windows Store-App-Projekt in Visual Studio 2013 handelt. Die Benutzeroberfläche dieser App und die durch den Mobile Services-Schnellstart generierte Benutzeroberfläche sind ähnlich, mit der Ausnahme, dass hinzugefügte Einträge lokal im Arbeitsspeicher gespeichert werden. 
...
2. Führen Sie Visual Studio 2013 mit Administratorberechtigung aus, indem Sie mit der rechten Maustaste auf Visual Studio klicken und dann auf **Als Administrator ausführen** klicken.
...
4. Drücken Sie die Taste **F5**, um das Projekt neu zu erstellen und die App zu starten.

5. Geben Sie in der App Text in **Insert a TodoItem** ein, und klicken Sie dann auf **Save**.
...
2. Klicken Sie auf die **Windows Store**-Plattform. Erweitern Sie im Abschnitt **Get Started** den Eintrag **Connect an existing Windows Store app**, und klicken Sie auf die Schaltfläche **Download**, um ein personalisiertes Startprojekt für den mobilen Dienst herunterzuladen. 

    ![][3]

3. Blättern zum Ende des Abschnitts **Get Started** zu dem Schritt mit dem Titel **Veröffentlichen des Diensts in der Cloud**. Klicken Sie auf den im Screenshot gezeigten Link, um eine Veröffentlichungsprofildatei für den soeben heruntergeladenen mobilen Dienst herunterzuladen. 
...
1. Klicken Sie in Visual Studio im Projektmappen-Explorer mit der rechten Maustaste auf das Windows Store-App-Projekt, und klicken Sie dann auf **NuGet-Pakete verwalten**.

    ![][7]

2. Suchen Sie im Dialogfeld "NuGet-Pakete verwalten" den Eintrag **WindowsAzure.MobileServices** in der Online-Paketauflistung, und klicken Sie, um das Azure Mobile Services NuGet-Paket zu installieren. Schließen Sie dann das Dialogfeld.

    ![][8]

3. Suchen Sie im Azure-Verwaltungsportal den Schritt **Verbinden der App und Speichern von Daten im Dienst**. Vergewissern Sie sich, dass **C#** als Sprache ausgewählt ist. Kopieren Sie den Codeausschnitt, der die `MobileServiceClient`-Verbindung erstellt.
...
1. Öffnen Sie "App.xaml.cs" in Visual Studio.  Kommentieren Sie den Code aus, der den `MobileServiceClient` erstellt, welcher mit dem lokal gehosteten mobilen Dienst verbunden wird. Entfernen Sie die Kommentarzeichen für den Code, der den `MobileServiceClient` erstellt, welcher mit dem Dienst in Azure verbunden wird. Speichern Sie die Änderungen an der Datei.
...
2. Drücken Sie in Visual Studio die Taste F5, oder klicken Sie im Debuggen-Menü auf **Debugging starten**. Damit wird die Windows Store-App mit den vorhergehenden Änderungen neu erstellt, bevor die App ausgeführt wird, um sie mit dem remote in Azure gehosteten mobilen Dienst zu verbinden. 
