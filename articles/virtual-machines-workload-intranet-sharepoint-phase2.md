<properties 
	pageTitle="SharePoint-Intranetfarm-Arbeitsauslastung Phase 2: Konfigurieren von Domänencontrollern" 
	description="In dieser zweiten Phase der Intranet-Bereitstellung einer SharePoint 2013-Farm mit SQL Server AlwaysOn-Verfügbarkeitsgruppen in den Azure-Infrastrukturdiensten erstellen und konfigurieren Sie die beiden Active Directory-Domänencontroller." 
	documentationCenter=""
...
# SharePoint-Intranetfarm-Arbeitsauslastung Phase 2: Konfigurieren von Domänencontrollern

In dieser Phase der Intranet-Bereitstellung einer SharePoint 2013-Farm mit SQL Server AlwaysOn-Verfügbarkeitsgruppen in den Azure-Infrastrukturdiensten konfigurieren Sie zwei Domänencontroller im virtuellen Azure-Netzwerk, so dass Webanforderungen der Clients nach Ressourcen der SharePoint-Farm im virtuellen Azure-Netzwerk authentifiziert werden können. Die für die Authentifizierung erforderlichen Daten müssen dann nicht über die VPN- oder ExpressRoute-Verbindung an das lokale Netzwerk gesendet werden.
...
Mit dem folgenden PowerShell-Befehlsblock erstellen Sie die virtuellen Computer für die beiden Domänencontroller. Geben Sie die Werte für die Variablen ein, lassen Sie dabei aber die < and >-Zeichen weg. Die Werte für diesen PowerShell-Befehlsblock entnehmen Sie den folgenden Tabellen:
...
### <a id="logon"></a>Anmelden bei einem virtuellen Computer über eine Remotedesktopverbindung

1.	Klicken Sie im Azure-Verwaltungsportal im linken Bereich auf **Virtuelle Computer**.
...
4.	Im Verwaltungsportal wird angezeigt, dass die RDP-Datei abgerufen wird. Klicken Sie auf **OK**.
...
2.	Testen Sie an der Windows PowerShell-Eingabeaufforderung mit dem Befehl **Ping** die Konnektivität zu Namen und IP-Adressen von Ressourcen in Ihrem Unternehmensnetzwerk.
...
- sqlservice: Benutzerkonto, unter dem SQL-Instanzen ausgeführt werden können
...
1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf **Netzwerke**, und klicken Sie dann auf den Namen Ihres virtuellen Netzwerks (Tabelle V – Element 1 – Wertspalte).
...
6.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf **Virtuelle Computer**, und klicken Sie dann neben dem Namen Ihres ersten Domänencontrollers auf die Spalte **Status**.
...
Zum Fortsetzen der Konfiguration dieser Arbeitsauslastung gehen Sie zu [Phase 3: Konfigurieren der SQL Server-Infrastruktur](virtual-machines-workload-intranet-sharepoint-phase3.md).
