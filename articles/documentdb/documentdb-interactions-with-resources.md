...
# Lernprogramm zu RESTful-Webdiensten: RESTful-Interaktionen mit DocumentDB-Ressourcen 
...
Nachdem Sie dieses Lernprogramm zu RESTful-Webdiensten gelesen haben, können Sie die folgenden Fragen beantworten:
...
Wie im folgenden HTTP-Verbendiagramm dargestellt, kann POST nur bei einer Feedressource ausgegeben werden; PUT und DELETE können nur bei einer Elementressource verwendet werden; GET und HEAD können sowohl bei Feed- als auch bei Elementressourcen verwendet werden.

![Übersicht über HTTP-Verben in diesem Lernprogramm zu RESTful-Diensten][1]

**Interaktionsmodell unter Verwendung der HTTP-Standardmethoden**

## Erstellen einer neuen Ressource mit der HTTP-Methode POST 
...
## Registrieren einer gespeicherten Prozedur über die HTTP-Methode POST
...
## Ausführen einer gespeicherten Prozedur über die HTTP-Methode POST
...
Beachten Sie, dass das HTTP-Verb POST für das Erstellen einer neuen Ressource sowie für das Ausführen einer gespeicherten Prozedur und das Erstellen einer SQL-Abfrage verwendet wird. Zur Veranschaulichung der Ausführung einer SQL-Abfrage berücksichtigen Sie Folgendes.
...
## Verwenden der HTTP-Verben PUT, GET und DELETE
...
Konnektivitätsmodus|Protokoll|Details|DocumentDB SDKs
---|---|---|---
Gateway|HTTPS|Anwendungen können unter Verwendung logischer URIs eine direkte Verbindung mit den Egdeknoten herstellen. Dabei fällt ein zusätzlicher Netzwerkhop an.|REST API, .NET, Node.js, Java, Python, JavaScript
Direkte Konnektivität|HTTPS und TCP|Die Anwendungen können direkt auf die Routingtabelle zugreifen und die direkte Verbindung mit den Replikaten durch das clientseitige Routing herstellen.|.NET
...
- Entitätstags [http://en.wikipedia.org/wiki/HTTP\_ETag](http://en.wikipedia.org/wiki/HTTP_ETag)
