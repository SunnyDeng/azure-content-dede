...
>[AZURE.NOTE] Sie benötigen ein Microsoft Azure-Konto, um dieses Lernprogramm auszuführen. Wenn Sie kein Konto haben, können Sie Ihre <a href="/de-de/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F" target="_blank">MSDN-Abonnentenvorteile aktivieren</a> oder <a href="/de-de/pricing/free-trial/?WT.mc_id=A261C142F" target="_blank">sich für eine kostenlose Testversion registrieren</a>.
...
1. Nennen Sie die Anwendung **ContactManager**, und klicken Sie auf **OK**.
	...
	**Hinweis:** Geben Sie unbedingt "ContactManager" ein. In den Codeblöcken, die Sie später kopieren, wird davon ausgegangen, dass der Projektname "ContactManager" lautet.

1. Wählen Sie im Dialogfeld **Neues ASP.NET-Projekt** die Vorlage **MVC** aus. Stellen Sie sicher, dass **Authentifizierung** auf **Einzelne Benutzerkonten** festgelegt, **Host in der Cloud** aktiviert und **Web-Apps** ausgewählt ist.
...
Mit der Klasse **Contacts** werden die Daten definiert, die für die einzelnen Kontakte gespeichert werden, sowie der Primärschlüssel *ContactID*, der von der Datenbank benötigt wird.
...
7. Drücken Sie STRG+F5, um die Anwendung auszuführen, und klicken Sie dann auf den Link **CM Demo**, oder navigieren Sie zu "http://localhost:(Portnr.)/Cm". 
...
<span></span>
   ...
   Dieser Code erstellt eine neue Rolle namens *canEdit* und den neuen lokalen Benutzer *user1@contoso.com*, und fügt *user1@contoso.com* der Rolle *canEdit* hinzu. Weitere Informationen finden Sie in den [ASP.NET Identity-Lernprogrammen](http://www.asp.net/identity/overview/features-api).
...
Führen Sie den **Update-Database**-Befehl aus, mit dem die **Seed**-Methode ausgeführt wird. Daraufhin wird die gerade hinzugefügte **AddUserAndRole**-Methode ausgeführt. **AddUserAndRole** erstellt den Benutzer *user1@contoso.com* und fügt ihn der Rolle *canEdit* hinzu.
...
4. Klicken Sie mit der rechten Maustaste auf die Web-App, und wählen Sie **Beenden** aus. 
	...
	Sie können die Web-App auch im [Azure-Portal](http://go.microsoft.com/fwlink/?LinkId=529715) auswählen und dann auf das Symbol **Beenden** unten auf der Seite klicken.
...
2. Stellen Sie sicher, dass Sie Daten bearbeiten können, während Sie beim Konto angemeldet sind.
 	**Hinweis:** Sie können sich nicht aus Google aus dieser App abmelden und sich unter einem anderen Google-Konto mit demselben Browser wieder anmelden. Wenn Sie einen Browser verwenden, müssen Sie zu Google navigieren und sich abmelden. Sie können sich mit einem anderen Konto von demselben Authentifizierungsdrittanbieter (wie Google) in einem anderen Browser anmelden.
...
**Hinweis:** Wenn Sie **SQL-Datenbanken** nicht erweitern und die **ContactDB** *nicht* von Visual Studio aus anzeigen können, folgen Sie den Anweisungen unten, um einen Firewallport oder einen Portbereit zu öffnen. Befolgen Sie die Anweisungen unter **Einrichten von Azure-Firewallregeln**. Sie müssen möglicherweise nach dem Hinzufügen der Firewallregel einige Minuten warten, bevor Sie wieder auf die Datenbank zugreifen.
...
5.	[Getting Started with ASP.NET MVC 5 (in englischer Sprache)](http://www.asp.net/mvc/tutorials/mvc-5/introduction/getting-started)
...
[Wichtige Informationen zu ASP.NET in Azure-Web-Apps]: #aspnetwindowsazureinfo
