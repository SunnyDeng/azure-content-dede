...
Update 1.2 umfasst Gerätesoftwareupdates, LSI-Treiberupdates und Updates für die Datenträgerfirmware. Die Software- und LSI-Treiberupdates führen nicht zu einer Unterbrechung des Betriebs und können über das Verwaltungsportal angewendet werden. Die Updates für die Datenträgerfirmware führen zu einer Betriebsunterbrechung und können nur über die Windows PowerShell-Schnittstelle des Geräts angewendet werden.
...
> - Es wird empfohlen, dass Sie die Software- und Treiberupdates über das Azure-Verwaltungsportal installieren. Sie sollten nur dann die Windows PowerShell-Schnittstelle des Geräts (zum Installieren der Updates) verwenden, wenn die Vorabprüfung für das Gateway im Portal nicht erfolgreich ist. Die Installation des Updates kann zwischen 5 und 10 Stunden dauern (Windows-Updates eingeschlossen). Wartungsmodusupdates müssen über die Windows PowerShell-Schnittstelle des Geräts ausgeführt werden. Da Updates im Wartungsmodus den Betrieb unterbrechen, führen sie zu einer Ausfallzeit für Ihr Gerät.
...
1. Stellen Sie sicher, dass die festen Controller-IP-Adressen routingfähig sind und eine Verbindung mit dem Internet herstellen können. Diese festen IPs werden zum Anwenden von Updates auf Ihr Gerät verwendet. Sie können die Konnektivität testen, indem Sie von der Windows PowerShell-Schnittstelle des Geräts aus auf jedem Controller das folgende Cmdlet ausführen:
...
## Installieren von Update 1.2 über das Verwaltungsportal 
...
Sie sollten dieses Verfahren nur dann ausführen, wenn beim Versuch, die Updates über das Verwaltungsportal zu installieren, ein Fehler bei der Gatewayüberprüfung aufgetreten ist. Die Überprüfung zeigt einen Fehler, weil Sie ein Gateway auf einer anderen Netzwerkschnittstelle als DATA 0 zugewiesen haben und auf Ihrem Gerät eine Softwareversion vor Update 1 ausgeführt wird. Wenn Ihr Gerät kein Gateway aufweist, das auf eine andere Netzwerkschnittstelle als DATA 0 eingestellt ist, können Sie das Gerät direkt über das Verwaltungsportal aktualisieren. Siehe [Verwenden des Verwaltungsportals zum Installieren von Update 1](#install-update-12-via-the-management-portal).
...
> - Dieses Verfahren muss nur einmal ausgeführt werden, um Update 1.2 anzuwenden. Über das Azure-Verwaltungsportal können Sie nachfolgende Updates anwenden.
...
- **Option 2**: Entfernen Sie die Gatewaykonfiguration, und installieren Sie das Update direkt über das Verwaltungsportal.
...
Sie sollten dieses Verfahren nur ausführen, wenn Sie Update 0.1, 0.2 oder 0.3 ausführen und die Gatewayüberprüfung beim Installieren der Updates aus dem Verwaltungsportal zu einem Fehler geführt hat. Wenn Sie die Releaseversion (GA) der Software ausführen, wenden Sie sich zum Aktualisieren Ihres Geräts an den [Microsoft Support](storsimple-contact-microsoft-support.md).
...
## Option 2: Verwenden des Azure-Portals zum Anwenden von Update 1.2 nach Entfernung der Gatewaykonfiguration
