...
In diesem Lernprogramm wird davon ausgegangen, dass Sie noch keine Erfahrung mit Serverclustern oder HPC Pack haben. Es soll Ihnen lediglich helfen, ein Hybrid-Rechencluster für Demonstrationszwecke schnell bereitzustellen. Welche Überlegungen und Schritte zur Bereitstellung eines größeren Hybrid-Clusters mit HPC-Pack in einer Produktionsumgebung anzustellen bzw. auszuführen sind, ist in der [ausführlichen Anleitung][ausführlichen Anleitung] beschrieben.

> [WACOM.NOTE] Azure bietet für unterschiedliche Arbeitsplasten Serverressourcen [unterschiedlicher Größen][unterschiedlicher Größen] an. Die Instanzen A8 und A9 kombinieren beispielweise hohe Leistung und Zugang zu einem Anwendungsnetz mit niedrigen Latenzzeiten und hohem Durchsatz, was für bestimmte HPC-Anwendungen benötigt wird. Einzelheiten finden Sie unter [Informationen zu den rechenintensiven A8- und A9-Instanzen][Informationen zu den rechenintensiven A8- und A9-Instanzen].

In diesem Lernprogramm werden die grundlegenden Schritte erläutert:

-   [Voraussetzungen][Voraussetzungen]
-   [Installieren des HPC Pack auf dem Hauptknoten][Installieren des HPC Pack auf dem Hauptknoten]
-   [Erstellen des Azure-Abonnements][Erstellen des Azure-Abonnements]
-   [Konfigurieren des Hauptknotens][Konfigurieren des Hauptknotens]
-   [Hinzufügen von Azure-Knoten zum Cluster][Hinzufügen von Azure-Knoten zum Cluster]
-   [Starten der Azure-Knoten][Starten der Azure-Knoten]
-   [Clusterweites Ausführen eines Befehls][Clusterweites Ausführen eines Befehls]
-   [Ausführen eines Testauftrags][Ausführen eines Testauftrags]
-   [Anhalten der Azure-Knoten][Anhalten der Azure-Knoten]

## Voraussetzungen

> [WACOM.NOTE]Sie benötigen ein Windows Azure-Konto, um dieses Lernprogramm durchführen zu können. Wenn Sie noch kein Konto haben, können Sie in nur wenigen Minuten ein kostenloses Testkonto erstellen. Ausführliche Informationen finden Sie unter [Create a Windows Azure account (in englischer Sprache)][Create a Windows Azure account (in englischer Sprache)].

Darüber hinaus benötigen Sie für dieses Lernprogramm Folgendes:

-   Einen lokalen Computer, auf dem eine Edition von Windows Server 2012 R2 oder Windows Server 2012 ausgeführt wird. Dieser Computer fungiert als der Hauptknoten des HPC-Clusters. Wenn Sie Windows Server noch nicht verwenden, können Sie eine [Evaluierungsversion][Evaluierungsversion] herunterladen und installieren.
...
-   Installationsdateien für HPC Pack 2012 R2. Diese stehen kostenlos zur Verfügung. Das komplette Installationspaket müssen Sie [herunterladen][herunterladen] und die Dateien auf den Hauptknoten-Computer oder auf einen Speicherort im Netzwerk kopieren. Wählen Sie Installationsdateien in derselben Sprache aus wie für die Installation von Windows Server.
...
3.  Klicken Sie im **HPC Pack 2012 R2 Setup**-Bildschirm auf **New installation or add new features to an existing installation**.
    ...
    ![HPC Pack 2012 Setup][HPC Pack 2012 Setup]
...
5.  Klicken Sie auf der Seite **Select Installation Type** auf **Create a new HPC cluster by creating a head node** und dann auf **Next**.
    ...
    ![Installationstyp auswählen][Installationstyp auswählen]
...
6.  Der Installations-Assistent führt vor der Installation verschiedene Tests durch. Klicken Sie auf **Next** auf der Seite **Installation Rules**, wenn alle Tests erfolgreich durchgeführt worden sind. Lesen Sie andernfalls die bereitgestellten Informationen, und nehmen Sie eventuell notwendige Änderungen in Ihrer Umgebung vor. Führen Sie dann die Tests nochmals aus, oder starten Sie gegebenenfalls den Installations-Assistenten erneut.
    ...
    ![Installationsregeln][Installationsregeln]
...
Führen Sie im [Verwaltungsportal][Verwaltungsportal] nachfolgend beschriebene Schritte für Ihr Azure-Abonnement aus. Diese sind erforderlich, damit Sie später Azure-Knoten von dem lokalen Hauptknoten aus bereitstellen können.
...
-   Erstellen eines Azure-Speicherkontos
    ...
    > [WACOM.NOTE]Notieren Sie auch Ihre Azure-Abonnement-ID, die Sie später noch brauchen werden. Sie finden diese Kennung in den Azure-[Kontoinformationen][Kontoinformationen].
...
1.  Melden Sie sich vom Hauptknoten-Computer aus im [Verwaltungsportal][Verwaltungsportal] an.

2.  Klicken Sie auf **Settings** und dann auf **Verwaltungszertifikate**.

3.  Klicken Sie in der Befehlsleiste auf **Upload**.

    ![Zertifikateinstellungen][Zertifikateinstellungen]
...
3.  Geben Sie eine URL für den Clouddienst ein, und klicken Sie dann auf **Cloud-Dienst erstellen**.
    ...
    ![Erstellen des Dienstes][Erstellen des Dienstes]
...
3.  Geben Sie eine URL für das Konto ein, und klicken Sie dann auf **Speicherkonto erstellen**.
    ...
    ![Erstellen des Speicherkontos][Erstellen des Speicherkontos]
...
6.  Geben Sie in das Dialogfeld **Installation Credentials** die Anmeldedaten des Domänenkontos ein, das Sie für die Installation von HPC Pack verwendet haben. Klicken Sie anschließend auf **OK**.

    ![Installations-Anmeldeinformationen][Installations-Anmeldeinformationen]
...
8.  Akzeptieren Sie im Dialogfeld **Specify Node Naming Series** die Standard-Knotenbenennungsreihe, und klicken Sie auf **OK**.
    ...
    ![Knotenbenennung][Knotenbenennung]
...
10. Führen Sie im Create Node Template Wizard, folgende Schritte aus:
    ...
    ![Knotenvorlage][Knotenvorlage]

    b. Klicken Sie auf **Next**, und akzeptieren Sie damit den Standard-Vorlagennamen.

    c. Geben Sie auf der Seite **Provide Subscription Information** Ihre Azure-Abonnement-ID ein (zu finden in den Azure [Kontoinformationen][Kontoinformationen]). Klicken Sie dann neben **Verwaltungszertifikat** auf **Browse**, und wählen Sie **Default HPC Azure Management.** Klicken Sie auf **Next**.

    ![Knotenvorlage][1]

    d. Wählen Sie auf der Seite **Provide Service Information** den Clouddienst und das Speicherkontos aus, das Sie in einem früheren Schritt erstellt haben. Klicken Sie auf **Next**.
...
3.  Wählen Sie auf der Seite **Specify New Nodes** die Azure-Knotenvorlage aus, die Sie zuvor angelegt haben (Standardname **Default AzureNode Template**). Spezifizieren Sie dann **2** Knoten der Größe **Small**, und klicken Sie anschließend auf **Next**.
    ...
    ![Spezifikation von Knoten][Spezifikation von Knoten]

    Einzelheiten zu den Größen virtueller Computer denen Sie in [Größen virtueller Computer und Cloud-Dienste für Windows Azure][Größen virtueller Computer und Cloud-Dienste für Windows Azure].
...
4.  Klicken Sie auf der Seite **Completing the Add Node Wizard** auf **Fertig stellen**.
    ...
    ![Hinzugefügte Knoten][Hinzugefügte Knoten]
...
1.  Klicken Sie in HPC Cluster Manager bei unterlegtem **Node Management**, auf einen oder beide Knoten und dann im Bereich **Actions** auf **Start**.
    ...
    ![Starten von Knoten][Starten von Knoten]

2.  Klicken Sie im Dialogfeld **Start Azure Nodes** auf **Start**.

    ![Starten von Knoten][3]
    ...
    ![Bereitstellung von Knoten][Bereitstellung von Knoten]
...
4.  Sie können wie folgt prüfen, ob die Rolleninstanzen laufen: Klicken Sie im [Verwaltungsportal][Verwaltungsportal] auf **Cloud-Dienste**, dann auf den Namen Ihres Clouddienstes und abschließend auf **Instances**.

    ![Ausführen von Instanzen][Ausführen von Instanzen]
...
5.  Um nun die Azure-Knoten online zu schalten und Clusteraufträge ausführen zu können, wählen Sie die Knoten aus, klicken mit der rechten Maustaste und klicken dann auf **Bring online**.
    ...
    ![Offline geschaltete Knoten][Offline geschaltete Knoten]
...
4.  Klicken Sie auf **View Tasks** und dann auf eine Unteraufgabe, so dass die berechnete Ausgabe dieser Unteraufgabe angezeigt wird.
    ...
    ![Ergebnisse einer Aufgabe][Ergebnisse einer Aufgabe]
...
1.  Wählen Sie in HPC Cluster Manager bei unterlegtem **Node Management** beide Azure-Knoten aus. Klicken Sie dann im Bereich **Actions** auf **Stop**.
    ...
    ![Knoten anhalten][Knoten anhalten]
...
3.  Die Knoten wechseln in den Status **Stopping**. Nach einigen Minuten zeigt HPC Cluster Manager an, dass die Knoten **Not-Deployed** sind.
    ...
    ![Nicht bereitgestellte Knoten][Nicht bereitgestellte Knoten]

4.  Sie können sich wie folgt vergewissern, dass die Rolleninstanzen nicht mehr in Azure ausgeführt werden: Klicken Sie im [Verwaltungsportal][Verwaltungsportal] auf **Cloud-Dienste**, dann auf den Namen Ihres Clouddienstes und abschließend auf **Instances**. In der Produktionsumgebung werden keine Instanzen mehr bereitgestellt.

    ![Keine Instanzen][Keine Instanzen]

    Damit schließt dieses Lernprogramm.

## Zugehörige Ressourcen

-   [HPC Pack 2012 R2 und HPC Pack 2012][HPC Pack 2012 R2 und HPC Pack 2012]
-   [Zugriff auf Azure mit Microsoft HPC Pack][ausführlichen Anleitung]
-   [Bereitstellen von Anwendungen auf Azure-Knoten][Bereitstellen von Anwendungen auf Azure-Knoten]
-   [Microsoft HPC Pack in Azure Virtual Machines][Microsoft HPC Pack in Azure Virtual Machines]

  [Hybrid-HPC-Cluster]: ./media/set-up-hybrid-cluster-microsoft-hpc-pack-2012-sp1/hybrid_cluster_overview.png
  [ausführlichen Anleitung]: http://go.microsoft.com/fwlink/p/?LinkID=200493
  [unterschiedlicher Größen]: http://go.microsoft.com/fwlink/p/?LinkId=389844
  [Informationen zu den rechenintensiven A8- und A9-Instanzen]: http://go.microsoft.com/fwlink/p/?Linkid=328042
  ...
  [Evaluierungsversion]: http://technet.microsoft.com/evalcenter/dn205286.aspx
  [herunterladen]: http://go.microsoft.com/fwlink/p/?linkid=389557
  ...
  [Kontoinformationen]: [https://account.windowsazure.com/Subscriptions
  ...
  [Größen virtueller Computer und Cloud-Dienste für Windows Azure]: http://msdn.microsoft.com/library/windowsazure/dn197896.aspx
