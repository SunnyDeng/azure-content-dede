...
Die obige Abbildung zeigt ein virtuelles Netzwerk mit zwei Subnetzen und vier virtuelle Computer (zwei in jedem Subnetz). Beachten Sie, dass den virtuellen Computer im *Back-End-* Subnetz öffentliche IP-Adressen (PIPs) direkt zugeordnet sind, und die virtuellen Computer im *Front-End-* Subnetz befinden sich hinter einem Azure Load Balancer. Mithilfe von mit jedem Subnetz verknüpften NSGs können Sie den Datenfluss zum Subnetz steuern, unabhängig davon, ob sie mit einer VIP oder einer PIP erstellt wurden.
...
Eine NSG-Regel enthält die folgenden Eigenschaften:

|Eigenschaft|Beschreibung|Beispielwerte|
|---|---|---|
|**Beschreibung**|Beschreibung der Regel|Eingehenden Datenverkehr für alle virtuellen Computer in Subnetz X zulassen|
|**Protokoll**|Protokoll entsprechend der Regel|TCP, UDP oder *||**Quellportbereich**|Quellportbereich entsprechend der Regel|80, 100-200 *||**Zielportbereich**|Zielportbereich entsprechend der Regel|80, 100-200 *||**Quelladresspräfix**|Quelladresspräfix entsprechend der Regel|10\.10.10.1, 10.10.10.0/24, VIRTUAL\_NETWORK|
|**Zieladresspräfix**|Zieladresspräfix entsprechend der Regel|10\.10.10.1, 10.10.10.0/24, VIRTUAL\_NETWORK|
|**Richtung**|Richtung des Datenverkehrs entsprechend der Regel|Eingehend oder ausgehend|
...
Sehen Sie sich alle [Einschränkungen für Netzwerkdienste in Azure](../azure-subscription-service-limits/#networking-limits) an, bevor Sie Ihre Lösung entwerfen.
