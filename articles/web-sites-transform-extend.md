<properties
	pageTitle="Azure App Service-Web-Apps: erweiterte Konfiguration und Erweiterungen"
...
# Azure App Service-Web-Apps: erweiterte Konfiguration und Erweiterungen

Mithilfe von [XML Document Transformation](http://msdn.microsoft.com/library/dd465326.aspx)-Deklarationen (XDT) können Sie die Datei [ApplicationHost.config](http://www.iis.net/learn/get-started/planning-your-iis-architecture/introduction-to-applicationhostconfig) in Azure App Service transformieren. Sie können XDT-Deklarationen auch dazu verwenden, private Erweiterungen hinzuzufügen, um benutzerdefinierte Web-App-Verwaltungsaktionen zu ermöglichen. Dieser Artikel umfasst ein Beispiel für eine PHP-Manager-Web-App-Erweiterung, mit der PHP-Einstellungen über eine Weboberfläche verwaltet werden können.
...
Um diese Transformationsfunktion zu nutzen, erstellen Sie die Datei "ApplicationHost.xdt" mit XDT-Inhalt und platzieren sie im Web-App-Stammverzeichnis. Möglicherweise müssen Sie die Web-App neu starten, damit die Änderungen wirksam werden.
...
Sie können diese Anforderung umgehen, indem Sie in Ihrer Webanwendung entweder nur relative Pfade oder im Falle von ASP.NET-Webanwendungen die `@Html.ActionLink`-Methode verwenden, die die entsprechenden Links automatisch erstellt.
...
Es ist möglich, alle privaten (nicht vorinstallierten) Erweiterungen für Ihre Web-App bei Entwicklungs- und Untersuchungsvorgängen zu deaktivieren, indem Sie eine App-Einstellung hinzufügen mit dem Schlüssel `WEBSITE_PRIVATE_EXTENSIONS` und dem Wert `0`.
