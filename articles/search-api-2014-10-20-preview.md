<properties 
	pageTitle="Azure-Suchdienst-REST-API: 2014-10-20-Preview" 
	description="Azure-Suchdienst-REST-API: 2014-10-20-Preview" 
	services="search" 
	documentationCenter="" 
	authors="HeidiSteen" 
	manager="mblythe"/>
...
# Azure-Suchdienst-REST-API: Version 2014-10-20-Preview #

Dieses Dokument beschreibt die Version **20.10.2014-Preview** der Azure-Suchdienst-REST-API. Dieses ist eine Prototypversion der API, die jederzeit Änderungen unterliegen kann. Stellen Sie keine Abhängigkeiten von dieser API in Code für den produktiven Einsatz her. 

> [AZURE.NOTE] Da es sich um eine experimentelle Version handelt, stehen nicht alle in diesem Dokument beschriebenen Funktionen in der Benutzeroberfläche auf dem Azure-Portal zur Verfügung. Insbesondere die mehrsprachige Unterstützung mithilfe der Option  `analyzer` in Feldern, die neue  `tag`-Bewertungsfunktion und die neue  `suggesters`-Funktion werden in der API nur vorübergehend verfügbar gemacht. Wenn Sie die API verwenden, um eine dieser Funktionen zu aktivieren, können Sie die Indexdefinitionen auf dem Portal sehen, sie aber nicht aktualisieren, und Sie sehen keine Visualisierung der neuen Funktionen.

Zu den anderen API-Inhalten, die sich auf diese Version beziehen, gehören die folgenden:

- [Bewertungsprofile (Azure-Suchdienst-REST-API: 20.10.2014-Preview)](../search-api-scoring-profiles-2014-10-20-preview/)
...
- Alle API-Anforderungen müssen den Parameter  `api-version` für Abfragezeichenfolgen beinhalten. Sein Wert muss auf die Version des aktuellen Dienstrelease festgelegt sein, wie im folgenden Beispiel dargestellt:
    ...
    GET /indexes?api-version=20.10.2014-Preview
...
Der Endpunkt für Dienstvorgänge ist die URL des von Ihnen bereitgestellten Azure-Suchdiensts.: https://*your-service-name*.search.windows.net.


### Versionen ###

Es gibt mehrere API-Versionen für Azure Search. Wenn Sie Azure Search für den Einsatz in einer Anwendung für die Produktion bewerten, empfehlen wir  `api-version=2014-07-31-Preview`. Dies ist zurzeit die einzige festgeschriebene Version. Details finden Sie unter [Search Service-Versionsverwaltung](http://msdn.microsoft.com/library/azure/dn864560.aspx).
...
Sie können einen neuen Index innerhalb eines Azure-Suchdiensts mithilfe einer HTTP-POST-Anforderung erstellen. Der Textteil der Anforderung ist ein JSON-Dokument, in dem der Indexname, die Felder, die Attribute zur Steuerung des Abfrageverhaltens, die Bewertung der Ergebnisse, die Vorschlagsmodule und die CORS-Optionen angegeben sind.
...
Beim Erstellen eines Index können die folgenden Attribute festgelegt werden. Details zur Bewertung und zu Bewertungsprofilen finden Sie unter [Bewertungsprofile (Azure Search Service-REST-API: 2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/).
...
`filterable` - Ermöglicht den Verweis auf das Feld in `$filter`-Abfragen.  `filterable` unterscheidet sich von  `searchable` in der Art, wie Zeichenfolgen verarbeitet werden. Felder vom Typ  `Edm.String` oder  `Collection(Edm.String)`, die  `filterable` sind, werden keiner Wörtertrennung unterzogen, daher sind Vergleiche auf genaue Übereinstimmungen beschränkt. Wenn Sie z. B. ein solches Feld auf  `f` to "sunny day", `$filter=f  festlegen, findet eq 'sunny'` keine Übereinstimmungen,  `$filter=f eq 'sunny day'` w findet aber Übereinstimmungen. Alle anderen Felder sind standardmäßig `filterable`. 
...
`suggestions` - Legt fest, ob das Feld für das AutoAusfüllen bei der Eingabe verwendet werden kann. Dies kann nur für Felder vom Typ  `Edm.String` oder  `Collection(Edm.String)` festgelegt werden.  `suggestions` ist standardmäßig  `false`, da dafür zusätzlicher Platz im Index erforderlich ist. **Hinweis**: Ziehen Sie für Vorschläge die Verwendung der Eigenschaft  `suggesters`, die in Version 2014-10-20-Preview vorgestellt wurde, anstelle dieser Option in Erwägung. In einer kommenden Version wird die Eigenschaft  `suggestions` als veraltet definiert und durch eine separate  `suggesters`-Spezifikation ersetzt.
...
Ein Vorschlagsmodul ermöglicht AutoVervollständigen bei Suchvorgängen. Normalerweise werden während der Benutzereingabe Teilstücke von Suchzeichenfolgen an die Vorschlags-API gesendet, und die API gibt eine Menge von Formulierungen als Vorschlag zurück.

Azure Search befindet sich in der Umstellung auf eine neue Vorschlags-API. In der enger ausgelegten API für Vorschläge in Version 2014-07-31-Preview konnte ein Feld mit `"suggestions": true` gekennzeichnet werden, und dann konnten Präfixvorschläge für kurze Zeichenfolgen (3-25 Zeichen) erstellt werden. Seit der neuen Version 2014-10-20-Preview verfügt Azure Search über eine leistungsfähigere Version der Vorschlagsfunktion, die auf Vorschlagsmodulen ("suggesters") basiert, wie in diesem Abschnitt beschrieben. Diese neue Implementierung beherrscht Präfix- und Infixabgleich und weist eine höhere Toleranz für Fehler in Suchzeichenfolgen auf. Von Version 2014-10-20-Preview an empfehlen wir dringend die Verwendung der neuen Vorschlagsmodul-API.

Die aktuelle Unterstützung von Vorschlagsmodulen funktioniert optimal beim Vorschlagen spezifischer Dokumente anstelle unverbundener Ausdrücke/Formulierungen. Gute Quellfelder für diese Art von Vorschlagsmodulen sind Titel, Namen und andere relativ kurze Formulierungen, mit denen ein Element identifiziert werden kann. Beispiele für Feldarten, die tendenziell weniger effektiv sind, sind stark repetitive Felder, wie Kategorien oder Tags, oder sehr lange Felder, wie Beschreibungs- und Kommentarfelder.
...
- `sourceFields`: Eine Liste mit mindestens einem Feld, das die Quelle für die Inhalte von Vorschlägen bildet. Nur Felder der Typen  `Edm.String` und  `Collection(Edm.String)` kommen als Quelle für Vorschläge in Frage. In der aktuellen Vorschauversion der Vorschlagsmodule können nur Felder verwendet werden, für die kein benutzerdefiniertes Analysemodul festgelegt ist.

Derzeit können Sie in der Auflistung der Vorschlagsmodule in der aktuellen Version der API nur ein Vorschlagsmodul verwenden. 
...
Sie können einen vorhandenen Index in Azure Search mithilfe einer HTTP-PUT-Anforderung aktualisieren. In der öffentlichen Vorschauversion können Aktualisierungen im Hinzufügen neuer Felder zum vorhandenen Schema, im Ändern der CORS-Optionen und im Verändern von Bewertungsprofilen bestehen (siehe dazu [Bewertungsprofile (Azure Search Service-REST-API: 2014-10-20-Preview)](../search-api-scoring-profiles-2014-10-20-preview/). Sie geben den Namen des zu aktualisierenden Index im Anforderungs-URI an:

    PUT https://[search service url]/indexes/[index name]?api-version=[api-version]
    Content-Type: application/json
    api-key: [admin key]

**Wichtig:** Die Unterstützung für Aktualisierungen des Indexschemas beschränkt sich auf Vorgänge, die keine Neuerstellung des Suchindex erfordern. Schemaupdates, für die eine Neuindizierung erforderlich ist, etwa das Ändern von Feldtypen, werden zurzeit nicht unterstützt. Neue Felder können jederzeit hinzugefügt werden, jedoch können vorhandene Felder weder geändert noch gelöscht werden. Das gleiche gilt für Vorschlagsmodule: Einem Vorschlagsmodul können beim Hinzufügen neu erstellte Felder hinzugefügt werden, Felder können aber nicht aus Vorschlagsmodulen entfernt werden, und vorhandene Felder können Vorschlagsmodulen nicht hinzugefügt werden.
...
Der Vorgang **Delete Index** entfernt einen Index und zugeordnete Dokumente aus Ihrem Azure-Suchdienst. Sie können den Indexnamen im Dienstdashboard im Azure-Vorschauportal oder über die API abrufen. Unter [Indizes auflisten](#ListIndexes) zur Verfügung.
...
`highlight=[zeichenfolge]` (optional) - eine Menge durch Kommas getrennter Feldnamen, die für die Hervorhebung von Treffern verwendet wird. Nur  `searchable`-Felder können für die Hervorhebung von Treffern verwendet werden.
  ...
  `highlightPreTag=[string]` (optional) - Ein Zeichenfolgentag, das hervorgehobenen Treffern voransteht. Muss mit festgelegt werden `highlightPostTag`. Reservierte Zeichen in URL müssen prozentcodiert sein (z. B. "%23" anstelle von "#").

  `highlightPostTag=[string]` (optional) - Ein Zeichenfolgentag, das hervorgehobenen Treffern voransteht. Muss mit festgelegt werden `highlightPreTag`. Reservierte Zeichen in URL müssen prozentcodiert sein (z. B. "%23" anstelle von "#").
...
10)  Abrufen von Dokumenten, die einem bestimmten Abfrageausdruck entsprechen
    ...
    GET /indexes/hotels/docs?$filter=(baseRate ge 60 and baseRate lt 300) or hotelName eq 'Fancy Stay'&api-version=20.10.2014-Preview
...
12) Durchsuchen des Index und Rückgabe von Dokumenten, die nach der Entfernung (von näher nach weiter entfernt) von einem Referenzstandort sortiert sind
    ...
    GET /indexes/hotels/docs?search=something&$orderby=geo.distance(location, geography'POINT(-122.12315 47.88121)')&api-version=20.10.2014-Preview
...
Der Vorgang **Lookup Document** ruft ein Dokument bei Azure Search ab. Dies ist nützlich, wenn ein Benutzer auf ein bestimmtes Suchergebnis klickt und Sie spezifische Details zum Dokument nachschlagen möchten.
    ...
    GET https://[dienstname].search.windows.net/indexes/[indexname]/docs/[key]?[abfrageparameter] 
    api-key: [Administratorschlüssel]
...
`highlightPreTag=[zeichenfolge]` (optional, Standardwert = ``) - ein Zeichenfolgentag, das Suchtreffern vorangestellt wird. Muss mit  `highlightPostTag` festgelegt werden. Reservierte Zeichen in URL müssen prozentcodiert sein (z. B. "%23" anstelle von "#").

`highlightPostTag=[zeichenfolge]` (optional, Standardwert = ``) - ein Zeichenfolgentag, das Suchtreffern nachangestellt wird. Muss mit  `highlightPreTag` festgelegt werden. Reservierte Zeichen in URL müssen prozentcodiert sein (z. B. "%23" anstelle von "#").

`suggesterName=[zeichenfolge]` - (optional) Der Name des Vorschlagsmoduls, wie er in der  `suggesters`-Auflistung angegeben ist, die Teil der Indexdefinition ist. Wenn diese Option nicht verwendet wird, basieren Vorschläge auf der Implementierung der Vorversion, die nur Präfixzuordnung unterstützt und mit den Feldern arbeitet, die mit `"suggestions": true` markiert sind.
