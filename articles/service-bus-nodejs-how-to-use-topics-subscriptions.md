...
-Abonnements über Node.js-Anwendungen verwenden. Die behandelten Szenarien umfassen das
...
3.  Sie können den Befehl **ls** manuell aufrufen, um sich davon zu überzeugen, dass der Ordner
    **node\_modules** erstellt wurde. In diesem Ordner finden
    Sie ein **azure**-Paket, das die für den Zugriff auf Service Bus-Themen
...
Das **ServiceBusService**-Objekt ermöglicht Ihnen das Arbeiten mit Themen. Der
...
Durch das Aufrufen von **createTopicIfNotExists** für das Objekt
**ServiceBusService** wird das angegebene Thema zurückgegeben
(falls vorhanden), oder es wird ein neues Thema mit dem angegebenen Namen erstellt. Folgende Codes verwenden
**createTopicIfNotExists**, um das Thema namens "MyTopic"
zu erstellen oder eine Verbindung dazu herzustellen:
...
mit denen Sie die Standardthemeneinstellungen
überschreiben können, wie zum Beispiel die Nachrichtenlebensdauer oder maximale Themengröße. Das folgende Beispiel zeigt, wie Sie die
...
<p>Abonnements sind persistent und bleiben erhalten bis
sie selbst oder die mit ihnen verbundenen Themen gel&ouml;scht werden. Wenn Ihre
Anwendung Logik zum Erstellen eines Abonnements enth&auml;lt, sollte sie zuerst
pr&uuml;fen, ob das Abonnement bereits existiert. Hierf&uuml;r ist die
Methode <strong>getSubscription</strong> zu verwenden.</p>
</div>
...
Sie können mithilfe der **createRule**
...
> Da der Standardfilter automatisch auf alle neuen
...
> die Standardregel mithilfe der Methode **deleteRule** des
...
Zum Senden einer Nachricht an ein Service Bus-Thema muss die Anwendung
die Methode **sendTopicMessage** des Objekts **ServiceBusService**
verwenden. Nachrichten, die an die Service Bus-Themen gesendet werden,
sind **BrokeredMessage**-Objekte. Diese verfügen über einen Satz
von Standardeigenschaften (zum Beispiel **Label** und **TimeToLive**),
ein Wörterbuch, in dem benutzerdefinierte anwendungsspezifische Eigenschaften enthalten sind, sowie einen Bestand beliebiger Zeichenfolgendaten. Eine
 Anwendung kann den Hauptteil der Nachricht festlegen, indem
sie einen Zeichenfolgenwert an **sendTopicMessage** weitergibt.
...
**receiveSubscriptionMessage**-Methode auf dem **ServiceBusService**-Objekt empfangen. 
Nachrichten werden standardmäßig aus dem Abonnement gelöscht,
wenn sie gelesen wurden. Sie können jedoch Nachrichten lesen
(einen Blick darauf werfen) und sperren, ohne sie aus dem Abonnement
zu löschen, indem Sie den optionalen Parameter **isPeekLock** auf **true** festlegen.
...
markiert die Nachricht als verarbeitet, und entfernt sie aus dem Abonnement.

Das Beispiel unten zeigt, wie Nachrichten mithilfe von
**receiveSubscriptionMessage** empfangen und verarbeitet werden können. In diesem Beispiel
wird zuerst eine Nachricht des Abonnements "LowMessages" empfangen
und gelöscht, und anschließend eine Nachricht des Abonnements
"HighMessages" empfangen, wobei für **isPeekLock** "true" festgelegt ist. Anschließend wird die Nachricht durch
