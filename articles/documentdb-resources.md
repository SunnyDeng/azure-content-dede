...
Die Datenbankentitäten, die DocumentDB verwaltet, werden als **Ressourcen** bezeichnet. Jede Ressource wird durch einen logischen URI eindeutig identifiziert. Sie können über standardmäßige HTTP-Verben, Anforderungs-/Antwortheader und Statuscodes mit den Ressourcen interagieren. 
...
- Wie arbeite ich mit gespeicherten Prozeduren, Triggern und UDFs?
...
>[AZURE.NOTE] DocumentDB bietet ein hocheffizientes TCP-Protokoll mit einem REST-konformen Kommunikationsmodell und ist über das [.NET Client-SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx) verfügbar.

![][1]  
**Hierarchical resource model under a database account**   

Um mit Ressourcen zu arbeiten, müssen Sie [ein DocumentDB-Datenbankkonto erstellen](./documentdb-create-account.md). Verwenden Sie hierzu Ihr Azure-Abonnement. Ein Datenbankkonto kann aus einer Reihe von **Datenbanken** bestehen, die jeweils mehrere **Sammlungen** enthalten, die wiederum jeweils **gespeicherte Prozeduren, Trigger, benutzerdefinierte Funktionen, Dokumente** und zugehörige **Anhänge** enthalten. Eine Datenbank verfügt auch über mit ihr verbundene **Benutzer**, von denen jeder über eine Reihe von **Berechtigungen** verfügt, um auf Sammlungen, gespeicherte Prozeduren, Trigger, UDFs, Dokumente und Anhänge zuzugreifen. Während Datenbanken, Benutzer, Berechtigungen und Sammlungen vom System definierte Ressourcen mit bekannten Schemas sind, enthalten Dokumente und Anhänge beliebige, benutzerdefinierte JSON-Inhalte.  

|Ressource 	|Beschreibung
|-----------|-----------
|Datenbankkonto	|Ein Datenbankkonto ist mit einer oder mehreren Kapazitätseinheiten verknüpft, die den bereitgestellten Dokumentspeicher und -durchsatz, eine Gruppe von Datenbanken und den Blobspeicher darstellen. Sie können mithilfe Ihres Azure-Abonnements mindestens ein Datenbankkonto erstellen.
...
|Benutzer	|Der logische Namespace für Bereichs-/Partitionierungsberechtigungen. 
...
|Gespeicherte Prozedur|Mit JavaScript geschriebene Anwendungslogik, die mit einer Sammlung registriert und über Transaktionen innerhalb des Datenbankmoduls ausgeführt wird.
...
|Anhang	|Anhänge sind besondere Dokumente mit Verweisen und Metadaten, die mit einem externen Blob/mit externen Medien verknüpft sind. Entwickler können auswählen, ob der Blob von DocumentDB verwaltet oder mit einem externen Blobdienstanbieter wie OneDrive, Dropbox usw. gespeichert wird. 
...
|-------------------|-----------
|/dbs	|Feeds der Datenbanken unter einem Datenbankkonto.
|/dbs/{_rid-db}	|Datenbank mit der eindeutigen ID-Eigenschaft mit dem Wert {_rid-db}.
|/dbs/{_rid-db}/colls/	|Feed der Sammlungen unter einer Datenbank.
|/dbs/{_rid-db}/colls/{_rid-coll}	|Sammlung mit der eindeutigen ID-Eigenschaft mit dem Wert {_rid-coll}.
|/dbs/{_rid-db}/users/	|Feed der Benutzer unter einer Datenbank. 
|/dbs/{_rid-db}/users/{_rid-user}	|Benutzer mit der eindeutigen ID-Eigenschaft mit dem Wert {_rid-user}.
|/dbs/{_rid-db}/users/{_rid-user}/permissions	|Feed der Berechtigungen unter einer Datenbank. 
...
<td valign="top" ><p>Legen Sie diese Eigenschaft fest, um die Standardkonsistenzebene für alle Sammlungen unter Ihrem Datenbankkonto zu konfigurieren. Sie können die Konsistenzebene mithilfe des Anforderungsheaders [x-ms-consistency-level] anforderungsweise außer Kraft setzen. Zukünftig wird möglicherweise die Außerkraftsetzung der Konsistenzebene auf Sammlungsbasis unterstützt. </p>
...
Beachten Sie, dass Sie zusätzlich zur Bereitstellung, Konfiguration und Verwaltung Ihres Datenbankkontos über das Azure-Portal auch über [Azure DocumentDB-REST-APIs](https://msdn.microsoft.com/library/azure/dn781481.aspx) und [Client-SDKs](https://msdn.microsoft.com/library/azure/dn781482.aspx) die DocumentDB-Datenbankkonten programmgesteuert erstellen und verwalten können.  

## Datenbanken
Eine DocumentDB-Datenbank ist ein logischer Container einer oder mehrerer Sammlungen und Benutzer, wie im folgenden Diagramm dargestellt. Sie können eine beliebige Anzahl von Datenbanken unter einem DocumentDB-Datenbankkonto erstellen, abhängig von der angegebenen Einschränkung.  
...
-	Wählen Sie, ob die Sammlung automatisch alle Dokumente indiziert oder nicht. Standardmäßig werden automatisch alle Dokumente indiziert. Sie können die automatische Indizierung deaktivieren und wahlweise nur bestimmte Dokumente zum Index hinzufügen. Umgekehrt können Sie wahlweise auch nur bestimmte Dokumente ausschließen. Dies können Sie erreichen, indem Sie für die Eigenschaft "automatic" der Indizierungsrichtlinie einer Sammlung den Wert "true" oder "false" festlegen und beim Einfügen, Ersetzen oder Löschen eines Dokuments den Anforderungsheader [x-ms-indexingdirective] verwenden.  
...
Das Abfragemodell von DocumentDB versucht die Balance zwischen Funktionalität, Effizienz und Einfachheit zu finden. Das Datenbankmodul von DocumentDB kompiliert die SQL-Abfrageanweisungen und führt sie aus. Sie können eine Sammlung mithilfe der [Azure DocumentDB-REST-APIs](https://msdn.microsoft.com/library/azure/dn781481.aspx) oder einem der [Client-SDKs](https://msdn.microsoft.com/library/azure/dn781482.aspx) abfragen. Das .NET-SDK verfügt über einen LINQ-Anbieter. In einer zukünftigen Version werden systemeigene Unterstrichzuordnungen bereitgestellt, die sowohl von clientseitigen [DocumentDB-JavaScript-SDKs](https://github.com/Azure/azure-documentdb-js) als auch von serverseitigen gespeicherten Prozeduren und Triggern verwendet werden können.   
...
Der Client kann die obige JavaScript-Logik zur transaktionalen Ausführung über HTTP POST an die Datenbank übermitteln. Weitere Informationen zur Verwendung von HTTP-Methoden finden Sie unter [RESTful-Interaktionen mit DocumentDB-Ressourcen](../documentdb-interactions-with-resources/). 
...
Eine benutzerdefinierte Funktion kann im Rahmen der SQL-Abfrage angegeben werden und wird als Methode verwendet, um die vorhandene [SQL-Abfragesprache von DocumentDB](https://msdn.microsoft.com/library/azure/dn782250.aspx) zu erweitern.   
...
![][4]  
**Horizontale Partitionierungsstrategien und Benutzermodellierung**
...
## Nächste Schritte
Weitere Informationen zum Arbeiten mit Ressourcen mithilfe von HTTP-Befehlen finden Sie unter [RESTful-Interaktionen mit DocumentDB-Ressourcen](../documentdb-interactions-with-resources/).
