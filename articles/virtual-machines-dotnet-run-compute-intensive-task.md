...
7.  Aktivieren Sie das Kontrollkästchen. Der Dienstnamespace wird nun erstellt und aktiviert. Eventuell müssen Sie einige Minuten warten, bis die Ressourcen für Ihr Konto bereitgestellt sind.

    ![Screenshot beim Klicken auf "Erstellen"](./media/virtual-machines-dotnet-run-compute-intensive-task/ClickCreate.png)

Der erstellte Namespace wird dann im Verwaltungsportal angezeigt und nach einem Moment aktiviert. Warten Sie, bis **Aktiv** als Status angezeigt wird, bevor Sie mit dem nächsten Schritt fortfahren.
...
1.  Laden Sie auf dem Entwicklungscomputer (der nicht mit dem erstellten virtuellen Computer identisch sein muss) das [Azure-SDK für .NET](http://www.windowsazure.com/en-us/develop/net/) herunter.
...
7.  Kompilieren Sie die Anwendung. Damit wird **TSPSolver.exe** im Ordner **bin** des Projekts (entweder **bin\\release** oder **bin\\debug**, je nachdem, ob Sie als Ziel eine Releaseversion oder eine Debugversion vorgesehen haben) erstellt. Später kopieren Sie diese ausführbare Datei und Microsoft.ServiceBus.dll auf den virtuellen Computer.
    	...
    	namespace TSPSolver
    	{
        	class Program
        	{
            	// Wert, der angibt, wie oft die Konsole aktualisiert werden soll.
            ...
            static void Main(string[] args)
            {
                try
                {
                    ...
                    int numCities = 10; // Als Standardwert verwenden, wenn in der
                    // Befehlszeile kein Wert angegeben wird.
                    if (args.Count() != 0)
                    {

                        if (args[0].ToLower().CompareTo("createqueue") == 0)
                        {
                            // Es findet kein anderer Verarbeitungsschritt als die Erstellung der Warteschlange statt.
                        ...
                        {
                            // Es findet kein anderer Verarbeitungsschritt als das Löschen der Warteschlange statt.
                            namespaceManager = NamespaceManager.CreateFromConnectionString(connectionString);
                        ...
                        // Eine Warteschlange wird weder erstellt noch gelöscht.
                        // Als eingegebener Wert wird die Anzahl der zu lösenden Städte angenommen.
    	...
    	namespace TSPClient
    	{
            ...
            static void Main(string[] args)
            {
                ...
                try
                {
                    ...
                    int waitMinutes = 3;  // Als Standardwert verwenden, wenn in der
                    // Befehlszeile kein Wert angegeben wird.
         ...
         "minute" : waitMinutes.ToString() + " minutes";
                    ...
                    while (true)
                    {
                        ...
                        if (message != null)
                        {
                            ...
                            {
                                // Zeigt ein Problem an. Entsperrt die Nachricht in der Warteschlange.
                                message.Abandon();
                                throw e;
                            }
                        }
