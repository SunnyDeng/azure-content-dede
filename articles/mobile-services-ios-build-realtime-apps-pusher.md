...
1.  [Erstellen eines neuen Pusher-Kontos][Erstellen eines neuen Pusher-Kontos]
2.  [Aktualisieren Ihrer App][Aktualisieren Ihrer App]
3.  [Installieren von Serverskripts][Installieren von Serverskripts]
4.  [Testen Ihrer App][Testen Ihrer App]
...
Mit der [libPusher][libPusher]-Bibliothek erhalten Sie Zugriff auf den Pusher von iOS aus.

1.  Laden Sie die libPusher-Bibliothek [von hier][von hier] herunter.
...
1.  Klicken Sie im Projektverzeichnis im Projekt-Explorer auf **Build Phases**. Klicken Sie danach auf **Add Build Phase** und **Add Copy Files**.

2.  Ziehen Sie die Datei **libPusher-combined.a** vom Projekt-Explorer in die neue Build-Phase.

3.  Ändern Sie **Destination** in **Frameworks**, und klicken Sie auf **Copy only when installing**.

    ![][1]

4.  Fügen Sie innerhalb des Bereichs **Binärdatei mit Bibliotheken verknüpfen** die folgenden Bibliotheken hinzu:
...
5.  Ermitteln Sie innerhalb der **Build Settings** abschließend die Build-Zieleinstellung **Other Linker Flags**, und fügen Sie das Kennzeichen **-all\_load** hinzu.
...
9.  Fügen Sie in **QSTodoListViewController.m** die folgende Zeile unter den anderen [\*\*@synthesise\*\*][\*\*@synthesise\*\*]-Zeilen hinzu, um die neue Eigenschaft zu implementieren:
...
5.  Ersetzen Sie die Platzhalter im oben angegebenen Skript durch jene Werte, die Sie zuvor aus dem Verbindungsinformationsdialog kopiert haben:
    ...
    -   **`**your_app_id**`**: der app\_id-Wert
    -   **`**your_app_key**`**: der app\_key-Wert
