<properties
 ...
 description="Hier finden Sie Hintergrundinformationen und Überlegungen zur Verwendung der rechenintensiven Azure-Instanzen A8 und A9, A10 und A11."
...
* **Virtuelles Netzwerk**: Ein virtuelles Azure-Netzwerk ist nicht erforderlich, um die rechenintensiven Instanzen zu verwenden. Sie benötigen möglicherweise jedoch für viele IaaS-Szenarien mindestens ein cloudbasiertes virtuelles Azure-Netzwerk oder für den Zugriff auf lokale Ressourcen, z. B. einen Anwendungslizenzserver, eine Site-to-Site-Verbindung. Sie müssen ein neues \(regionales\) virtuelles Netzwerk erstellen, bevor Sie die Instanzen bereitstellen. Das Hinzufügen von A8-, A9-, A10- oder A11-VMs zu einem virtuellen Netzwerk in einer Affinitätsgruppe wird nicht unterstützt. Weitere Informationen finden Sie unter [Erstellen eines virtuellen Netzwerks \(VNet\)](../virtual-network/virtual-networks-create-vnet.md) sowie unter [Konfigurieren eines virtuellen Netzwerks mit einer Site-to-Site-VPN-Verbindung](../vpn-gateway/vpn-gateway-site-to-site-create.md).

* **Clouddienst oder Verfügbarkeitsgruppe**: Zum Herstellen der Verbindung über das RDMA-Netzwerk müssen die A8- und A9-Instanzen im gleichen Clouddienst \(für IaaS-Szenarien mit Linux- oder Windows-basierten virtuellen Computern in der Azure-Dienstverwaltung oder für PaaS-Szenarien mit Windows Server\) oder in der gleichen Verfügbarkeitsgruppe \(für Linux- oder Windows-basierte virtuelle Computer im Azure-Ressourcen-Manager\) bereitgestellt werden.
...
>[AZURE.NOTE]Für IaaS-Szenarien muss den virtuellen Computern die [HpcVmDrivers-Erweiterung](https://msdn.microsoft.com/library/azure/dn690126.aspx) hinzugefügt werden, sodass die für RDMA-Verbindungen benötigten Windows-Treiber installiert werden.
