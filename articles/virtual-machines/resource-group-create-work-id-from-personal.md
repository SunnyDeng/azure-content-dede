...
Sehr gut. Nun erfahren Sie, wie Ihre Standarddomäne lautet ... klicken Sie auf **DOMÄNEN**.
...
Klicken Sie auf **BENUTZER**, und suchen Sie dann nach Ihrem einzigen persönlichen Konto. Daraufhin sollte in der Spalte **QUELLE** angezeigt werden, dass es sich um ein `Microsoft account` handelt. Sie möchten einen Benutzer in Ihrer **.onmicrosoft.com**-Azure Active Directory-Standarddomäne erstellen.
...
Notieren Sie den Benutzernamen und die E-Mail-Adresse, oder geben Sie eine E-Mail-Adresse ein, an die die Informationen gesendet werden sollen. In beiden Fällen benötigen Sie die Informationen in Kürze für die Anmeldung.

![](./media/resource-group-create-work-id-from-personal/receivedtemporarypassworddialog.png)

Nun wird der neue Benutzer angezeigt, in diesem Fall `Ahmet the Developer`, der aus Azure Active Directory stammt. Sie haben die neue Arbeits- oder Schulidentität mit Azure Active Directory erstellt. Diese Identität verfügt jedoch noch nicht über Berechtigungen zum Verwenden von Azure-Ressourcen.

![](./media/resource-group-create-work-id-from-personal/defaultdirectoryusersaftercreate.png)

Wenn Sie eine E-Mail mit den Informationen senden, sieht diese ungefähr folgendermaßen aus:
...
Nun müssen Sie den neuen Benutzer als Co-Administrator Ihres Abonnements hinzufügen, damit der neue Benutzer sich am Verwaltungsportal anmelden kann. Suchen Sie hierzu das Symbol **Einstellungen** im unteren linken Bereich, und klicken Sie darauf.

![](./media/resource-group-create-work-id-from-personal/thesettingswidget.png)

Klicken Sie oben im Haupteinstellungsbereich auf **ADMINISTRATOREN** – es sollte nur Ihre persönliche Microsoft-Konto-Identität angezeigt werden. Klicken Sie unten auf der Seite auf **+HINZUFÜGEN**, um einen Co-Administrator anzugeben. Geben Sie hier die E-Mail-Adresse des neuen Benutzers ein, den Sie erstellt haben, einschließlich der Standarddomäne. Wie unten gezeigt sollte ein grünes Häkchen neben dem Benutzer im Standardverzeichnis angezeigt werden. Denken Sie daran, alle Abonnements auswählen, die dieser Benutzer verwalten können soll.
...
Sie werden sofort aufgefordert, ein neues Kennwort zu erstellen. Nach Abschluss geschieht jedoch Folgendes ...
...
Nun können Sie ganz normal fortfahren. Beispielsweise können Sie nun mit Ihrer neuen Azure Active Directory-Identität [Azure-Ressourcengruppenvorlagen](xplat-cli-azure-resource-manager.md) verwenden.
