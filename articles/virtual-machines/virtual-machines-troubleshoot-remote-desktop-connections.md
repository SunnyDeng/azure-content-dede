...
Falls Sie keine Verbindung mit Windows-basierten virtuellen Azure-Computern herstellen können, finden Sie in diesem Thema die Beschreibung eines methodischen Ansatzes für die Korrektur und Bestimmung der Hauptursache von Remotedesktopverbindungen.
...
Orientieren Sie sich bei der Verwendung dieser Abschnitte an der Fehlermeldung, die Sie erhalten, wenn Sie versuchen, eine Verbindung herzustellen.
...
Als vorübergehende Problemumgehung speichern Sie eine lokale Kopie der RDP-Datei aus dem Azure-Portal, und führen Sie diesen Befehl an einer Windows PowerShell-Eingabeaufforderung aus, um eine Verbindung herzustellen.
...
Es folgt eine RDP-Beispieldatei, die vom Azure-Portal generiert wird:

	full address:s:tailspin-azdatatier.cloudapp.net:55919
	prompt for credentials:i:1

Der Adressteil in diesem Beispiel besteht aus dem vollqualifizierten Domänennamen des Clouddiensts, der den virtuellen Computer enthält, (in diesem Beispiel "tailspin-azdatatier.cloudapp.net") und dem externen TCP-Port des Endpunkts für den Remotedesktop-Datenverkehr (55919).
...
- Wenn Sie eine lokal gespeicherte RDP-Datei verwenden, versuchen Sie, die vom Azure-Portal generierte Datei zu verwenden, um sicherzustellen, dass der richtige DNS-Name für den virtuellen Computer oder Clouddienst und der richtige Endpunktport des virtuellen Computers angegeben werden.

### Meldungsfenster der Remotedesktopverbindung: Authentifizierungsfehler. Die lokale Sicherheitsautorität (LSA) ist nicht erreichbar.
...
- Wenn es sich um ein lokales Benutzerkonto auf dem virtuellen Computer handelt, überprüfen Sie die Schreibweise des Namens des virtuellen Computers.
- Wenn das Benutzerkonto ein Active Directory-Domänenkonto ist, überprüfen Sie die Schreibweise des Domänennamens.
- Wenn das Benutzerkonto ein Active Directory-Domänenkonto ist und der Domänenname richtig geschrieben ist, stellen Sie sicher, dass ein Domänencontroller für die Active Directory-Domäne verfügbar ist. Dies kann ein häufiges Problem in einem virtuellen Azure-Netzwerk sein, das Domänencontroller enthält und in dem ein Domänencontrollercomputer nicht gestartet wurde. Eine vorübergehende Lösung ist die Verwendung eines lokalen Administratorkontos statt eines Domänenkontos.
