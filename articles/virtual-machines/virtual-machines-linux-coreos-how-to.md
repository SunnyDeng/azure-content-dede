<properties
	...
	description="Beschreibt CoreOS, das Erstellen eines Clusters mit virtuellen CoreOS-Computern in Azure im klassischen Bereitstellungsmodell und dessen grundlegende Verwendung."
...
[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-include.md)]In diesem Artikel wird das Erstellen von virtuellen CoreOS-Computern mithilfe der Azure-Befehlszeilenschnittstelle im klassischen Bereitstellungsmodell beschrieben. Sie können virtuelle CoreOS-Computer auch mithilfe von Vorlagen im [Ressourcen-Manager-Bereitstellungsmodell](https://azure.microsoft.com/documentation/templates/coreos-with-fleet-multivm/) erstellen.
...
## <a id='usingcoreos'>Verwenden von CoreOS in Azure</a>
...
In Ihrem Arbeitsverzeichnis sollten die beiden Dateien `myPrivateKey.key` und `myCert.pem` enthalten sein.


### Abrufen der etcd-ID Ihres Clusters

Für den `etcd`-Daemon von CoreOS ist eine Ermittlungs-ID zum automatischen Abfragen sämtlicher Knoten im Cluster erforderlich. Geben Sie Folgendes ein, um Ihre Ermittlungs-ID abzurufen und sie in der `etcdid`-Datei zu speichern:
...
Erstellen Sie im selben Arbeitsverzeichnis eine Datei mit Ihrem bevorzugten Text-Editor mit dem folgenden Text, und speichern Sie sie als `cloud-config.yaml`. (Sie können sie mit einem beliebigen Dateinamen speichern. Wenn Sie Ihre virtuellen Computer jedoch im nächsten Schritt erstellen, müssen Sie in der Option **--custom-data** für den Befehl **azure vm create** auf diesen Dateinamen verweisen.)

> [AZURE.NOTE]Geben Sie zum Abrufen der Ermittlungs-ID aus der von Ihnen oben erstellten Datei `etcdid` den Befehl `cat etcdid` ein, und ersetzen Sie `<token>` in der folgenden Datei `cloud-config.yaml` mit der generierten Zahl aus der Datei `etcdid`. Wenn Sie am Ende nicht in der Lage sind, Ihr Cluster zu überprüfen, haben Sie möglicherweise einen Schritt übersehen!
...
2. Suchen Sie das CoreOS-Image. Um die Images jederzeit finden zu können, geben Sie `azure vm image list | grep CoreOS` ein. Sie sehen eine Liste der Ergebnisse, die folgender Liste ähnelt:

	data: 2b171e93f07c4903bcad35bda10acf22\_\_CoreOS-Stable-522.6.0 Public Linux

3. Erstellen Sie einen Clouddienst für Ihren grundlegenden Cluster, indem Sie`azure service create <cloud-service-name>` eingeben. <*cloud-service-name*> ist hierbei der Name Ihres CoreOS-Clouddiensts. In diesem Beispiel wird der Name **`coreos-cluster`** verwendet. Sie müssen den Namen erneut verwenden, um Ihre virtuellen CoreOS-Computerinstanzen im Clouddienst zu erstellen.

	Wenn Sie Ihre bisherige Arbeit im [Vorschauportal](https://portal.azure.com) überprüfen, können Sie sehen, dass Ihr Clouddienstname sowohl eine Ressourcengruppe als auch eine Domäne ist, wie in der folgenden Abbildung gezeigt wird:

	![][CloudServiceInNewPortal]

4. Stellen Sie eine Verbindung mit Ihrem Clouddienst her, und erstellen Sie darin mithilfe des Befehl **azure vm create** einen neuen virtuellen CoreOS-Computer. Sie übergeben den Speicherort Ihres X.509-Zertifikats in der Option **--ssh-cert**. Erstellen Sie Ihr erstes Image für virtuelle Computer, indem Sie Folgendes eingeben (beachten Sie, **coreos-cluster** durch den von Ihnen erstellten Clouddienstnamen zu ersetzen):
...
Geben Sie nach dem Herstellen der Verbindung `sudo fleetctl list-machines` ein, um anzuzeigen, ob der Cluster bereits alle virtuellen Computer im Cluster identifiziert hat. Sie sollten eine Antwort empfangen, die in etwa der im Folgenden aufgeführten entspricht:
...
Erstellen Sie **fleet**, indem Sie zum Verzeichnis `fleet` wechseln und Folgendes eingeben:
