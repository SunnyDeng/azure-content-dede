...
- Verwenden Sie [Storage Premium](../storage/storage-premium-storage-preview-portal.md).

- Verwenden Sie als [VM-Größe](virtual-machines-size-specs.md) für SQL Enterprise Edition DS3 oder höher bzw. für SQL Standard Edition DS2 oder höher.

- Verwenden Sie mindestens 2 [P30-Datenträger](../storage/storage-premium-storage-preview-portal/#scalability-and-performance-targets-whde-DEing-premium-storage) (1 für Protokolldateien; 1 für Datendateien und TempDB).

- Speichern Sie das [Speicherkonto](../storage/storage-create-storage-account.md) und die SQL Server-VM in derselben Region.

- Deaktivieren Sie [georedundanten Azure-Speicher](../storage/storage-redundancy.md) (Geo-Replikation) für das Speicherkonto.
...
- Aktivieren Sie das Caching für Lesevorgänge auf den Datenträgern, auf denen die Datendateien und TempDB gehostet werden.
...
- Nehmen Sie Formatierungen mit dokumentierten Zuordnungsgrößen vor.

- Aktivieren Sie die Datenbankseitenkomprimierung.

- Aktivieren Sie die sofortige Dateiinitialisierung für Datendateien.

- Begrenzen oder deaktivieren Sie die automatische Vergrößerung der Datenbank.
...
- Wenden Sie SQL Server-Leistungskorrekturen an.

- Richten Sie Standardspeicherorte ein.

- Aktivieren Sie gesperrte Seiten.

- Sichern Sie direkt in BLOB-Speicher.
...
- **SQL Server Standard Edition**: DS2 oder höher
...
- **Anzahl der Datenträger für Daten und Protokolldateien**: Verwenden Sie mindestens zwei [P30-Datenträger](../storage/storage-premium-storage-preview-portal.md), von denen ein Datenträger die Protokolldatei(en) und der andere Datenträger die Datendatei(en) und TempDB enthält. Für einen höheren Durchsatz werden möglicherweise zusätzliche Datenträger benötigt. Um die Anzahl der Datenträger für Daten zu bestimmen, müssen Sie die Anzahl der für Ihre Daten- und Protokolldatei-Datenträger verfügbaren IOPS analysieren. Diese Informationen finden Sie in den Tabellen zu IOPS pro [VM-Größe](virtual-machines-size-specs.md) und Datenträgergröße im folgenden Artikel: [Verwenden von Storage Premium für Datenträger](../storage/storage-premium-storage-preview-portal.md). Wenn Sie mehr Bandbreite benötigen, können Sie zusätzliche Datenträger hinzufügen und Datenträgerstriping verwenden. Wenn Sie Storage Premium nicht verwenden, lautet die Empfehlung, die für Ihre [VM-Größe](virtual-machines-size-specs.md) maximal unterstützte Anzahl von Datenträgern für Daten hinzuzufügen und Datenträgerstriping zu verwenden. Weitere Informationen zu Datenträgerstriping finden Sie unter dem entsprechenden Abschnitt unten.
...
- **Datenträgerstriping**: Es wird empfohlen, die folgenden Richtlinien zu beachten:
	...
	- Für Windows 2008 R2 oder früher können Sie dynamische Datenträger (Betriebssystem-Stripesetvolumes) verwenden, und die Stripesetgröße ist immer 64 KB. Beachten Sie, dass diese Option seit Windows 8 / Windows Server 2012 veraltet ist. Informationen hierzu finden Sie in der Supporterklärung unter [Virtual Disk Service is transitioning to Windows Storage Management API](https://msdn.microsoft.com/library/windows/desktop/hh848071.aspx).
...
- Wenn Sie SQL Server 2012 ausführen, installieren Sie Service Pack 1, kumulatives Update 10. Dieses Update enthält ein Fix für schlechte E/A-Leistung beim Ausführen von SELECT in temporären Tabellenanweisungen in SQL Server 2012 Weitere Informationen finden Sie in diesem [Knowledge Base-Artikel](http://support.microsoft.com/kb/2958012).
...
- **SQL Server-Datendateien in Azure**: Dieses neue Feature [SQL Server-Datendateien in Windows Azure](https://msdn.microsoft.com/library/dn385720.aspx), ist am SQL Server 2014 verfügbar. Das Ausführen von SQL Server mit Datendateien in Azure zeigt vergleichbare Leistungseigenschaften wie die Verwendung von Azure-Datenträgern.
...
Weitere Themen zu SQL Server-VMs finden Sie unter [Übersicht zu SQL Server auf virtuellen Azure-Computern.](virtual-machines-sql-server-infrastructure-services.md)
