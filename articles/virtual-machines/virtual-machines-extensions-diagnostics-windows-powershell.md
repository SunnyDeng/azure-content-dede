<properties pageTitle="Aktivieren der Diagnose auf einer virtuellen Azure-Maschine unter Windows mithilfe von PowerShell | Microsoft Azure" description="Hier erfahren Sie, wie Sie mithilfe von PowerShell die Diagnose auf einer virtuellen Azure-Maschine unter Windows aktivieren." services="virtual-machines" documentationCenter="" authors="sbtron" manager="" editor="""/>
...
# Aktivieren der Diagnose auf einer virtuellen Azure-Maschine unter Windows mithilfe von PowerShell

[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-both-include.md)]

Mithilfe der Azure-Diagnoseerweiterung können Sie Diagnosedaten wie Anwendungsprotokolle, Leistungsindikatoren und Ähnliches von einer virtuellen Azure-Maschine unter Windows sammeln. In diesem Artikel erfahren Sie, wie Sie die Azure-Diagnoseerweiterung für eine virtuelle Maschine mithilfe von PowerShell aktivieren. Informationen zu den erforderlichen Komponenten für diesen Artikel finden Sie unter [Installieren und Konfigurieren von Azure PowerShell](powershell-install-configure.md).

## Aktivieren der Azure-Diagnoseerweiterung auf einer virtuellen Maschine mithilfe des Ressourcen-Manager-Bereitstellungsmodells

Die Diagnoseerweiterung kann im Zuge der Erstellung einer virtuellen Windows-Maschine mit dem Ressourcen-Manager-Bereitstellungsmodell durch Hinzufügen der Erweiterungskonfiguration zur Ressourcen-Manager-Vorlage aktiviert werden. Weitere Informationen finden Sie unter [Erstellen einer virtuellen Windows-Maschine mit Überwachung und Diagnose mithilfe von Azure-Ressourcen-Manager-Vorlagen](virtual-machines-extensions-diagnostics-windows-template.md).

Wenn Sie die Azure-Diagnoseerweiterung auf einer bereits vorhandenen virtuellen Maschine aktivieren möchten, die mithilfe des Ressourcen-Manager-Bereitstellungsmodells erstellt wurde, können Sie wie weiter unten gezeigt das PowerShell-Cmdlet [Set-AzureRMVMDiagnosticsExtension](https://msdn.microsoft.com/library/mt603499.aspx) verwenden.
...
## Aktivieren der Azure-Diagnoseerweiterung auf einer virtuellen Maschine mithilfe des klassischen Bereitstellungsmodells

Mit dem Cmdlet [Set-AzureVMDiagnosticsExtension](https://msdn.microsoft.com/library/mt589189.aspx) können Sie die Azure-Diagnoseerweiterung auf einer virtuellen Maschine aktivieren, die mit dem klassischen Bereitstellungsmodell erstellt wurde. Das folgende Beispiel zeigt, wie Sie mit dem klassischen Bereitstellungsmodell eine neue virtuelle Maschine mit aktivierter Azure-Diagnoseerweiterung erstellen.
...
Wenn Sie die Azure-Diagnoseerweiterung auf einer bereits vorhandenen (klassischen) virtuellen Maschine aktivieren möchten, rufen Sie mithilfe des Cmdlets [Get-AzureVM](https://msdn.microsoft.com/library/mt589152.aspx) zunächst die Konfiguration der virtuellen Maschine ab. Aktualisieren Sie anschließend mithilfe des Cmdlets [Set-AzureVMDiagnosticsExtension](https://msdn.microsoft.com/library/mt589189.aspx) die Konfiguration der virtuellen Maschine, sodass diese die Azure-Diagnoseerweiterung beinhaltet. Wenden Sie abschließend mithilfe von [Update-AzureVM](https://msdn.microsoft.com/library/mt589121.aspx) die aktualisierte Konfiguration auf die virtuelle Maschine an.
...
- Das Attribut *resourceID* des Elements **Metrics** muss mit der Ressourcen-ID für die virtuelle Maschine aktualisiert werden. 
	...
	- Wenn die Abonnement-ID für das Abonnement mit der VM also beispielsweise **11111111-1111-1111-1111-111111111111**, der Ressourcengruppenname **MyResourceGroup** und der VM-Name **MyWindowsVM** lautet, sieht der Wert für *resourceID* wie folgt aus:
		...
		```
	- Weitere Informationen zur Generierung von Metriken auf der Grundlage von Leistungsindikatoren und Metriken finden Sie in der [WAD-Metriktabelle im Speicher](virtual-machines-extensions-diagnostics-windows-template.md#wadmetrics-tables-in-storage).
