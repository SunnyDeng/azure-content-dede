...
Weitere Informationen zur Verwendung der einzelnen Stufen finden Sie unter [Gründe für die Verwendung neuer Dienstebenen][Gründe für die Verwendung neuer Dienstebenen]
...
7.  Prüfen Sie die Metriken über dem Zeitfenster, wenn der Dienst Probleme aufwies.
    ...
    ![Azure-Verwaltungsportal – SQL-Datenbankmetriken][Azure-Verwaltungsportal – SQL-Datenbankmetriken]

Wenn eine Metrik 80 % Auslastung für einen längeren Zeitraum überschreitet, kann dies auf ein Leistungsproblem hinweisen. Weitere Einzelheiten zur Datenbankauslastung finden Sie unter [Grundlegendes zur Ressourcenverwendung][Grundlegendes zur Ressourcenverwendung].

Wenn die Metriken darauf hindeuten, dass die Datenbank hochgradig ausgelastet ist, könnten Sie als ersten Schritt zur Problemminderung **die Datenbank auf eine höhere Dienststufe skalieren**. Um Probleme sofort zu beheben, können Sie die Registerkarte **Skalieren** der Datenbank verwenden, um sie zu skalieren. Dadurch erhöht sich Ihr Rechnungsbetrag.
![Azure-Verwaltungsportal – SQL-Datenbankskalierung][Azure-Verwaltungsportal – SQL-Datenbankskalierung]
...
3.  Wählen Sie die Metrik aus, für die Sie eine Warnung festlegen möchten, und wählen Sie **Regel hinzufügen**
    ![Azure-Verwaltungsportal – SQL-Warnung][Azure-Verwaltungsportal – SQL-Warnung]
4.  Geben Sie einen Namen und eine Beschreibung für die Warnung ein.
    ![Azure-Verwaltungsportal – Name und Beschreibung der SQL-Warnung][Azure-Verwaltungsportal – Name und Beschreibung der SQL-Warnung]
5.  Geben Sie den Wert an, der als Warnschwellenwert verwendet werden soll. Empfohlen werden **80%**, damit noch Reaktionszeit bleibt. Geben Sie unbedingt auch eine E-Mail-Adresse an, die Sie aktiv überwachen.
    ![Azure-Verwaltungsportal – Schwellenwert und E-Mail für SQL-Warnung][Azure-Verwaltungsportal – Schwellenwert und E-Mail für SQL-Warnung]

Weitere Informationen zum Diagnostizieren von SQL-Problemen finden Sie unter [Erweiterte Diagnose][Erweiterte Diagnose] am Ende dieses Dokuments.
...
Wenn Sie also häufig WHERE- oder JOIN-Anweisungen für bestimmte Spalten vornehmen, sollten Sie diese unbedingt indizieren. Weitere Informationen finden Sie im Abschnitt [Erstellen von Indizes][Erstellen von Indizes].
...
SQL Server Management Studio ist eine kostenlose Windows-Anwendung, die die fortgeschrittensten Funktionen bietet. Wenn Sie keinen Zugang zu einem Windows-Computer haben (wenn Sie z. B. einen Mac benutzen), können Sie einen virtuellen Computer in Azure wie in [Erstellen eines virtuellen Windows Server-Computers][Erstellen eines virtuellen Windows Server-Computers] beschrieben bereitstellen und dann eine Remoteverbindung mit ihm herstellen. Wenn Sie den virtuellen Computer hauptsächlich für die Ausführung von SQL Server Management Studio benötigen, sollte eine **Basic A0**-Instanz (früher „Sehr klein“) ausreichend sein.
...
![SQL Server Management Studio – dynamische Verwaltungssichten][SQL Server Management Studio – dynamische Verwaltungssichten]
...
![SQL-Datenbankverwaltungsportal – neue Abfrage][SQL-Datenbankverwaltungsportal – neue Abfrage]

Um eine der Abfragen unten auszuführen, fügen Sie sie in das Fenster ein, und klicken Sie auf **Ausführen**.

![SQL-Datenbankverwaltungsportal – Abfrage ausführen][SQL-Datenbankverwaltungsportal – Abfrage ausführen]

#### Erweiterte Metriken

Im Verwaltungsportal werden einige Metriken bereitgestellt, wenn die Stufen Basic, Standard und Premium verwendet werden. Wenn Sie aber die Stufen Web und Business verwenden, ist nur die Speichermetrik über das Portal verfügbar. Glücklicherweise lassen sich diese und andere Metriken leicht über die Verwaltungssicht **[sys.resource\_stats][sys.resource\_stats]** abrufen, unabhängig von der verwendeten Stufe. Betrachten Sie die folgende Abfrage:
...
![SQL Server Management Studio – Abfrageplan][SQL Server Management Studio – Abfrageplan]
...
![SQL-Datenbankverwaltungsportal – Abfrageplan][SQL-Datenbankverwaltungsportal – Abfrageplan]

## Weitere Informationen

-   [Azure SQL-Datenbankdokumentation][Azure SQL-Datenbankdokumentation]
-   [Azure SQL-Datenbankleistung und -skalierung][Azure SQL-Datenbankleistung und -skalierung]
-   [Problembehandlung in Azure SQL-Datenbank][Problembehandlung in Azure SQL-Datenbank]

### Indizieren

-   [Grundlagen zu Indizes][Grundlagen zu Indizes]
-   [Allgemeine Richtlinien zum Indexdesign][Allgemeine Richtlinien zum Indexdesign]
-   [Eindeutige Richtlinien zum Indexdesign][Eindeutige Richtlinien zum Indexdesign]
-   [Richtlinien für das Design von gruppierten Indizes][1]
-   [Primär- und Fremdschlüsselbeschränkungen][zusammengesetzter Schlüssel]
-   [Was kostet dieser Schlüssel?][Was kostet dieser Schlüssel?]
