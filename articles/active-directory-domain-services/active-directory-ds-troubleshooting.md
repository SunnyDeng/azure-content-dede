...
Dieser Artikel enthält Tipps zur Behandlung von Problemen, die beim Einrichten oder Verwalten von Azure AD-Domänendiensten auftreten können.
...
- **Synchronisierte Konten:** Falls die betroffenen Benutzerkonten über ein lokales Verzeichnis synchronisiert werden, müssen Sie sicherstellen, dass die folgenden Schritte eingehalten werden:
    - Sie haben die GA-Version (also die allgemein verfügbare Version) von Azure AD Connect bereitgestellt bzw. das Update darauf durchgeführt. Bei älteren Versionen werden keine Anmeldeinformationshashes synchronisiert, die für die NTLM/Kerberos-Authentifizierung erforderlich sind.
    ...
    - Je nach Größe Ihres Verzeichnisses kann es einige Zeit dauern, bis die Benutzerkonten und Anmeldeinformationshashes in den Azure AD-Domänendiensten verfügbar sind. Stellen Sie sicher, dass Sie lange genug warten, bevor Sie versuchen, die Authentifizierung erneut durchzuführen (abhängig von der Größe Ihres Verzeichnisses – für große Verzeichnisse zwischen einigen Stunden und ein bis zwei Tagen).
