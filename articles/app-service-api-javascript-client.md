...
Dieser Artikel zeigt, wie Sie einen HTML- und einen JavaScript-Client für eine [API-App](app-service-api-apps-why-best-platform.md) in[ Azure App Service](/documentation/services/app-service/) erstellen. In diesem Artikel wird vorausgesetzt, dass Sie bereits mit der Arbeit in HTML und JavaScript vertraut sind. Der Schwerpunkt dieses Artikels liegt auf dem JavaScript-Framework [AngularJS](https://angularjs.org/) zum Durchführen von REST-Aufrufen an die API-App.
...
Der vorliegende Artikel baut auf den genannten Artikeln auf und veranschaulicht, wie Ihre HTML-Anwendungen mithilfe von JavaScript auf Ihre Back-End-API-Apps zugreifen können.
...
1. Klicken Sie mit der rechten Maustaste auf die Datei *index.html*, und wählen Sie das Menüelement **Als Startseite festlegen** aus. Führen Sie anschließend ein Debugging für das Webprojekt aus, damit es in Ihrem Standardbrowser geöffnet wird.

	!["apiapp.json" und "Metadata" im Projektmappen-Explorer](./media/app-service-api-javascript-client/08-run-the-web-app.png)

1. Beachten Sie die handlebars-Vorlagen in der HTML-Ausgabe. Sie führen im nächsten Schritt eine Datenbindung für diese HTML-Elemente durch.
...
1. Fügen Sie den nachstehenden JavaScript-Code der Datei *index.html* hinzu, um die API aufzurufen und eine Datenbindung zwischen der HTML-Oberfläche und der API-Ausgabe herzustellen.
	...
	> **Hinweis**: Ihre Portnummer kann anders lauten. Wenn Ihr API-Projekt auf einem anderen Port ausgeführt wird, ändern Sie den obigen JavaScript-Code einfach so ab, dass er Ihrer Umgebung entspricht.

1. Stellen Sie sicher, dass das API-App-Projekt ebenfalls ausgeführt wird, sonst funktioniert der JavaScript-HTML-Code nicht ordnungsgemäß. Klicken Sie mit der rechten Maustaste auf die Projektmappe, und wählen Sie **Eigenschaften**. Legen Sie anschließend beide Webprojekte auf **Starten ohne Debugging** fest, und geben Sie an, dass das API-Projekt zuerst ausgeführt wird.

	!["apiapp.json" und "Metadata" im Projektmappen-Explorer](./media/app-service-api-javascript-client/10-run-both-web-projects.png)

1. Führen Sie ein Debugging des Projekts durch, und vergewissern Sie sich, dass der HTML/JavaScript-Client eine Verbindung mit dem API-App-Projekt herstellen und Daten aus dem API-App-Projekt anzeigen kann.
