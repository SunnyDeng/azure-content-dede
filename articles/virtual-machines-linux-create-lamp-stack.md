<properties pageTitle="Erstellen eines LAMP-Stapels mit Microsoft Azure" description="Erfahren Sie, wie Sie einen LAMP-Stack mit Microsoft Azure mithilfe von virtuellen Azure-Computern, auf denen Linux ausgeführt wird, erstellen." services="virtual-machines" documentationCenter="" authors="NingKuang" manager="timlt" editor="tysonn"/>
...
-	Vorbereiten des virtuellen Computers für den LAMP-Stapel.
-	Installieren von Software, die von Ihrem LAMP-Server auf dem virtuellen Computer benötigt wird.

Es wird vorausgesetzt, dass der Leser bereits über ein Azure-Abonnement verfügt.  Falls dies nicht der Fall ist, können Sie sich für eine kostenlose Testversion unter [http://azure.microsoft.com](http://azure.microsoft.com) registrieren. Wenn Sie über ein MSDN-Abonnement verfügen, finden Sie weitere Informationen unter [Microsoft Azure-Sonderpreis: MSDN-, MPN- und Bizspark-Vorteile](http://azure.microsoft.com/de-de/pricing/member-offers/msdn-benefits/?c=14-39). Weitere Informationen zu Azure finden Sie unter [Was ist Azure?](http://azure.microsoft.com/de-de/overview/what-is-azure/).

Zusätzlich zu diesem Thema werden im folgende Video detaillierte Informationen zur Verwendung des LAMP-Stapels unter Microsoft Azure vorgestellt:  

> [AZURE.VIDEO lamp-stack-on-azure-vms-with-guy-bowerman]

##Phase 1: Erstellen eines Images
In dieser Phase erstellen Sie den virtuellen Computer mithilfe eines Linux-Images in Azure.  

###Schritt 1: Generieren eines SSH-Authentifizierungsschlüssels
SSH ist ein wichtiges Tool für Systemadministratoren. Ist jedoch aus Sicherheitsgründen nicht immer ratsam, ein Kennwort zu verwenden, das ein Mensch festgelegt hat. Mithilfe eines starken SSH-Schlüssels können Sie den Remotezugriff öffnen, ohne sich Gedanken über Kennwörter machen zu müssen. Die Methode besteht aus Authentifizierung mit asymmetrischer Kryptografie. Der private Schlüssel des Benutzers ist der Schlüssel, der die Authentifizierung vornimmt. Sie können sogar das Konto des Benutzers sperren, um Kennwortauthentifizierung vollständig zu untersagen.    
...
![][2]
-	Markieren und kopieren Sie den öffentlichen Schlüssel unter **Key**, und speichern Sie ihn dann in einer Datei namens **publicKey.pem**. Klicken Sie nicht auf **Save public key**, weil sich das Dateiformat des gespeicherten öffentlichen Schlüssels vom gewünschten öffentlichen Schlüssel unterscheidet.
-	Klicken Sie auf **Save private key**, und speichern Sie den Schlüssel dann in einer Datei namens **privateKey.ppk**. 

###Schritt 2: Erstellen des Images im Azure Preview-Portal
Klicken Sie im [Azure Preview-Portal](https://portal.azure.com/) auf der Taskleiste auf **Neu**, und erstellen Sie ein Image anhand dieser Anleitungen. Wählen Sie dabei das Linux-Image basierend auf Ihren Anforderungen aus. Dieses Beispiel verwendet das Ubuntu-Image 14.04. 
...
Endpunkte in Azure bestehen aus einem Protokoll (TCP oder UDP) sowie aus einem öffentlichen und einem privaten Port. Der private Port ist der Port, an dem der Dienst auf dem virtuellen Computer lauscht. Der öffentliche Port ist der Port, an dem der Azure-Cloud-Dienst extern auf internetbasierten Datenverkehr lauscht. In einigen Fällen handelt es sich um die gleiche Portnummer.  
...
Klicken Sie im Azure Preview-Portal auf **Durchsuchen -> Virtueller Computer**, und klicken Sie dann auf den virtuellen Computer, den Sie erstellt haben.
...
-	Lauschadresse/Port des Apache-Webdiensts
	...
	-	Öffnen Sie "/etc/httpd/conf/httpd.conf", und suchen Sie dann nach der Zeichenfolge "Listen". Stellen Sie sicher, dass der Apache-Lauschport mit den privaten Port identisch ist, den Sie für Ihren Endpunkt konfiguriert haben. Der Standardport für Apache ist 80. Beispiel:  
...
Wenn Sie die Apache-Standardseite vom lokalen Host aus sehen können, besteht das Problem möglicherweise darin, dass der Port, an dem Apache lauscht, durch die Firewall blockiert ist. Sie können das Tool w3m verwenden, um die Apache-Webseite zu durchsuchen. Die folgenden Befehle installieren w3m und navigieren zur Apache-Standardseite:  
...
Während der Installation fragt MySQL Ihre Berechtigung zwei Mal an. Nachdem Sie auf beide Abfragen mit "Ja" geantwortet haben, wird MySQL installiert.
...
Sie werden aufgefordert, ein root-Kennwort festzulegen. Fahren Sie fort, wählen Sie "Y" aus, und folgen Sie dann den Anweisungen.  

CentOS automatisiert das Einrichten von MySQL und stellt eine Reihe von Fragen, die mit "Ja" oder "Nein" beantwortet werden müssen. Diese Fragen werden unten gezeigt. Wählen Sie "Y" oder "N" für Ihre Konfiguration aus. Nach Abschluss dieses Vorgangs wird MySQL erneut geladen, und die neuen Änderungen werden implementiert.  
...
Antworten Sie mit "y", um Softwarepakete herunterzuladen. Antworten Sie mit "y" auf "Importing GPG key 0xE8562897 "CentOS-5 Key (CentOS 5 Official Signing Key)". PHP wird installiert.   
...
Führen Sie im nächsten Schritt den Assistenten aus, und wählen Sie Ihr **MySQL root password** aus.
...
Sollten Sie sie auskommentieren (fügen Sie ein # am Anfang der Zeilen hinzu) und dann MySQL neu starten.  

Informationen zum Hinzufügen eines Endpunkts, um Remotezugriff zu erlauben, finden Sie in den Anleitungen in Phase 1: Erstellen eines Images, um einen neuen Endpunkt zu erstellen. Die TCP-Standardportnummer für Remotezugriff von MySQL ist 3306. Beispiel:
 
![][17]

###Bereitstellen von Webanwendungen auf dem Apache-Server
Nachdem Sie den LAMP-Stapel erfolgreich eingerichtet haben, können Sie Ihre vorhandene Webanwendung auf dem Apache-Webserver (Ihrem virtuellen Computer) bereitstellen. Dabei werden die gleichen Schritten wie bei der Bereitstellung einer vorhandenen Webanwendung auf einem physischen Server ausgeführt.
...
-	**Mögliche Ursache** 
	...
	Überprüfen Sie die Endpunkteinstellungen des öffentlichen und des privaten Ports, und stellen Sie sicher, dass der private Port mit dem Apache-Lauschport identisch ist. In Phase 1: "Erstellen eines Images", erhalten eine Anleitung zum Konfigurieren von Endpunkten für Ihren virtuellen Computer.</br>   
	...
	Wenn Sie die Apache-Standardseite vom lokalen Host aus sehen können, besteht das Problem wahrscheinlich darin, dass der Port, an dem Apache lauscht, durch die Firewall blockiert ist. Sie können das Tool w3m verwenden, um die Webseite zu durchsuchen. Die folgenden Befehle installieren w3m und navigieren zur Apache-Standardseite:  
...
-	**Lösung**
	...
	1.	Wenn der Apache-Lauschport nicht identisch mit den privaten Port des Endpunkts für Webdatenverkehr auf dem virtuellen Computer ist, müssen Sie den privaten Port des Endpunkts so ändern, dass er mit dem Apache-Lauschport identisch ist.
...
-	**Lösung**  
Sie müssen Berechtigungen aus dem root-Konto abrufen. Sie können den Besitzer dieses Ordners aus "root" in den Benutzernamen ändern, die Sie bei der Bereitstellung des Computers verwendet haben. Das folgende Beispiel zeigt den Kontonamen "azureuser":  
