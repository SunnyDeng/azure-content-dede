<properties pageTitle="Was ist das Azure WebJobs-SDK?" metaKeywords="Azure Azure WebJobs SDK, Azure storage, Azure queues, Azure tables" description="Eine Einf&uuml;hrung in Bezug auf das Azure WebJobs-SDK. Erl&auml;utert, was das SDK ist, typische Szenarien, f&uuml;r die es n&uuml;tzlich ist, und enth&auml;lt Codebeispiele." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Was ist das Azure WebJobs-SDK?" authors="tdykstra" solutions="" manager="wpickett" editor="jimbe" />
...
-   [Verwenden des WebJobs-SDKs außerhalb von WebJobs][Verwenden des WebJobs-SDKs außerhalb von WebJobs]
-   [Verwenden des WebJobs-SDKs zum Aufrufen einer beliebigen Funktion][Verwenden des WebJobs-SDKs zum Aufrufen einer beliebigen Funktion]
-   [Nächste Schritte][Nächste Schritte]

## <span id="overview"></span></a>Übersicht

[WebJobs][WebJobs] ist ein Feature von Azure-Websites, mit dem Sie ein Programm oder Skript im selben Kontext als eine Website ausführen können. Der Zweck des WebJobs-SDKs besteht darin, die Aufgabe für das Schreiben von Code zu vereinfachen, die als ein WebJob ausgeführt wird und mit Azure Storage-Warteschlangen, Blobs und Tabellen sowie Servicebus-Warteschlangen funktioniert.
...
## <span id="scenarios"></span></a>Szenarien
...
-   Andere lang ausgeführte Aufgaben, die Sie in einem Hintergrundthread ausführen möchten, wie das [Senden von E-Mails][Senden von E-Mails].

## <span id="code"></span></a> Codebeispiele
...
## <span id="workerrole"></span></a>Verwenden des WebJobs-SDKs außerhalb von WebJobs
...
Das Dashboard steht jedoch nur als eine Websiteerweiterung für eine Azure-Website zur Verfügung. Wenn Sie es außerhalb von einem WebJob ausführen und das Dashboard weiterhin verwenden möchten, können Sie eine Azure-Website so konfigurieren, dasselbe Speicherkonto zu verwenden, worauf Ihre WebJobs-SDK-Dashboardverbindungszeichenfolge verweist, und das WebJobs-Dashboard dieser Website zeigt dann Daten über die Funktionsausführung von Ihrem Programm an, das irgendwo anders ausgeführt wird. Sie können das Dashboard mithilfe der URL „<https://>*{Websitename}*.scm.azurewebsites.net/azurejobs/\#/functions“ abrufen. Weitere Informationen finden Sie unter [Getting a dashboard for local development with the WebJobs SDK][Getting a dashboard for local development with the WebJobs SDK] (Abrufen eines Dashboards für die lokale Entwicklung mit dem WebJobs-SDK) (in englischer Sprache). Beachten Sie jedoch, dass im Blogbeitrag ein alter Verbindungszeichenfolgenname gezeigt wird.

## <span id="nostorage"></span></a>Verwenden des WebJobs-SDKs zum Aufrufen einer beliebigen Funktion
...
-   Sie können Protokolle im Dashboard anzeigen, die mit dem bestimmten WebJob (Anwendungsprotokolle) oder mit dem bestimmten Funktionsaufruf verknüpft sind, die sie generiert haben (`TextWriter`-Parameterprotokolle).

-   Weitere Informationen finden Sie im Thema über das [Gewusst wie: Manuelle Aufrufen einer Funktion][Gewusst wie: Manuelle Aufrufen einer Funktion] und im Thema über das [Gewusst wie: Schreiben von Protokollen][Gewusst wie: Schreiben von Protokollen]

## <span id="nextsteps"></span></a>Nächste Schritte

Weitere Informationen über das WebJobs-SDK finden Sie im Thema über die [empfohlenen Azure WebJobs-Ressourcen][empfohlenen Azure WebJobs-Ressourcen].

  [Übersicht]: #overview
  [Szenarien]: #scenarios
  [Codebeispiele]: #code
  [Verwenden des WebJobs-SDKs außerhalb von WebJobs]: #workerrole
  [Verwenden des WebJobs-SDKs zum Aufrufen einer beliebigen Funktion]: #nostorage
  [Nächste Schritte]: #nextsteps
  [WebJobs]: /de-de/documentation/articles/web-sites-create-web-jobs/
  [Lernprogramm für die ersten Schritte über das WebJobs-SDK]: ../websites-dotnet-webjobs-sdk-get-started/
  [Senden von E-Mails]: https://github.com/victorhurdugaci/AzureWebJobsSamples/tree/master/SendEmailOnFailure
  ...
  [Gewusst wie: Manuelle Aufrufen einer Funktion]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#manual
  [Gewusst wie: Schreiben von Protokollen]: ../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs
  [empfohlenen Azure WebJobs-Ressourcen]: http://go.microsoft.com/fwlink/?linkid=390226
