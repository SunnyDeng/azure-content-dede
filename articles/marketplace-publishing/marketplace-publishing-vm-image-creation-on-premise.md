<properties
   pageTitle="Erstellen eines lokalen Images eines virtuellen Computers für Azure Marketplace | Microsoft Azure"
...
Es wird dringend empfohlen, Azure-VHDs direkt in der Cloud über RDP zu entwickeln. Wenn dies erforderlich ist, ist es jedoch auch möglich, eine VHD herunterzuladen und mithilfe lokaler Infrastruktur zu entwickeln.
...
Suchen nach der BLOB-URL im neuen [Microsoft Azure-Portal](https://ms.portal.azure.com):

1.	Wechseln Sie zu **Durchsuchen > VMs**, und wählen Sie den bereitgestellten virtuellen Computer.
...
### Herunterladen der VHD-Datei
Wenn Sie die BLOB-URL kennen, können Sie die virtuelle Festplatte über das [Azure-Verwaltungsportal](http://manage.windowsazure.com/) oder per PowerShell herunterladen.
> [AZURE.NOTE]Zum Zeitpunkt der Erstellung dieses Handbuchs ist die Funktionalität zum Herunterladen einer VHD-Datei noch nicht im neuen Microsoft Azure-Portal vorhanden.

**Herunterladen der Betriebssystem-VHD-Datei über das aktuelle [Azure-Verwaltungsportal](http://manage.windowsazure.com/)**
...
4.	Dadurch werden die Eigenschaften des Speicherkontos geöffnet. Wählen Sie die Registerkarte "Container" aus.
...
6.	Wählen Sie die VHD-Datei mit dem richtigen Betriebssystem aus, indem Sie die URL mit der gespeicherten vergleichen.
7.	Klicken Sie auf **Download**.

  ![Abbildung](media/marketplace-publishing-vm-image-creation-on-premise/img07.png)

### Herunterladen der VHD-Datei mithilfe von PowerShell
Zusätzlich zum Azure-Verwaltungsportal können Sie das Cmdlet [Save-AzureVhd](http://msdn.microsoft.com/library/dn495297.aspx) zum Herunterladen der Betriebssystem-VHD-Datei verwenden:
...
Wenn Sie die virtuellen Festplatten lokal vorbereitet haben, müssen Sie diese in ein Speicherkonto in Azure hochladen. Dieser Schritt findet statt, nachdem die VHD-Datei lokal erstellt wurde, jedoch bevor eine Zertifizierung für das VM-Image abgerufen wurde.

### Erstellen eines Speicherkontos und Containers
Wie bereits erwähnt, sollten Sie virtuelle Festplatten in ein Speicherkonto in einer Region in den USA hochladen. Alle virtuellen Festplatten für eine einzelne SKU sollten in einem einzelnen Container innerhalb eines einzelnen Speicherkontos platziert werden.
...
3.	Geben Sie den Namen des Speicherkontos ein, und wählen Sie den Speicherort aus.
...
7.	Wählen Sie auf dem Blatt **Container** die Option **Hinzufügen** aus, und geben Sie einen Containernamen und die Berechtigungen für den Container ein. Wählen Sie **Privat** als Containerberechtigung aus.
...
So erstellen Sie über das [Linux-Befehlszeilentool](../virtual-machines/command-line-tools/) ein Speicherkonto
...
## Hochladen von virtuellen Festplatten
Sobald das Speicherkonto und der Container erstellt wurden, können Sie Ihre vorbereiteten virtuellen Festplatten hochladen. Es können PowerShell, das Befehlszeilentool für Linux oder andere Azure-Speicherverwaltungstools verwendet werden.
...
### Hochladen einer virtuellen Festplatt über das Befehlszeilentool für Mac und Linux
