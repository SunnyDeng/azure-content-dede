<properties 
   pageTitle="Verwenden von BizTalk-Regeln" 
   description="In diesem Thema werden die Merkmale von BizTalk-Regeln behandelt und Anleitungen zu ihrer Verwendung vorgestellt." 
...
#Verwenden von BizTalk-Regeln 

Geschäftsregeln umfassen die Richtlinien und Entscheidungen, welche Geschäftsprozesse steuern. Diese Richtlinien können formal in Verfahrenshandbüchern, Verträgen oder Vereinbarungen festgelegt werden oder als Wissen oder Erfahrung der Mitarbeiter vorliegen. Diese Richtlinien sind dynamisch und verändern sich eventuell im Laufe der Zeit aufgrund von Änderungen an Geschäftsplänen, Vorschriften oder aus anderen Gründen.
...
Geschäftsregeln sind deklarative Anweisungen, die die Durchführung von Geschäftsprozessen steuern. Eine Regel besteht aus einer Bedingung und Aktionen. Die Bedingung wird ausgewertet, und wenn sie mit "true" ausgewertet wird, löst das Regelmodul eine oder mehrere Aktionen aus.
...
2.	Klicken Sie auf "Neu" -> "Marketplace" -> "API-Apps" - > "Biz Talk-Regeln" -> "Erstellen".
 
![Alt text][3]

3.	Geben Sie auf dem neuen Blatt, das geöffnet wird, Folgendes ein:  
...
##XML-Definition
1.	Wenn als Vokabulartyp XML ausgewählt wird, muss folgende Eingabe bereitgestellt werden:  
...
Nachdem die Richtlinie erstellt und getestet wurde, steht sie jetzt zum Gebrauch bereit. Benutzer können eine neue Logik-App über "Neu" -> "Logik-App" erstellen. Im Designer ist "BizTalk-Regeln" in der Galerie rechts verfügbar. Diese können jetzt auf die Designeroberfläche gezogen und dort abgelegt werden. Anschließend steht eine Option zur Verfügung, mit der die Ziel-Regel-API-App ausgewählt werden kann. Nachdem dies erfolgt ist, steht die Liste der verfügbaren zu konfigurierenden Richtlinien bereit. Wählen Sie eine bestimmte Richtlinie. Anschließend muss die für die Richtlinie erforderliche Eingabe bereitgestellt werden. Benutzer können die nachfolgende Ausgabe der Regel-API-App für weitere Entscheidungen nutzen.

## Verwenden von Regeln über APIs
Die Regel-API-App kann auch über einen umfangreichen Satz verfügbarer APIs aufgerufen werden. Auf diese Weise werden Benutzer nicht auf die einfache Verwendung von Datenflüssen eingeschränkt und können Regeln in einer beliebigen Anwendung durch REST-Aufrufe verwenden.
