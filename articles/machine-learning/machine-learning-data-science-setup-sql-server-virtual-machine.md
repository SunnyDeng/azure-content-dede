<properties 
	pageTitle="Einrichten eines virtuellen Azure-Computers mit SQL-Server für Data Science" 
...
# Einrichten eines virtuellen Azure-Computers mit SQL-Server für Data Science
...
Im Katalog der virtuellen Computer von Azure sind verschiedene Images zu finden, die Microsoft SQL Server enthalten. Wählen Sie ein VM-Image mit SQL Server aus, das Ihren Datenanforderungen entspricht. Folgende Images werden empfohlen:
...
1.  Melden Sie sich mit Ihrem Konto auf dem [Azure-Verwaltungsportal](http://manage.windowsazure.com/) an. 
	Wenn Sie kein Azure-Konto haben, sollten Sie die Seite [Kostenlose einmonatige Testversion](http://www.windowsazure.com/pricing/free-trial/) besuchen.
...
3.  Wählen Sie auf der Seite **Virtuellen Computer erstellen** ein Image eines virtuellen Computers aus, das SQL Server enthält und Ihren Datenanforderungen entspricht, und klicken Sie dann unten rechts auf den Pfeil für die nächste Seite. Die aktuellsten Informationen zu den unterstützten SQL Server-Images auf Azure finden Sie unter [Erste Schritte mit SQL Server auf Azure Virtual Machines](http://go.microsoft.com/fwlink/p/?LinkId=294720) in der Dokumentation [SQL Server auf Azure Virtual Machines](http://go.microsoft.com/fwlink/p/?LinkId=294719).

	![Select SQL Server VM][1]

4.  Geben Sie auf der ersten Seite zur **Konfiguration des virtuellen Computers** die folgenden Informationen ein:
...
5.  Konfigurieren Sie auf der zweiten Seite **Konfiguration des virtuellen Computers** die Ressourcen für Netzwerk, Speicher und Verfügbarkeit:
    ...
    -   Wählen Sie unter **Speicherkonto** ein vorhandenes oder ein automatisch generiertes Speicherkonto aus.
    -   Wählen Sie im Feld **VERFÜGBARKEITSGRUPPE** den Eintrag **(Keine)** aus.
...
7.  Die SQL Server-VM kann auch als IPython Notebook-Server dienen, der in einem späteren Schritt konfiguriert wird. 
	Fügen Sie einen neuen Endpunkt hinzu, um den Port für den IPython Notebook-Server anzugeben. Geben Sie einen Namen in der Spalte **NAME** ein, wählen Sie eine Portnummer für den öffentlichen Port und "9999" für den privaten Port aus.
...
##<a name="InstallIPython"></a>Installieren von IPython Notebook und anderer Tools
...
4.  Wählen Sie auf der Seite **Sicherheit**, unter **Serverauthentifizierung**, die Option **SQL Server- und Windows-Authentifizierungsmodus** aus, und klicken Sie dann auf **OK**.

    ![Select Authentication Mode][8]

5.  Klicken Sie im Dialogfeld von SQL Server Management Studio auf **OK**, um den erforderlichen Neustart von SQL Server zu bestätigen.
...
##<a name="Logins"></a>Erstellen von Anmeldenamen für SQL Server-Authentifizierung
...
1.  Wählen Sie im Azure-Verwaltungsportal (oder aus dem vorherigen Schritt) **VIRTUELLE COMPUTER** aus.
...
2.  Geben Sie im Dialogfeld **Verbindung mit Server herstellen** oder **Verbindung mit Datenbankmodul herstellen** im Feld **Servername** den DNS-Namen des virtuellen Computers (im vorherigen Schritt festgelegt) sowie die Portnummer eines öffentlichen Endpunkts im Format *DNSName,portnumber* ein, z. B. **TestVM.cloudapp.net,57500**.

3.  Wählen Sie im Feld **Authentifizierung** den Eintrag **SQL Server-Authentifizierung** aus.

4.  Geben Sie im Feld **Anmeldename** den Anmeldenamen ein, den Sie zuvor erstellt haben.

5.  Geben Sie im Feld **Kennwort** das Kennwort des Anmeldenamens ein, das Sie zuvor erstellt haben.

6.  Klicken Sie auf **Verbinden**.

##<a name="amlconnect"></a>Herstellen einer Verbindung von Azure Machine Learning zum Datenbankmodul

In späteren Phasen des Cloud Data Science-Ablaufs verwenden Sie [Azure Machine Learning Studio](https://studio.azureml.net) zum Erstellen und Bereitstellen der Machine Learning-Modelle. Verwenden Sie zum Erfassen von Daten aus den SQL Server-VM-Datenbanken direkt in Azure Machine Learning für das Training oder die Bewertung das Reader-Modul in einem neuen [Azure Machine Learning Studio](https://studio.azureml.net)-Experiment. In diesem Thema werden weitere Einzelheiten über Links im Cloud Data Science-Ablaufplan bereitgestellt. Eine Einführung finden Sie unter [Was ist Azure Machine Learning Studio??](machine-learning-what-is-ml-studio.md).
...
3.	Geben Sie im Textfeld **Database server name** die Zeichenfolge `tcp:<DNS-Name Ihres virtuellen Computers>,1433` ein.
...
* Informationen zum Verschieben von Daten nach HDInsight aus dem Azure-Blob-Speicher finden Sie unter [Erstellen und Laden von Daten in Hive-Tabellen aus Azure-Blob-Speicher](machine-learning-data-science-hive-tables.md).
...
* Informationen zum Extrahieren Stichproben aus Daten in HDInsight finden Sie unter [Extrahieren von Stichproben aus Daten in Azure HDInsight Hive-Tabellen](machine-learning-data-science-sample-data-hive.md).
