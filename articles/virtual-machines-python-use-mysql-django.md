...
-   Installieren eines [MySQL-Treibers](http://pypi.python.org/pypi/MySQL-python/1.2.3) für Python
...
1.  Folgen Sie den [hier](/de-de/manage/windows/tutorials/virtual-machine-from-gallery/) aufgeführten Anweisungen, um einen virtuellen Azure-Computer der *Windows Server 2008 R2*-Distribution zu erstellen.
...
3.  Fügen Sie einen weiteren Endpunkt hinzu: NAME = **web**, PROTOKOLL = **TCP**, ÖFFENTLICHER PORT = **80**, PRIVATER PORT = **80**.. Auf diese Weise werden externe Internetanforderungen zu dem Port umgeleitet, auf dem Django ausgeführt wird, nämlich *80*.

4.  Verwenden Sie Windows *Remotedesktop*, um sich von einem Remotestandort aus am neu erstellten virtuellen Azure-Computer anzumelden.

5.  Öffnen Sie den TCP-Port **80** auf dem virtuellen Computer:
...
6.  Installieren Sie die neueste Version von [MySQL Community Server](http://dev.mysql.com/downloads/mysql/) für Windows auf dem virtuellen Computer:
    ...
    **Hinweis**: Wir empfehlen, die Datenbank auf einer Datenpartition zu installieren, auf der das Betriebssystem nicht vorhanden ist.
    ...
    -   Führen Sie den [hier](http://dev.mysql.com/downloads/mysql/) verfügbaren Link für das *Windows (x86, 64-Bit)-MSI-Installationsprogramm* aus, und laden Sie das entsprechende MSI-Installationsprogramm vom nächstgelegenen Downloadserver herunter. Nützliche Tipps:
   ...
   -   Installieren Sie eine beispielhafte ["world"-Datenbank](http://dev.mysql.com/doc/index-other.html) (MyISAM-Version):
         -   Laden Sie [diese](http://downloads.mysql.com/docs/world.sql.zip) Zip-Datei auf den virtuellen Azure-Computer herunter.
...
1.  Bevor Sie mit der Entwicklung Ihrer Django-Anwendung beginnen können, müssen Sie zunächst Python+Django auf dem virtuellen Computer installieren. Sie können dafür den [Webplattform-Installer](http://www.microsoft.com/web/downloads/platform.aspx) verwenden. Nachdem Sie den Webplattform-Installer eingerichtet haben, können Sie mit dessen Hilfe nach "Django" suchen und das Django (Python)-Produkt installieren.
...
1.  Installieren Sie das MySQL Python-Clientpaket. [Über diesen Link](http://code.google.com/p/soemin/downloads/detail?name=MySQL-python-1.2.3.win32-py2.7.exe&can=2&q=) können Sie das Paket direkt installieren. Führen Sie nach Abschluss den folgenden Befehl aus, um die Installation zu überprüfen:
...
1.  Befolgen Sie die im Lernprogramm [Django Hello World](http://windowsazure.com/de-de/documentation/articles/virtual-machines-python-django-web-app-windows-server) aufgeführten Anweisungen, um eine einfache "Hello World"-Webanwendung in Django zu erstellen.
...
2.  Öffnen Sie in Ihrem lokalen Webbrowser **http://*yourVmName*.cloudapp.net** (wobei *yourVmName* für die Bezeichnung steht, die Sie bei der Erstellung des virtuellen Computers verwendet haben). In der Anzeige sollte "Hello ... !" erscheinen (siehe Screenshot weiter unten). Dies bedeutet, dass Django auf dem virtuellen Computer ausgeführt wird und ordnungsgemäß funktioniert.
