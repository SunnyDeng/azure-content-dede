...
oder die Professional Edition von Visual Studio, falls Sie diese bereits
besitzen.
...
In diesem Lernprogramm fügen Sie einer einfachen Anwendung zur Auflistung von Konferenzdaten mobile Funktionen hinzu. Die Anwendung wird dann über das [Starterprojekt][StarterProject] bereitgestellt. Der folgende Screenshot zeigt die ASP.NET-Sitzungen in der fertigen Anwendung, so wie sie im Browseremulator in den Internet Explorer 11 F12-Entwicklertools angezeigt werden.
...
-   Browseremulator in [Internet Explorer 11 F12-Entwicklertools][EmulatorIE11] (wird in allen Screenshots mobiler
...
Visual Studio-Projekte mit C\#-Quellcode sind zur Ergänzung
dieses Themas verfügbar:
...
- [Bereitstellen des Starterprojekts auf einer Microsoft Azure-Website][]
...
<h3><a name="bkmk_DeployStarterProject"></a>Bereitstellen des Starterprojekts auf einer Microsoft Azure-Website</h3>
...
3. 	Wählen Sie im Dialogfeld **Eigenschaften von Mvc5Mobile.zip**
die Schaltfläche **Blockierung aufheben**. (Durch die Aufhebung der Blockierung wird eine Sicherheitswarnung unterdrückt,
die angezeigt wird, wenn Sie versuchen, eine aus dem Internet heruntergeladene *.zip*-Datei
zu verwenden.)

4.	Klicken Sie mit der rechten Maustaste auf die Datei *Mvc5Mobile.zip*, und wählen Sie **Alle extrahieren** aus, um
die Datei zu entpacken. 

5. 	Öffnen Sie in Visual Studio die Datei *Mvc5Mobile.sln*.

6.  Klicken Sie im Projektmappen-Explorer mit der rechten Maustaste auf das Projekt, und klicken Sie auf **Veröffentlichen**.
...
13.	Das Dialogfeld "Web veröffentlichen" enthält jetzt die Websiteeinstellungen für Ihre neue Website. Klicken Sie auf **Veröffentlichen**.
	...
	Nachdem das Starterprojekt von Visual Studio auf der Microsoft Azure-Website veröffentlicht wurde, wird der Desktopbrowser mit der Livewebsite angezeigt.

14.		Starten Sie Ihren Emulator für mobile Browser, kopieren Sie die URL für die Konferenzanwendung (*<Präfix>*.azurewebsites.net) in den Emulator, und klicken Sie dann auf die obere rechte Schaltfläche, und wählen Sie **Browse by tag**. Wenn Sie Internet Explorer 11 als Standardbrowser verwenden, müssen Sie nur "F12" und dann "STRG+8" drücken und das Browserprofil anschließend in **Windows Phone** ändern. Das Bild unten zeigt die Ansicht *AllTags* im Hochformat (nach Auswahl von **Browse by tag**).

	![][AllTags]

>[WACOM.NOTE] Während Sie Ihre MVC 5-Anwendung in Visual Studio debuggen, können Sie Ihre Website erneut auf Microsoft Azure veröffentlichen, um die Livewebsite direkt in Ihrem mobilen Browser oder einem Browseremulator zu überprüfen.
...
beiden Anweisungen der Methode "RegisterBundles" nach der Änderung:
...
Die reduzierbare Navigationsleiste wird jetzt als gewöhnliche unsortierte Liste dargestellt. Klicken Sie erneut auf **Browse by tag** und dann auf **ASP.NET**. Die mobile Emulatoransicht ist nun nicht mehr auf Bildschirmgröße gezoomt, und Sie müssen zur Seite scrollen, um die rechte Seite der Tabelle zu sehen.
...
Bootstrap ist nicht spezifisch für ASP.NET MVC 5. Diese Features lassen sich in beliebigen Webanwendungen verwenden. Bootstrap ist jetzt jedoch in die ASP.NET MVC 5-Projektvorlage integriert, sodass Ihre MVC 5-Webanwendung Bootstrap standardmäßig nutzen kann.
...
Um zu beginnen kopieren Sie *Views\\Shared\\\_Layout.cshtml* nach *Views\\Shared\\\_Layout.Mobile.cshtml*. Öffnen Sie *\_Layout.Mobile.cshtml* und ändern Sie den Titel von **MVC5 Application** in **MVC5 Application (Mobil)**.

Entfernen Sie in jedem "Html.ActionLink"-Aufruf für die Navigationsleiste "Browse by" in jedem *ActionLink*. Der folgende Code zeigt das fertige "<ul class="nav navbar-nav">"-Tag der mobilen Layoutdatei.
...
dann "using System.Web.WebPages;" aus. Dadurch wird dem Namespace
"System.Web.WebPages", in dem die Typen
"DisplayModeProvider" und "DefaultDisplayMode" definiert werden, ein Verweis hinzugefügt.
...
Speichern Sie die Änderungen. Kopieren Sie die Datei *Views\\Shared\\\_Layout.Mobile.cshtml* nach *Views\\Shared\\\_Layout.iPhone.cshtml*. Öffnen Sie die neue Datei, und ändern Sie dann den Titel von "MVC5 Application (Mobil)" in "MVC5 Application (iPhone)".
...
Wählen Sie im mobilen Browser den Link **Speakers** aus. Da es keine mobile Ansicht (*AllSpeakers.Mobile.cshtml*) gibt, wird die Speakers-Standardansicht (*AllSpeakers.cshtml*) mit der mobilen Layoutansicht (*\_Layout.Mobile.cshtml*) gerendert. Wie im Folgenden gezeigt, ist der Titel **MVC5 Application (Mobile)** in *\_Layout.Mobile.cshtml* definiert.
...
Wenn "RequireConsistentDisplayMode" auf "true" gesetzt ist, wird das mobile Layout (*\_Layout.Mobile.cshtml*) nur für mobile Ansichten verwendet (d.h. wenn die Ansichtsdatei die Form ***ViewName**.Mobile.cshtml* aufweist). Sie können für "RequireConsistentDisplayMode" auch den Wert "true" festlegen, wenn Ihr mobiles Layout mit den nicht mobilen Ansichten nicht zufriedenstellend funktioniert. Der folgende Screenshot zeigt, wie die Seite *Speakers* gerendert wird, wenn für "RequireConsistentDisplayMode" der Wert "true" (ohne die Zeichenfolge "(Mobile)" oben in der Navigationsleiste) festgelegt wird.

![][AllSpeakers_LayoutMobileOverridden]

Sie können den konsistenten Anzeigemodus in einer spezifischen Ansicht deaktivieren, indem Sie in der Ansichtsdatei für "RequireConsistentDisplayMode" den Wert "false" festlegen. Durch das folgende Markup in der Datei *Views\\Home\\AllSpeakers.cshtml* wird "RequireConsistentDisplayMode" auf "false" gesetzt:
...
Sie können den Bootstrap-Stil [linked list group][] verwenden, um die Ansicht *Speakers* zu verbessern. Ersetzen Sie in *Views\Home\AllSpeakers.cshtml* die Inhalte der Razor-Datei durch den folgenden Code.
...
Durch das Attribut "class="list-group"" im Tag "<div>" wird der
Bootstrap-Listenstil und durch das Attribut "class="input-group-item""
...
Beachten Sie, dass sowohl auf das "<form>"-Tag als auch auf das "<input>"-Tag die Bootstrap-Stile angewendet sind. Das "<span>"-Element fügt ein Bootstrap-[glyphicon][] zur search box.
...
Außerdem müssen Sie "filter.js" in Ihre registrierten Bundle einschließen. Öffnen Sie *App\_Start\\BundleConfig.cs*, und ändern Sie die ersten Bundle. Ändern Sie die erste Anweisung "bundles.Add" (für das **jquery**-Bundle) folgendermaßen, sodass *Scripts\\filter.js* eingeschlossen wird: 

     bundles.Add(new ScriptBundle("~/bundles/jquery").Include(
                "~/Scripts/jquery-{version}.js",
                "~/Scripts/filter.js"));

Das **jquery**-Bundle wird bereits von der Standardansicht *\_Layout*
gerendert. Später können Sie denselben JavaScript-Code verwenden, um die
Filterfunktion auf andere Listenansichten anzuwenden.
...
So wie die *Speakers*-Ansicht ist auch die *Tags*-Ansicht gut lesbar, die Links
sind jedoch klein und können auf mobilen Geräten schlecht angetippt werden. Sie können die *Tags*-Ansicht auf die gleiche Weise reparieren wie die *Speakers*-Ansicht, indem Sie die oben beschriebenen Codeänderungen vornehmen, wobei Sie allerdings die folgende "Html.ActionLink"-Methodensyntax in *Views\\Home\\AllTags.cshtml* verwenden:
...
Sie können die *Dates*-Ansicht weiter verbessern, indem Sie die date-time-Werte
nach Datum organisieren. Zu diesem Zweck kann der Bootstrap-Stil
[panels][] verwendet werden. Ersetzen Sie
die Inhalte der Datei *Views\\Home\\AllDates.cshtml* durch den
folgenden Code:
...
jedes Datum in der Liste erstellt, und für die entsprechenden Links wird wie zuvor [linked list group][]
verwendet. Wenn Sie diesen Code ausführen, sieht der
mobile Browser etwa wie folgt aus:
...
-   Er wendet das [grid system][] auf das Layout an, damit die Sitzungselemente horizontal im Desktopbrowser und vertikal im mobilen Browser ("col-md-4"-Klasse) fließen.
-   Er verwendet die [responsive utilities][], um die Sitzungstags bei der Anzeige im mobilen Browser ("hidden-xs"-Klasse) auszublenden.
...
Tippen Sie im mobilen Browser auf die Schaltfläche **Tag**, und geben Sie dann "asp" in das
Suchfeld ein.
...
Wählen Sie den Link **Building a Single Page Application with ASP.NET and
AngularJS** aus.
...
Öffnen Sie *Views\\Home\\SessionByCode.cshtml*, und ersetzen Sie die Inhalte durch
das folgende Markup:
...
Aktualisieren Sie den mobilen Browser. Das folgende Bild zeigt die vorgenommenen
Codeänderungen:
...
In diesem Lernprogramm haben Sie erfahren, wie Sie ASP.NET MVC 5 zur Entwicklung
für die mobile Nutzung optimierter Webanwendungen verwenden. Das umfasst:
...
-   Erstellen von Ansichten speziell für bestimmte Browser wie den
    iPhone-Browser
...
<!-- Internal Links -->
[Bereitstellen des Starterprojekts auf einer Microsoft Azure-Website]: #bkmk_DeployStarterProject
...
[reaktionsfähige Dienstprogramme]: http://getbootstrap.com/css/#responsive-utilities
