...
Bevor Sie den Buildcomputer einrichten können, müssen Sie einen [Dienstprinzipal](../resource-group-create-service-principal-portal.md) erstellen. Der Build-Agent verwendet ihn, um sich bei Azure zu authentifizieren. Außerdem müssen Sie ein Zertifikat erstellen und es in einen Schlüsseltresor hochladen, da der Azure-Schlüsseltresor die Dienstprinzipal-Authentifizierung nicht unterstützt. Diese Schritte können auf einem beliebigen Computer ausgeführt werden. Der Entwicklungscomputer ist dabei eine gute Wahl.
...
4.	Melden Sie sich bei Azure PowerShell an.
    ...
    1. Führen Sie den Befehl `Login-AzureRmAccount` aus.
...
2.	Wechseln Sie in einer Admin-PowerShell-Befehlszeile innerhalb des extrahierten Archivs in das Verzeichnis `Powershell\Manual`.
...
1.	Wechseln Sie über eine PowerShell-Eingabeaufforderung mit Administratorrechten zu dem Verzeichnis, in das Sie `Manual.zip` extrahiert haben.

2.	Führen Sie das PowerShell-Skript `CreateAndUpload-Certificate.ps1` mit den folgenden Parametern aus.

    |Parameter|Wert|
    |---|---|
    |KeyVaultLocation|Ein beliebiger Wert. Muss dem Ort entsprechen, an dem Sie den Cluster erstellen möchten.|
    |CertificateSecretName|Ein beliebiger Wert.|
    |SecureCertificatePassword|Ein beliebiger Wert. Wird verwendet, wenn Sie das Zertifikat in Ihren Buildcomputer importieren.|
...
2.	Wenn Sie noch nicht über einen Computer verfügen, können Sie schnell einen virtuellen Azure-Computer mit vorinstalliertem Visual Studio 2015 bereitstellen. Gehen Sie dazu folgendermaßen vor:
    ...
    1.	Melden Sie sich am [Azure-Verwaltungsportal](https://portal.azure.com) an.
...
3. Geben Sie `PSModulePath` für den Namen und `%PROGRAMFILES%\WindowsPowerShell\Modules` für den Wert ein.
	...
	1. Ersetzen Sie `%PROGRAMFILES%` mit dem Wert der Umgebungsvariablen `PROGRAMFILES`.
...
1.	Importieren Sie das Zertifikat in Ihren Buildcomputer. Gehen Sie dazu folgendermaßen vor:
    ...
    2. Öffnen Sie ein PowerShell-Administratorkonto, und führen Sie die folgenden Befehle aus. Verwenden Sie dabei das Kennwort, das Sie zuvor an `CreateAndUpload-Certificate.ps1` übergeben haben.
...
3.	Erteilen Sie dem lokalen Dienstkonto die Berechtigung zur Verwendung Ihres Automation-Zertifikats.
    ...
    4.	Wählen Sie die Schaltfläche **Hinzufügen** aus, geben Sie **Lokaler Dienst** ein, und klicken Sie dann auf **Namen überprüfen**.
...
1.	Laden Sie „agent.zip“ herunter. Gehen Sie dazu folgendermaßen vor:
    ...
    1.	Melden Sie sich bei Ihrem Teamprojekt an. Beispiel: ****https://[your-VSTS-account-name].visualstudio.com**.

    2.	Wählen Sie das Zahnradsymbol in der rechten oberen Bildschirmecke aus.

    3.	Wählen Sie in der Systemsteuerung die Registerkarte **Agentpools** aus.

    4.	Wählen Sie **Download-Agent** aus, um die Datei „agent.zip“ herunterzuladen.

    5.	Kopieren Sie „agent.zip“ auf den zuvor erstellten Buildcomputer.

    6.	Entzippen Sie „agent.zip“ auf dem Buildcomputer unter `C:\agent` (oder an einem anderen Speicherort mit einem kurzen Pfad).
...
4. Vergewissern Sie sich, dass Ihr Build-Agent registriert wurde. Gehen Sie dazu folgendermaßen vor:
    ...
    1. Wechseln Sie wieder zu Ihrem Webbrowser. Dieser müsste die Seite `https://[your-VSTS-account-name].visualstudio.com/_admin/_AgentPool` anzeigen. Aktualisieren Sie die Seite.

    2. Wählen Sie den Agentpool aus, den Sie zuvor beim Ausführen von „ConfigureAgent.ps1“ ausgewählt haben.

    3. Vergewissern Sie sich, dass der Build-Agent in der Liste enthalten und die Statusanzeige grün ist. Wenn die Statusanzeige rot ist, kann der Build-Agent keine Verbindung mit Team Services herstellen.
...
1.	Extrahieren Sie [ServiceFabricContinuousIntegrationScripts.zip](https://gallery.technet.microsoft.com/Set-up-continuous-f8b251f6) in einem beliebigen Ordner dieses Computers. Kopieren Sie den Inhalt von `Powershell\Automation` in einen beliebigen Ordner der Quellcodeverwaltung.
...
1.	Erstellen Sie eine leere Builddefinition. Gehen Sie dazu folgendermaßen vor:
    ...
    4.	Wählen Sie die Option **Leer** und anschließend die Schaltfläche **Weiter** aus.

    5.  Vergewissern Sie sich, dass das richtige Repository und die richtige Verzweigung ausgewählt sind.

    6.  Wählen Sie die Agent-Warteschlange aus, bei der Sie Ihren Build-Agent registriert haben, und aktivieren Sie das Kontrollkästchen **Continuous Integration**.
...
Klicken Sie auf **Build in Warteschlange**, um einen Build zu starten. Buildvorgänge werden auch beim Pushvorgang/Einchecken ausgelöst.
...
1.	Erstellen Sie über das Azure-Verwaltungsportal oder über Azure PowerShell manuell einen Testcluster. Sie können das `ProvisionAndDeploy-SecureCluster.ps1`-Skript als Referenz verwenden.
