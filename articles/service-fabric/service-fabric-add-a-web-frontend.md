...
In diesem Tutorial erfahren Sie, wie Sie ein ASP.NET 5 Web API Front-End zu einer Anwendung hinzufügen, die bereits einen zustandsbehafteten Dienst umfasst. Lesen Sie vor diesem Lernprogramm [Erstellung Ihrer ersten Anwendung in Visual Studio](service-fabric-create-your-first-application-in-visual-studio.md), falls noch nicht geschehen.
...
2. Nach Abschluss der Bereitstellung startet Visual Studio den Browser zum Stamm des ASP.NET Web API-Diensts, z. B. http://localhost:33003 (die Portnummer wird nach dem Zufallsprinzip zugewiesen und kann auf Ihrem Computer anders lauten). Da die ASP.NET 5 Web-API-Vorlage kein Standardverhalten für den Stamm bereitstellt, wird eine Fehlermeldung im Browser angezeigt.

3. Fügen Sie dem Speicherort im Browser `/api/values` hinzu. Dies ruft die `Get`-Methode im ValuesController in der Web-API-Vorlage auf und gibt die Standardantwort von der Vorlage zurück – ein JSON-Array mit zwei Zeichenfolgen:
...
1. Klicken Sie im Projektmappen-Explorer mit der rechten Maustaste auf Ihre Lösung und dann auf **Hinzufügen > Neues Projekt**.

2. Wählen Sie im linken Navigationsbereich den Visual C#-Eintrag und dann die **Klassenbibliothek**-Vorlage. Stellen Sie sicher, dass Sie die .NET-Frameworkversion 4.5.1 verwenden.

  ![Erstellen eines Schnittstellenprojekts für Ihren zustandsbehafteten Dienst][vs-add-class-library-project]

3. Um eine Schnittstelle `ServiceProxy` zur Verfügung zu stellen, muss sie von der IService-Schnittstelle abgeleitet werden, die in einem der Service Fabric NuGet-Pakete enthalten ist. Klicken Sie zum Hinzufügen des Pakets mit der rechten Maustaste auf das neue Klassenbibliotheksprojekt, und wählen Sie **NuGet-Pakete verwalten** aus.

4. Aktivieren Sie das Kontrollkästchen **Vorversion einschließen**, suchen Sie nach dem Paket **Microsoft.ServiceFabric.Services**, und installieren Sie es.

  ![Abrufen des Services NuGet-Pakets][vs-services-nuget-package]

5. Erstellen Sie in der Klassenbibliothek eine Schnittstelle mit einer einzelnen Methode, `GetCountAsync`, und erweitern Sie die Schnittstelle aus IService.
...
3. Öffnen Sie im Ordner „Controllers“ die `ValuesController`-Klasse. Beachten Sie, dass derzeit nur die `Get`-Methode ein hartcodiertes Zeichenfolgenarray „Wert1“ und „Wert2“ zurückgeben, das dem vorherigen Browser-Verhalten entspricht. Ersetzen Sie diese Implementierung durch den folgenden Code:
  ...
  Durch die Partitionierung können Sie zustandsbehaftete Dienste skalieren, indem Sie ihren Status in verschiedene Buckets aufteilen, die auf einem von Ihnen definierten Schlüssel definieren, z. B. Kunden-ID oder PLZ. In unserer einfachen Anwendung verfügt der zustandsbehaftete Dienst nur über eine Partition, sodass der Schlüssel keine Rolle spielt – jede Taste, die Sie bereitstellen, führt zur gleichen Partition.

  Der Dienstname ist eine URI der Formular-Fabric: /&lt;application\_name&gt;/&lt;service\_name&gt;.

  Mit diesen zwei Angaben kann Service Fabric den Computer eindeutig identifizieren, an den Anforderungen gesendet werden sollen. Der `ServiceProxy` wird auch bei Ausfall des Computers angewendet, der die Partition des zustandsbehafteten Dienstes hostet, und ein anderer Computer muss dessen Stelle einnehmen. Diese Abstraktion vereinfacht das Schreiben des Clientcodes für den Umgang mit anderen Diensten beträchtlich.
