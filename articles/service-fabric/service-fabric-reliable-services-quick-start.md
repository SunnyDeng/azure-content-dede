<properties
   ...
   description="Service Fabric unterstützt sowohl zustandslose als auch zustandsbehaftete Dienste. Dieser Artikel führt Sie durch die Erstellung einer Service Fabric-Anwendung mit zustandslosen und zustandsbehafteten Diensten."
...
Die Plattform ruft diese Methode auf, wenn eine Instanz des Diensts platziert wurde und zur Ausführung bereit ist. Bei zustandslosen Diensten ist dies einfach der Fall, wenn die Dienstinstanz geöffnet wird. Ein Abbruchtoken koordiniert, wann die Dienstinstanz geschlossen werden muss. In Service Fabric kann dieser Öffnen-Schließen-Zyklus einer Dienstinstanz während der Gesamtlebensdauer des Diensts zahlreiche Male ausgeführt werden. Er tritt beispielsweise auf, wenn die Dienstinstanzen zum Zweck des Ressourcenausgleichs verschoben werden, bei Fehlern, während Anwendungs- oder System-Upgrades oder wenn die zugrunde liegende Hardware ausfällt. Diese Orchestrierung wird vom System so verwaltet, dass der Dienst hoch verfügbar bleibt und die Lasten ordnungsgemäß verteilt sind.

`RunAsync()` wird in der eigenen Aufgabe ausgeführt. In diesem Codeausschnitt verwenden wir direkt eine while-Schleife, da für die Arbeitsauslastung keine separate Aufgabe geplant werden muss. Zum Abbrechen der Arbeitsauslastung ist das Zusammenspiel verschiedener Aktionen erforderlich, die vom bereitgestellten Abbruchtoken orchestriert werden. Das System fährt erst fort, nachdem die Aufgabe beendet wurde (durch erfolgreichen Abschluss, Abbruch oder Fehler). Daher ist es *wichtig*, das Abbruchtoken zu berücksichtigen, jegliche Arbeiten abzuschließen und `RunAsync()` möglichst umgehend zu beenden, wenn vom System ein Abbruch angefordert wird.
...
Damit der Zählerwert selbst bei einer Verschiebung oder einem Neustart des Diensts hoch verfügbar und persistent ist, benötigen wir einen zustandsbehafteten Dienst.

Fügen Sie der Anwendung **HelloWorld** einen neuen Dienst hinzu, indem Sie mit der rechten Maustaste auf das Anwendungsprojekt klicken und die Option für einen neuen Fabric-Dienst wählen.
...
Ihre Anwendung sollte nun über zwei Dienste verfügen: den zustandslosen Dienst *HelloWorld* und den zustandsbehafteten Dienst *HelloWorldStateful*. Öffnen Sie **HelloWorldStateful.cs** in *HelloWorldStateful*:
...
**IReliableDictionary** repräsentiert ein Wörterbuch, das es Ihnen ermöglicht, den Zustand zuverlässig lokal im Dienst zu speichern. Es ist Teil der in Service Fabric integrierten [zuverlässigen Auflistungen](service-fabric-reliable-services-reliable-collections.md). Service Fabric sorgt dafür, dass der Dienst und der Zustand, den Sie in zuverlässigen Auflistungen speichern, hoch verfügbar sind. Mit Service Fabric und zuverlässigen Auflistungen können Sie Daten jetzt direkt in Ihrem Dienst zuverlässig speichern. Ein persistentes externes Speichern ist nicht erforderlich. Service Fabric erreicht dies durch Erstellen und Verwalten mehrerer *Replikate* des Diensts. Gleichzeitig wird eine API bereitgestellt, welche die Verwaltung dieser Replikate und ihrer Zustandsübergänge abstrahiert.
...
Zuverlässige Auflistungen können mit gewissen Einschränkungen beliebige .NET-Typen – einschließlich benutzerdefinierten Typen – speichern:
 ...
 2. Objekte werden zum Zweck einer hohen Verfügbarkeit repliziert, wenn Sie eine Transaktion auf eine zuverlässige Auflistung anwenden. In zuverlässigen Auflistungen gespeicherte Objekte bleiben im lokalen Speicher in Ihrem Dienst erhalten. Sie verfügen somit über einen lokalen Verweis auf das Objekt. Es ist wichtig, dass Sie lokale Instanzen dieser Objekte nur ändern, nachdem Sie ein Update für die zuverlässige Auflistung in einer Transaktion durchgeführt haben, da diese Änderungen nicht automatisch repliziert werden.
...
Zuverlässige Auflistungen beinhalten viele derselben Vorgänge wie ihre Pendants "System.Collections.Generic" und "System.Collections.Concurrent" – einschließlich LINQ. Vorgänge hinsichtlich zuverlässiger Auflistungen sind jedoch asynchron, da Schreibvorgänge auf zuverlässigen Auflistungen *repliziert* werden. Dabei wird der Vorgang zum Zweck einer hohen Verfügbarkeit an andere Replikate des Diensts auf verschiedenen Knoten gesendet.
