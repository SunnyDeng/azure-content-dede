...
Im nächsten Schritt wird die Dienstmanifestdatei so bearbeitet, dass sie folgende Informationen enthält: den Namen des Diensttyps. Diese "ID" verwendet Service Fabric zum Identifizieren eines Diensts - Der Befehl zum Starten der Anwendung (ExeHost) - Ein beliebiges Skript, das zum Einrichten/Konfigurieren der Anwendung (SetupEntryPoint) ausgeführt werden muss
...
Wenn Sie weitere Informationen benötigen, wie eine herkömmliche Service Fabric-App entwickelt wird, lesen Sie [dies](service-fabric-develop-your-service-index.md).
