...
Alle Anwendungen entwickeln sich im Laufe der Zeit weiter. Erfolgreiche Anwendungen entwickeln sich weiter, indem Sie für die Benutzer nützlich sind. Nicht erfolgreiche Anwendungen werden nicht weiterentwickelt und schließlich ausgesondert. Es stellt sich nur die Frage: Wie viel wissen Sie über Ihre heutigen Anforderungen und deren zukünftige Entwicklung? Wenn Sie beispielsweise eine Berichterstellungsanwendung für eine Abteilung erstellen und sicher sind, dass sie nur für Ihr Unternehmen bestimmt ist und die Berichte nur eine kurze Lebensdauer haben, wählen Sie einen anderen Ansatz als bei der Erstellung eines Diensts für die Bereitstellung von Videoinhalten für Millionen von Kunden. In einigen Fällen ist der treibende Faktor, dass ein Projekt als Machbarkeitsstudie in dem Wissen erstellt werden kann, dass der Entwurf der Anwendung später geändert werden kann. Es ist nicht sehr sinnvoll, zu viel Engineeringaufwand für etwas zu betreiben, was niemals verwendet wird. Dies ist der übliche Kompromiss, der beim Engineering häufig eingegangen wird. Wenn Unternehmen von der Erstellung für die Cloud sprechen, werden aber auch Wachstum und gute Auslastung erwartet. Das Problem hierbei ist, dass Wachstums- und Skalierungsanforderungen nicht vorhersagbar sind. Am liebsten würden wir schnell Prototypen erstellen können und gleichzeitig die Sicherheit haben, dass wir uns auf einem Weg zu zukünftigem Erfolg befinden. Dies entspricht dem schlanken Startup-Ansatz: Erstellen, Messen, Lernen, Wiederholen.

Während der Client/Server-Ära haben wir eher Anwendungen mit mehreren Ebenen erstellt, indem auf jeder Ebene spezielle Technologie verwendet wurde. Hierfür hat sich der Begriff „monolithische Anwendung“ etabliert. Die Schnittstellen befanden sich meist zwischen den Ebenen, und innerhalb einer Ebene wurde normalerweise ein Design mit engerer Kopplung zwischen den Komponenten genutzt. Die Entwickler haben für den Entwurf und die Einbindung also Klassen verwendet, die zu Bibliotheken kompiliert wurden, und diese über einige EXE- und DLL-Dateien verknüpft. Dieser monolithische Entwurfsansatz hat auch Vorteile. Es kann einfacher sein, schnellere Aufrufe zwischen Komponenten zu entwerfen und zu verwenden, da diese häufig per IPC durchgeführt werden. Außerdem wird jeweils ein einzelnes Produkt getestet, und dies ist in Bezug auf den Personalaufwand meist effizienter. Die Nachteile sind, dass die Anwendung innerhalb der mehrstufigen Ebenen eng verknüpft ist und dass Sie einzelne Komponenten nicht skalieren können. Wenn Sie Fixes oder Upgrades durchführen müssen, müssen Sie warten, bis andere Personen ihre Tests abgeschlossen haben. Hierbei ist es auch schwieriger, flexibel vorzugehen.

Microservices stellen die Antwort auf diese Nachteile dar, und sie sind enger an die oben beschriebenen geschäftlichen Anforderungen angelehnt. Aber auch sie haben sowohl Vor- als auch Nachteile. Die Vorteile von Microservices sind: Jeder Microservice kapselt normalerweise einfachere Geschäftsfunktionen, und sie können einzeln hoch- und herunterskaliert, getestet, bereitgestellt und verwaltet werden. Ein wichtiger Vorteil eines Microservice-Ansatzes ist, dass Teams stärker durch Geschäftsszenarios als durch Technologie beeinflusst werden, deren Nutzung durch den mehrstufigen Ansatz gefördert wird. In der Praxis bedeutet dies, dass kleinere Teams einen Microservice basierend auf einem Kundenszenario entwickeln, indem sie alle gewählten Technologien verwenden. Anders ausgedrückt: Das Unternehmen muss die Technologie nicht standardisieren, um den monolithischen Ansatz weiterzuverfolgen. Einzelne Teams, die im Besitz von Diensten sind, können zudem die Vorgehensweise wählen, die aufgrund der Expertise im Team sinnvoll ist oder die am besten zur Beseitigung des Problems geeignet ist, das mit dem Dienst gelöst werden soll. In der Praxis ist es natürlich vorzuziehen, über eine Gruppe empfohlener Technologiekomponenten zu verfügen, z. B. einen bestimmten NoSQL-Speicher oder ein Webanwendungs-Framework. Die Nachteile von Microservices sind die Verwaltung der höheren Anzahl von separaten Entitäten, die komplexeren Bereitstellungen, die Versionsverwaltung, mehr Netzwerkdatenverkehr zwischen den Microservices und die entsprechende Netzwerklatenz. Wenn eine hohe Zahl von sehr präzisen Diensten mit vielen Einzelaufrufen vorhanden ist, kann dies in Bezug auf die Leistung schnell zu einem Alptraum werden. Und ohne Tools zum Anzeigen dieser Abhängigkeiten ist es nicht einfach, das gesamte System zu „sehen“. Letztendlich funktioniert der Microservice-Ansatz aufgrund von Standards. Es wird vereinbart, wie kommuniziert wird und dass nur die Dinge toleriert werden, die Sie von einem Dienst benötigen, anstatt starre Verträge zu nutzen. Es ist wichtig, diese Kontakte im Voraus im Design festzulegen, da die Dienste unabhängig voneinander aktualisiert werden. Eine weitere Beschreibung, die für das Entwerfen mit einem Microservice-Ansatz gilt, ist „SOA mit feiner Abstufung“ (fine-grained SOA).
...
Das Entwerfen eines Microservice-Ansatzes ist kein Patentrezept für alle Projekte. Dieser Ansatz ist jedoch enger an die oben beschriebenen geschäftlichen Ziele angelehnt. Die anfängliche Verwendung eines monolithischen Ansatzes kann auch akzeptabel sein, wenn Sie wissen, dass Sie später die Gelegenheit haben, den Code bei Bedarf zu überarbeiten und zu einem Microservice-Design zu wechseln. Normalerweise beginnen Sie mit einer monolithischen App und teilen diese dann nach und nach in Phasen auf. Sie beginnen hierbei mit den Funktionsbereichen, die besser skalierbar oder flexibler werden sollen.
...
Es gibt unterschiedliche Definitionen von Microservices, und im Internet finden Sie viele gute Ressourcen mit eigener Sichtweise und Definition. Die meisten der folgenden Merkmale von Microservices werden aber immer genannt:

- Sie eignen sich zum Kapseln eines Kunden- oder Geschäftsszenarios Welches Problem möchten Sie lösen?
- Sie werden von einem kleinen Engineering-Team entwickelt
- Sie können mit einer beliebigen Programmiersprache geschrieben und mit einem beliebigen Framework verwendet werden
- Sie bestehen aus Code (optional: werden einzeln versioniert, bereitgestellt und skaliert)
- Sie interagieren mit anderen Microservices über gut definierte Schnittstellen und Protokolle
- Sie haben eindeutige Namen (URLs), die zum Auflösen des Standorts verwendet werden können
- Sie bleiben beim Auftreten von Fehlern konsistent und verfügbar
...
Nachdem Sie sich für eine Technologie entschieden haben, geht es mit der Betriebs- oder Lebenszyklusverwaltung und der Skalierung des Diensts weiter.
