...
- [Geräteidentitätsregistrierung](#identityregistry). Dieser Abschnitt beschreibt, welche Informationen in jede IoT Hub-Geräteidentitätsregistrierung gespeichert werden, und wie Sie auf die Informationen zugreifen und sie ändern können.
...
Azure IoT Hub ist ein mehrinstanzenfähiger Dienst, der Funktionen für zahlreiche Akteure bereitstellt. Das nachstehende Diagramm zeigt die verschiedenen Endpunkte, die von Azure IoT Hub verfügbar gemacht werden.

![IoT Hub-Endpunkte][img-endpoints]

Es folgt eine Beschreibung der Endpunkte:

* **Ressourcenanbieter**: Der IoT Hub-Ressourcenanbieter legt eine [Azure-Ressourcen-Manager][lnk-arm]-Schnittstelle offen, mit der Azure-Abonnementbesitzer IoT Hubs erstellen, IoT Hub-Eigenschaften aktualisieren und IoT Hubs löschen können. IoT Hub-Eigenschaften steuern Sicherheitsrichtlinien auf Hubebene (im Gegensatz zur Zugriffssteuerung auf Geräteebene, der [Zugriffssteuerung](#accesscontrol)) und funktionale Optionen für das Messaging zwischen Cloud und Gerät (Cloud-to-Device, C2D) sowie Gerät und Cloud (Device-to-Cloud, D2C). Der Ressourcenanbieter ermöglicht außerdem das [Exportieren von Geräteidentitäten](#importexport).
* **Geräteidentitätsverwaltung**: Jeder IoT Hub legt einen Satz an HTTP-REST-Endpunkten zum Verwalten von Geräteidentitäten offen (d. h. zum Erstellen, Abrufen, Aktualisieren und Löschen). Geräteidentitäten werden zur Geräteauthentifizierung und für die Zugriffssteuerung eingesetzt. Weitere Informationen finden Sie unter [Geräteidentitätsregistrierung](#identityregistry).
* **Geräteendpunkte**: Für jedes Gerät, das in der Geräteidentitätsregistrierung bereitgestellt wird, legt IoT Hub einen Satz an Endpunkten offen, der zur Kommunikation mit und von diesem Gerät verwendet wird. Diese Endpunkte werden aktuell sowohl in HTTP als auch in [AMQP][lnk-amqp] verfügbar gemacht:
    - *Senden von D2C-Nachrichten*. Dieser Endpunkt wird verwendet, um Nachrichten von einem Gerät an die Cloud zu senden. Weitere Informationen erhalten Sie unter [Messaging zwischen Gerät und Cloud](#d2c).
    - *Empfangen von C2D-Nachrichten*. Dieser Endpunkt wird vom Gerät verwendet, um Nachrichten aus der Cloud zu empfangen. Weitere Informationen erhalten Sie unter [Messaging zwischen Cloud und Gerät](#c2d).
* **Dienstendpunkte**: Jeder IoT Hub legt außerdem einen Satz an Endpunkten offen, die von Ihrem Anwendungs-Back-End (*Dienst*) zur Kommunikation mit Ihren Geräten verwendet werden. Diese Endpunkte werden aktuell nur bei Verwendung des [AMQP][lnk-amqp]-Protokolls verfügbar gemacht.
    ...
    - *Senden von C2D-Nachrichten und Empfangen von Übermittlungsbestätigungen*. Diese Endpunkte ermöglichen Ihrem Anwendungs-Back-End das Senden von zuverlässigen C2D-Nachrichten sowie das Empfangen zugehöriger Übermittlungs- oder Ablaufbestätigungen. Weitere Informationen erhalten Sie unter [Messaging zwischen Cloud und Gerät](#c2d).
...
Wenn Sie das [Azure Service Bus-SDK für .NET](https://www.nuget.org/packages/WindowsAzure.ServiceBus) oder den [Event Hubs-Ereignisprozessorhost] verwenden, können Sie eine beliebige IoT Hub-Verbindungszeichenfolge mit den richtigen Berechtigungen verwenden und anschließend `messages/events` als Ereignishubname nutzen.

Bei der Verwendung von SDKs (oder Produktintegrationen), die nicht IoT Hub-fähig sind, müssen Sie einen Event Hubs-kompatiblen Endpunkt und den Ereignishubnamen aus den IoT Hub-Einstellungen im [Azure-Vorschauportal] abrufen:
...
> [AZURE.NOTE]Gelegentlich ist für das SDK ein **Hostname** oder **Namespace** erforderlich. Entfernen Sie in diesem Fall das Schema aus dem **Event Hub-kompatiblen Endpunkt**. Wenn es sich bei Ihrem Event Hub-kompatiblen Endpunkt beispielsweise um `sb://iothub-ns-myiothub-1234.servicebus.windows.net/` handelt, lautet der **Hostname** `iothub-ns-myiothub-1234.servicebus.windows.net`, und der **Namespace** lautet `iothub-ns-myiothub-1234`.

Sie können in diesem Fall eine beliebige gemeinsam genutzte Sicherheitsrichtlinie mit **ServiceConnect**-Berechtigungen zur Verbindungsherstellung mit dem obigen Event Hub verwenden.

Wenn Sie eine Event Hub-Verbindungszeichenfolge mit den obigen Informationen erstellen müssen, können Sie das folgende Muster verwenden:

        Endpoint={Event Hub-compatible endpoint};SharedAccessKeyName={iot hub policy name};SharedAccessKey={iot hub policy key}


Es folgt eine Liste der SDKs und Integrationen, die mit IoT Hub verwendet werden können:
...
## Geräteidentitätsregistrierung <a id="identityregistry"></a>
Jeder IoT Hub verfügt über eine Geräteidentitätsregistrierung, die zum Erstellen von gerätebasierten Ressourcen im Dienst verwendet wird. Hierzu zählt beispielsweise eine Warteschlange mit C2D-Nachrichten. Außerdem wird der Zugriff auf geräteseitige Endpunkte ermöglicht, wie im Abschnitt [Zugriffssteuerung](#accesscontrol) beschrieben.

Auf höherer Ebene ist die Geräteidentitätsregistrierung eine RESTful-Sammlung von Geräteidentitätsressourcen. In den folgenden Abschnitten werden die Eigenschaften von Geräteidentitätsressourcen sowie die Vorgänge beschrieben, die über die Registrierung für Identitäten ausgeführt werden können.
...
| auth.symkey | optional | Ein zusammengesetztes Objekt, das einen primären und sekundäre Schlüssel enthält, die im Base64-Format gespeichert sind. |
| status | erforderlich | Kann **Aktiviert** oder **Deaktiviert** lauten. Sofern **Aktiviert**, kann das Gerät eine Verbindung herstellen. Lautet die Einstellung **Deaktiviert**, kann dieses Gerät auf keinen geräteseitigen Endpunkt zugreifen. |
...
Die Azure IoT Hub-Geräteidentitätsregistrierung macht die folgenden Vorgänge verfügbar:
...
Sie können Geräteidentitäten per Massenvorgang aus einer IoT Hub-Identitätsregistrierung exportieren. Dieser Export kann über asynchrone Vorgänge auf dem Azure IoT Hub-Ressourcenanbieterendpunkt ([Endpunkte](#endpoints)) ausgeführt werden.

Nachfolgend werden die möglichen Vorgänge für Exportaufträge aufgeführt:
...
| outputBlobContainerURI | erforderlich für alle Aufträge | Blob-SAS-URI (Shared Access Signature) mit Schreibzugriff auf einen Blobcontainer (siehe [Erstellen und Verwenden eines SAS mit dem Blobdienst][lnk-createuse-sas] und [Erstellen eines SAS in Java][lnk-sas-java]). Wird zur Ausgabe des Auftragsstatus und der Ergebnisse verwendet. |
| includeKeysInExport | optional | Bei Festlegung auf **true** werden Schlüssel in der Exportausgabe eingeschlossen. Andernfalls werden Schlüssel als "null" exportiert. Standard: **false**. |
...
Exportaufträge verwenden als Parameter einen Blob-SAS-URI (Shared Access Signature), der Schreibzugriff auf den Blobcontainer gewährt. Dies dient zur Ausgabe der Ergebnisse des Vorgangs.

Die Ausgabeergebnisse werden in den angegebenen Blobcontainer in eine Datei namens `job_{job_id}_devices.txt` geschrieben. Diese Datei enthält die Geräteidentitäten, serialisiert als JSON gemäß Festlegung in den [Geräteidentitätseigenschaften](#deviceproperties). Die Sicherheitsdaten werden auf **null** gesetzt, wenn **includeKeysInExport** auf **false** festgelegt ist.
...
Azure IoT Hub verwendet den folgenden Satz an *Berechtigungen*, um Zugriff auf den Endpunkt für jeden Hub zu gewähren. Berechtigungen beschränkten den Zugriff auf einen IoT Hub basierend auf der Funktionalität.

* **RegistryRead**. Diese Berechtigung gewährt Lesezugriff auf die Geräteidentitätsregistrierung. Weitere Informationen finden Sie unter [Geräteidentitätsregistrierung](#identityregistry).
* **RegistryReadWrite**. Diese Berechtigung gewährt Lese- und Schreibzugriff auf die Geräteidentitätsregistrierung. Weitere Informationen finden Sie unter [Geräteidentitätsregistrierung](#identityregistry).
...
* **Sicherheitsanmeldeinformationen auf Gerätebasis**. Jeder IoT Hub enthält eine [Geräteidentitätsregistrierung](#identityregistry). Sie können für jedes Gerät in dieser Registrierung Sicherheitsanmeldeinformationen konfigurieren, die **DeviceConnect**-Berechtigungen erteilen, deren Gültigkeitsbereich auf die entsprechenden Geräteendpunkte festgelegt ist.

**Beispiel**. Eine IoT Hub-Lösung umfasst üblicherweise eine Komponente zur Geräteverwaltung mit Verwendung der Richtlinie *registryReadWrite* sowie eine Ereignisprozessorkomponente und eine Laufzeit-Geschäftslogikkomponente für Geräte, die beide die Richtlinie *service* verwenden. Einzelne Geräte stellen unter Verwendung von Anmeldeinformationen eine Verbindung her, die in der Hubidentitätsregistrierung gespeichert sind.

Richtlinien zu IoT Hub-Sicherheitsthemen finden Sie im Abschnitt zur Sicherheit unter [ Azure IoT Hub – Anleitung][lnk-guidance-security].

### Authentifizierung
Azure IoT Hub gewährt Zugriff auf Endpunkte, indem ein Token zur Verifizierung mit gemeinsam genutzten Zugriffsrichtlinien und Sicherheitsanmeldeinformationen für die Geräteidentität verglichen wird. Sicherheitsanmeldeinformationen, beispielsweise symmetrische Schlüssel, werden niemals über eine physische Verbindung gesendet.
...
| Wert | Beschreibung |
| ----- | ----------- |
| {signature} | Eine HMAC-SHA256-Signatur der Zeichenfolge: `{URL-encoded-resourceURI} + "\n" + expiry` |
...
Bei Verwendung von [AMQP][lnk-amqp] unterstützt Azure IoT Hub [SASL PLAIN][lnk-sasl-plain] und die auf [AMQP-Ansprüchen basierte Sicherheit][lnk-cbs].
...
Bei SASL PLAIN kann der Benutzername * `{policyName}&commat;sas.root.{iothubName}` für ein Token auf Hubebene, * `{deviceId}` für ein gerätespezifisches Token lauten.
...
## Messaging
Azure IoT Hub stellt Grundtypen für das Messaging bereit, die für die Kommunikation zwischen einem Anwendungs-Back-End (*Dienst* oder *Cloud*) und Geräten sowie umgekehrt eingesetzt wird. Diese Funktionalität wird als [Gerät-an-Cloud](#d2c)(Device-to-Cloud, D2C) und [Cloud-an-Gerät](#c2d) (Cloud-to-Device, C2D) bezeichnet.
...
| -------- | ----------- |
| MessageId | Eine vom Benutzer festgelegte Kennung für die Nachricht, wird üblicherweise für Anforderung-Antwort-Muster verwendet. Format: Eine Zeichenfolge mit Berücksichtigung von Klein-/Großschreibung (bis zu 128 Zeichen lang), die aus alphanumerischen ASCII-7-Bit-Zeichen + `{'-', ':',’.', '+', '%', '_', '#', '*', '?', '!', '(', ')', ',', '=', '@', ';', '$', '''}` besteht. |
...
| Ack | Wird in C2D-Nachrichten verwendet, um IoT Hub anzuweisen, als Ergebnis der Nachrichtenverarbeitung durch das Gerät Feedback zu generieren. Mögliche Werte: **none** (Standardeinstellung): es wird kein Feedback generiert, **positive**: Feedbacknachricht generieren, wenn die Nachricht abgeschlossen wurde, **negative**: Feedbacknachricht generieren, wenn die Nachricht ohne Abschluss der Nachricht durch das Gerät abgelaufen ist (max. Zustellungsanzahl wurde erreicht), **full**: Feedback wird sowohl bei erfolgreicher als auch nicht erfolgreicher Nachrichtenverarbeitung generiert. Weitere Informationen finden Sie unter [Nachrichtenfeedback](#feedback). |
| ConnectionDeviceId | Wird von IoT Hub in D2C-Nachrichten festgelegt. Diese Eigenschaft enthält die Geräte-ID des Geräts, das die Nachricht sendet. |
...
* **C2D-Muster**. HTTP/1 verfügt über keine effiziente Methode zum Implementieren von Serverpushvorgängen. Wenn Sie HTTP/1 verwenden, rufen Geräte C2D-Nachrichten deshalb bei Azure IoT Hub ab. Dies ist sowohl für das Gerät als auch für IoT Hub ineffizient. Die aktuellen Richtlinien bei Verwendung von HTTP/1 sehen vor, für jedes Gerät ein Abrufintervall von weniger als 1 pro 25 Minuten festzulegen. AMQP dagegen unterstützt Serverpushvorgänge beim Empfang von C2D-Nachrichten und ermöglicht es, Nachrichten von IoT Hub sofort an das Gerät zu pushen. Wenn die Übermittlungslatenz eine wichtige Rolle spielt, ist daher die Verwendung des AMQP-Protokolls die bevorzugte Option. Für Geräte, die nur selten verbunden werden, kann durchaus HTTP/1 verwendet werden.
* **Bereichsgateway**. Aufgrund der Einschränkungen von HTTP/1 in Bezug auf Serverpushvorgänge ist es für eine Verwendung in [Szenarien mit Bereichsgateway][lnk-azure-gateway-guidance] nicht geeignet.
...
* **Netzwerkausnahme**. AMQP lauscht standardmäßig auf Port 5672. Dies kann Probleme in Netzwerken verursachen, die für Nicht-HTTP-Protokolle geschlossen sind. Beachten Sie, dass es sich um eine vorübergehende Einschränkung handelt, da für IoT Hub eine Implementierung von AMQP über WebSockets vorgesehen ist.
...
Azure IoT Hub implementiert das D2C-Messaging ähnlich wie [Event Hubs][lnk-event-hubs], wobei das D2C-Messaging von IoT Hub eher mit *Ereignissen* in Event Hubs als mit [Service Bus][lnk-servicebus]-*Nachrichten* vergleichbar ist.
...
* Wie Event Hubs-Ereignisse können D2C-Nachrichten maximal 256 KB umfassen und können in Batches gruppiert werden, um den Sendevorgang zu optimieren. Batches können maximal 256 KB groß sein und maximal 500 Nachrichten enthalten.
...
Weitere Informationen zur besten Verarbeitungsmethode für diese Art von Nachrichten finden Sie im Abschnitt zur D2C-Verarbeitung unter [Azure IoT Hub – Anleitung][lnk-guidance-d2c-processing].
...
* **Anzahl von Partitionen**. Diese Eigenschaft kann bei der Erstellung festgelegt werden und definiert die Anzahl von Partitionen für die D2C-Ereigniserfassung.
* **Aufbewahrungsdauer**. Diese Eigenschaft legt die Aufbewahrungszeit für D2C-Nachrichten fest. Der Standardwert lautet 1, er kann jedoch bis auf 7 erhöht werden.

Analog zu Event Hubs ermöglicht IoT Hub die Verwaltung von Consumergruppen am empfangenden D2C-Endpunkt.

Alle oben genannten Eigenschaften können sowohl über das [Azure-Verwaltungsportal][lnk-management-portal] als auch programmgesteuert über [Azure IoT Hub-Ressourcenanbieter-APIs][lnk-resource-provider-apis] geändert werden.
...
Die ersten zwei Eigenschaften enthalten die Werte für **deviceId** und **generationId** des ursprünglichen Geräts, wie beschrieben unter [Geräteidentitätseigenschaften](#deviceproperties). Die Eigenschaft **ConnectionAuthMethod** enthält ein serialisiertes JSON-Objekt mit den folgenden Eigenschaften:
...
Unter [Erste Schritte mit Azure IoT Hub-C2D-Nachrichten][lnk-getstarted-c2d-tutorial] finden Sie ein Tutorial zu C2D-Nachrichten, unter [IoT Hub-APIs und -SDKs][lnk-apis-sdks] finden Sie Informationen dazu, wie verschiedene APIs und SDKs C2D-Funktionalität offen legen.

> [AZURE.NOTE]Typischerweise sollte eine C2D-Nachrichten immer dann abgeschlossen werden, wenn der Verlust der Nachricht keine Auswirkung auf die Anwendungslogik hat. Dies kann in vielen verschiedenen Szenarien zutreffen, beispielsweise dann, wenn der Nachrichteninhalt erfolgreich im lokalen Speicher abgelegt oder ein Vorgang erfolgreich ausgeführt wurde, oder wenn die Nachricht kurzlebige Informationen enthält, deren Verlust sich nicht auf die Funktionalität der Anwendung auswirkt. Bei Tasks mit langer Ausführungsdauer ist es gängige Praxis, die C2D-Nachricht abzuschließen, nachdem die Taskbeschreibung in den lokalen Speicher geschrieben wurde. Anschließend wird das Anwendungs-Back-End mit einer oder mehreren D2C-Nachrichten in verschiedenen Phasen über den Taskfortschritt benachrichtigt.

#### Gültigkeitsdauer <a id="ttl"></a>
Jede C2D-Nachricht verfügt über eine Gültigkeitsdauer. Diese kann (in der Eigenschaft **ExpiryTimeUtc**) explizit durch den Dienst festgelegt werden, oder die Gültigkeitsdauer wird durch IoT Hub mithilfe der standardmäßigen Gültigkeitsdauer als IoT Hub-Eigenschaft festgelegt, siehe [Optionen für die C2D-Konfiguration](#c2dconfiguration).
...
| Description | Zeichenfolgenwerte für die oben genannten Ergebnisse. |
...
IoT Hub lässt pro Einheit und Tag (unabhängig von der SKU) maximal 1.100 Geräteaktualisierungen (Erstellen, Aktualisieren, Löschen) zu.

### Vorgangsdrosselung
Bei der Vorgangsdrosselung wird die Datenübertragungsrate pro Minute begrenzt, um einen Missbrauch zu verhindern. IoT Hub versucht, möglichst keine Fehler zurückzugeben, es wird jedoch eine Ausnahme ausgelöst, wenn die Drosselungsgrenze zu lange überschritten wird. Die nachfolgende Liste zeigt alle erzwungenen Werte für die Drosselung. Die Werte beziehen sich auf einen einzelnen Hub.
...
[lnk-arm]: https://azure.microsoft.com/de-DE/documentation/articles/resource-group-overview/
[lnk-azure-resource-manager]: https://azure.microsoft.com/de-DE/documentation/articles/resource-group-overview/
[lnk-cbs]: https://www.oasis-open.org/committees/download.php/50506/amqp-cbs-v1%200-wd02%202013-08-12.doc
[lnk-createuse-sas]: https://azure.microsoft.com/de-DE/documentation/articles/storage-dotnet-shared-access-signature-part-2/
[lnk-event-hubs-publisher-policy]: https://code.msdn.microsoft.com/Service-Bus-Event-Hub-99ce67ab
[lnk-event-hubs]: http://azure.microsoft.com/services/event-hubs/
[lnk-event-hubs-consuming-events]: https://azure.microsoft.com/de-DE/documentation/articles/event-hubs-programming-guide/#consume-events-event-consumers
[lnk-guidance-d2c-processing]: iot-hub-csharp-csharp-process-d2c.md
[lnk-management-portal]: https://portal.azure.com
[lnk-rfc7232]: https://tools.ietf.org/html/rfc7232
[lnk-sas-java]: https://msdn.microsoft.com/de-DE/library/azure/Hh875756.aspx
