<properties linkid="develop-mobile-tutorials-sso-android" urlDisplayName="Active Directory SSO Authentication with ADAL" pageTitle="Bei Active Directory anmelden und auf Ressourcen zugreifen (Android) | Mobile Dev Center" metaKeywords="" description="Erfahren Sie, wie Sie eine App mit Active Directory authentifizieren" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Authentifizieren Sie Ihre App mit Active Directory" authors="wesmc,mahender" />

<tags ms.service="mobile-services" ms.workload="mobile" ms.tgt_pltfrm="mobile-android" ms.topic="article" ms.date="01/01/1900" ms.author="yuaxu" />

# Authentifizieren Sie Ihre App mit Active Directory

<div class="dev-center-tutorial-selector sublanding">
<a href="/de-de/documentation/articles/mobile-services-windows-store-dotnet-adal-sso-authentication" title="Windows Store C#" >Windows Store C#</a>
<a href="/de-de/documentation/articles/mobile-services-dotnet-backend-ios-adal-sso-authentication" title="iOS">iOS</a>
<a href="/de-de/documentation/articles/mobile-services-dotnet-backend-xamarin-ios-adal-sso-authentication" title="Xamarin.iOS">Xamarin.iOS</a>
<a href="/de-de/documentation/articles/mobile-services-dotnet-backend-android-ad-authentication" title="Android" class="current">Android</a>
</div>

Lernen Sie anhand des Lernprogramms [Accessing Resources Secured by Azure Active Directory with iOS and Android][Accessing Resources Secured by Azure Active Directory with iOS and Android] (in englischer Sprache) von Chris Risner, wie Sie mit Active Directory Ihre App authentifizieren und auf Ressourcen zugreifen.



  [Accessing Resources Secured by Azure Active Directory with iOS and Android]: http://chrisrisner.com/Accessing-Resources-Secured-by-Azure-Active-Directory-with-iOS-and-Android
