...
## Erstellen des Backup-Tresors

1. Melden Sie sich beim [Verwaltungsportal](https://manage.windowsazure.com/) an.
2. Klicken Sie auf **Neu** -> **Data Services** -> **Recovery Services** -> **Backup-Tresor**, und wählen Sie **Schnellerfassung** aus.
...
5. Klicken Sie auf **Tresor erstellen**, um den Workflow abzuschließen. Es kann eine Weile dauern, bis der Sicherungstresor fertiggestellt wird. Sie können die Benachrichtigungen unten im Portal überwachen, um den Status zu überprüfen.
  ...
  ![Erstellen eines Tresors][2]

6. Nach dem Erstellen des Sicherungstresors erhalten Sie eine Nachricht mit dem Hinweis, dass der Tresor erfolgreich erstellt wurde. Er wird anschließend in den Ressourcen der Recovery Services als **Aktiv** angezeigt.

  ![Status zum Erstellen eines Tresors][3]
...
Die Speicherredundanz des Back-End-Speichers von Azure Backup wird durch Ihre geschäftlichen Anforderungen bestimmt. Wenn Sie Azure als primären Speicherendpunkt für die Sicherung verwenden (wenn Sie beispielsweise von einem Windows Server in Azure sichern), sollten Sie die Option "Georedundanter Speicher" in Betracht ziehen (Standardeinstellung). Diese finden Sie unter der Option **Konfigurieren** Ihres Backup-Tresors.
