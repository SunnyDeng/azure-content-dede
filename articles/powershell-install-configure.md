...
<div class="dev-center-tutorial-selector sublanding"><a href="/manage/install-and-configure-windows-powershell/" title="PowerShell" class="current">PowerShell</a><a href="/manage/install-and-configure-cli/" title="Cross-Platform CLI">Plattformübergreifende Befehlszeilenschnittstelle</a></div>
...
Sie können die Azure PowerShell-Module herunterladen und installieren, indem Sie den [Microsoft-Webplattform-Installer](http://go.microsoft.com/fwlink/p/?LinkId=320376) ausführen. Klicken Sie, wenn Sie dazu aufgefordert werden, auf **Ausführen**. Der Webplattform-Installer installiert die Azure PowerShell-Module und alle Abhängigkeiten. Befolgen Sie die Anweisungen, um die Installation abzuschließen.

> [AZURE.NOTE] Wenn Sie nur den PowerShell-Installer herunterladen möchten, besuchen Sie https://github.com/Azure/azure-powershell/releases.
In diesem Repository finden Sie auch den Quellcode für die PowerShell-Cmdlets.
...
- Verwenden Sie auf einem Computer, auf dem eine niedrigere Version als Windows 8 oder Windows Server 2012 ausgeführt wird, das Startmenü. Klicken Sie im Startmenü auf **Alle Programme**, dann auf **Azure** und anschließend auf **Azure PowerShell**.
...
1. Öffnen Sie die Azure PowerShell-Konsole, wie unter [Gewusst wie: Installieren von Azure PowerShell](#Install) beschrieben.
...
5. Ab Version 0.8.6 können Sie bei Verwendung eines Geschäfts- oder Schulkontos den folgenden Befehl eingeben, um das Popupfenster zu umgehen. Damit wird das Windows PowerShell-Standardfenster für Anmeldeinformationen eingeblendet, in dem Sie den Benutzernamen und das Kennwort Ihres Geschäfts- oder Schulkontos eingeben können.
	...
	>
	> 4. Wählen Sie im Verwaltungsportal **Einstellungen** und wählen Sie dann **Administratoren**. Wählen Sie **Hinzufügen**, und fügen Sie den neuen Benutzer als Co-Administrator hinzu. Dies ermöglicht es dem Geschäfts- oder Schulkonto, Ihr Azure-Abonnement zu verwalten.
	>
...
> [AZURE.NOTE] Die Cmdlets im "AzureResourceManager"-Modul erfordern die Azure AD-Methode (Add-AzureAccount). Diese Cmdlets bieten keine Unterstützung für Dateien mit Veröffentlichungseinstellungen. Weitere Informationen zu den Cmdlets im "AzureResourceManager"-Modul finden Sie unter [Azure Resource Manager Cmdlets](http://go.microsoft.com/fwlink/?LinkID=394765) (in englischer Sprache).


- Das Cmdlet **Get-AzurePublishSettingsFile** öffnet eine Webseite im
Azure-Verwaltungsportal, über die Sie die Abonnementinformationen herunterladen können. Die Informationen befinden sich in einer Datei mit der Erweiterung *.publishsettings.
...
2. Öffnen Sie die Azure PowerShell-Konsole, wie unter [Gewusst wie: Installieren von Azure PowerShell](#Install) beschrieben.
...
6. Um zu überprüfen, ob der Status der Web-App 'stopped' ist, führen Sie den Befehl "Get-AzureWebsite" erneut aus.
...
-   Sie können in der Konsole das integrierte Hilfesystem verwenden. Das Cmdlet **Get-Help** ermöglicht den Zugriff auf dieses System. Die folgende Tabelle enthält einige Beispiele für Befehle, die Sie verwenden können, um Hilfe anzufordern. Sie können weitere Informationen in der Konsole erhalten, indem Sie **help** eingeben.
	...
	<tr align="left" valign="top">
		...
		<td>Ruft die Hilfe zu einem Windows PowerShell-Cmdlet ab. Ersetzen Sie <Cmdlet> durch den Namen des Cmdlets.</td>
    </tr>
    <tr align="left" valign="top">
...
- Referenzinformationen zu den Cmdlets in den Azure PowerShell-Modulen sind in der Azure-Bibliothek verfügbar. Weitere Informationen finden Sie in der [Azure-Cmdlet-Referenz](http://msdn.microsoft.com/library/windowsazure/jj554330.aspx).
...
- Weitere Informationen zum Zugriff auf Azure-Speicherkomponenten finden Sie unter [Using Azure PowerShell with Azure Storage](storage-powershell-guide-full.md). (in englischer Sprache).
...
- Informationen zum Installieren, Erlernen, Verwenden und Anpassen von Windows PowerShell finden Sie unter [Scripting with Windows PowerShell](http://go.microsoft.com/fwlink/p/?LinkId=320210) (in englischer Sprache).

- Informationen zu Skripts und ihrer Ausführung in Windows PowerShell finden Sie unter [Running Scripts](http://go.microsoft.com/fwlink/p/?LinkId=320627) (in englischer Sprache). Dieser Artikel enthält grundlegende Informationen zum Erstellen von Skripten und Konfigurieren des Computers zur Ausführung von Skripten.
