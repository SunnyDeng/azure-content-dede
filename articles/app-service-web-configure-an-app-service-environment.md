...
Sie können nicht nur die Menge der Compute-Ressourcen verwalten, die einem bestimmten Pool zugewiesen werden können, Sie besitzen außerdem auch die Kontrolle über die Größe. In App Service-Umgebungen können Sie aus vier unterschiedlichen Größen mit den Namen P1 bis P4 auswählen. Einzelheiten zu diesen Größen und ihren Preisen finden Sie unter [App Service-Preise][AppServicePricing]. Die Größen P1 bis P3 der Compute-Ressourcen entsprechen denjenigen, die üblicherweise zur Verfügung stehen. Die P4-Compute-Ressource bietet 8 Kerne mit 14 GB Arbeitsspeicher und steht nur in einer App Service-Umgebung zur Verfügung.
...
Dies führt bei ausgeführten Apps zu sehr viel weniger Unterbrechungen als das Ändern der Größe von Compute-Ressourcen während der Ausführung von Workloads. Details zum Skalieren von Web-Apps in einer App Service-Umgebung finden Sie unter [Skalieren von Web-Apps in einer App Service-Umgebung][HowtoScale]
...
Falls gewünscht, können Sie den Zugriff jetzt auch mithilfe von Netzwerksicherheitsgruppen steuern. Diese Funktion ermöglicht Ihnen das Sperren Ihrer App Service-Umgebung, sodass nur die von Ihnen angegebenen IP-Adressen darauf zugreifen können. Weitere Informationen hierzu finden Sie unter [Steuern des eingehenden Datenverkehrs in einer App Service-Umgebung][ControlInbound].
