...
1. Klicken Sie im [Azure-Verwaltungsportal](http://manage.windowsazure.com/) auf den Verwaltungsseiten der Website auf die Registerkarte **Überwachen**, sodass die Verwaltungsseite **Überwachen** angezeigt wird. Standardmäßig zeigt das Diagramm auf der Seite **Monitor** dieselben Kennzahlen wie das Diagramm auf der Seite **Dashboard** an. 
...
4. Nachdem Sie die gewünschten Metriken für die Seite **Monitor** ausgewählt haben, klicken Sie den Haken unten. 
...
##<a name="howtoreceivealerts"></a>Vorgehensweise: Empfangen von Warnungen mittels Website-Metriken
Im Website-Modus **Standard** können Sie Warnungen basierend auf den Überwachungskennzahlen der Website empfangen. Zur Verwendung der Warnfunktion müssen Sie zuerst einen Webendpunkt zur Überwachung auswählen, was Sie im Abschnitt **Überwachung** der Seite **Konfigurieren** tun können. Auf der Seite **Einstellungen** des Azure-Verwaltungsportals können Sie anschließend eine Regel erstellen, um eine Warnung auszulösen, wenn die von Ihnen ausgewählte Kennzahl einen festgelegten Wert erreicht. Außerdem können Sie eine E-Mail senden, wenn der Alarm ausgelöst wird. Weitere Informationen finden Sie unter [Gewusst wie: Empfangen von Warnungs-Benachrichtigungen und Verwaltung von Alarmregeln in Azure](http://go.microsoft.com/fwlink/?LinkId=309356).  

##<a name="howtoviewusage"></a>Vorgehensweise: Anzeigen der Nutzungskontingente für eine Website

Websites können auf der Verwaltungsseite **Skalieren** zur Ausführung entweder im Websitemodus **Freigegeben** oder **Skalieren** konfiguriert werden. Jedes Azure-Abonnement hat Zugriff auf einen Ressourcenpool, der für die Ausführung von bis zu 100 Websites pro Region im Website-Modus **Freigegeben** bereitgestellt wird. Der für jedes Websiteabonnement zu diesem Zweck bereitgestellte Ressourcenpool wird auch von anderen Websites in derselben geografischen Region genutzt, die für die Ausführung im Modus **Freigegeben** konfiguriert sind. Da diese Ressourcen von anderen Websites ebenfalls genutzt werden, verfügen alle Abonnements über begrenzte Ressourcennutzung. Die Höchstgrenzen für die Ressourcennutzung von Abonnements werden als Nutzungskontingente bezeichnet und sind im Abschnitt "Nutzungsübersicht" der Verwaltungsseite **Dashboard** jeder Website aufgelistet.

>[WACOM.NOTE] Wenn eine Website für die Ausführung im Modus **Standard** konfiguriert ist, werden ihr spezielle Ressourcen zugeordnet, die der Größe der virtuellen Computer - **Klein** (Standard), **Mittel** oder **Groß** - in der Tabelle unter [Größen virtueller Computer und Cloud-Dienste für Azure][vmsizes] entsprechen. Es gibt kein Limit, wie viele Ressourcen ein Abonnement zum Ausführen von Websites im **Standard**-Modus verwenden kann. Die Anzahl der im **Standard**-Modus erstellten Websites pro Region ist jedoch auf 500 begrenzt.
...
2. Im Abschnitt **Nutzungsübersicht** werden die Nutzungskontingente für den jeweiligen Webhostingplan angezeigt, der eine Teilmenge von Folgendem darstellt:
	-	**Ausgehende Daten**, **CPU-Zeit** und **Arbeitsspeicher**. Wird das Kontingent überschritten, stoppt Azure die Website für den Rest des aktuellen Kontingentintervalls. Azure startet die Websites zu Beginn des nächsten Kontingentintervalls.
...
- Verschieben Sie die Website(s) auf einen größeren Webhostingplan und nutzen Sie das höhere Kontingent dort. Das einzige Kontingent bei den Plänen **Basic** und **Standard** ist der Dateisystemspeicher. 
...
- **Anwendungsprotokollierung (BLOB-Speicher)** - Aktiviert die Protokollierung der von der Anwendung erzeugten Informationen, ähnlich wie die Option "Anwendungsprotokollierung (Tabellenspeicher)". Die Protokollinformationen werden jedoch in einem Blob im Azure-Speicherkonto gespeichert.
	...
	Zur Angabe des Azure-Speicherkontos und der Tabelle aktivieren Sie **Ein** wählen Sie **Protokolliergrad** aus und legen **Tabellenspeicher verwalten** fest. Geben Sie das Speicherkonto, den Blob-Container und den zu verwendenden Blob-Namen an, oder erstellen Sie einen neuen Container mit Blob.
...
<div class="dev-callout"> 
	<b>Hinweis</b> 
	<p><b>Anwendungsdiagnostik (Dateisystem)</b>, <b>Anwendungsdiagnostik (Tabellenspeicher)</b>und <b>Anwendungsdiagnostik (Blobspeicher)</b> können gleichzeitig aktiviert werden und haben individuelle Konfigurationsmöglichkeiten für die Protokollebene. Sie können beispielsweise Fehler und Warnmeldungen als langfristige Protokollierungslösung im Speicher protokollieren, während die Dateisystemprotokollierung nach der Instrumentierung des Anwendungscodes zum Zweck der Fehlerbehebung ausführlich erfolgt.</p> </div>

<div class="dev-callout"> 
	<b>Hinweis</b> 
	<p>Diagnosen können auch über die Azure PowerShell mit dem Cmdlet <b>Set-AzureWebsite</b> aktiviert werden.</p><p>Wenn Sie Azure PowerShell nicht installiert haben oder nicht zur Verwendung des Azure-Abonnements konfiguriert haben, finden Sie weitere Informationen unter <a href="http://www.windowsazure.com/de-de/develop/nodejs/how-to-guides/powershell-cmdlets/">Verwenden von Azure PowerShell</a>.</p></div>

<div class="dev-callout"> 
<b>Hinweis</b> 
<p>Die Anwendungsprotokollierung beruht auf Protokollinformationen, die von Ihrer Anwendung erzeugt werden. Die verwendete Methode zum Erzeugen von Protokollinformationen sowie das Format der Informationen hängt von der Sprache ab, in der Ihre Anwendung geschrieben ist. Erläuterungen zur sprachspezifischen Verwendung der Anwendungsprotokollierung finden Sie in den folgenden Artikeln:</p>
<ul>
<li><b>.NET</b> - <a href="/de-de/develop/net/common-tasks/diagnostics-logging-and-instrumentation/">Aktivieren der Diagnoseprotokollierung für Azure-Websites</a></li>
...
<p>Anwendungsprotokollierung in einer Tabelle oder einem Blob-Speicher wird nur für .NET-Anwendungen unterstützt.</p>
</div>
...
1. Suchen Sie im **Startmenü** oder auf der **Startseite** nach **Azure PowerShell**. Klicken Sie mit der rechten Maustaste auf den Eintrag **Azure PowerShell**, und wählen Sie **Als Administrator ausführen** aus.
	...
	<div class="dev-callout"> 
	<b>Hinweis</b> 
	<p>Wenn <b>Windows Azure PowerShell</b> nicht installiert ist, finden Sie weitere Informationen unter <a href="http://msdn.microsoft.com/de-de/library/windowsazure/jj554332.aspx">Erste Schritte mit Azure PowerShell-Cmdlets</a> mit Hinweisen zu Installation und Konfiguration.</p> 
	</div>
...
<div class="dev-callout"> 
<b>Hinweis</b> 
<p>Wenn der <b>azure</b> -Befehl nicht installiert ist, finden Sie weitere Informationen unter <a href="http://www.windowsazure.com/de-de/develop/nodejs/how-to-guides/command-line-tools/">Verwenden des Azure-Befehlszeilentools</a> mit Hinweisen zu Installation und Konfiguration.</p>
</div>
...
- **Detaillierte Fehlerprotokollierung:** Enthält Informationen über Fehler wie <em>Modul</em>, <em>Handler</em>, <em>Fehlercode</em>und <em>Angeforderte URL</em>.
...
- Location: /LogFiles/http/RawLogs. Die in den Dateien gespeicherten Informationen sind im [erweiterten W3C-Protokollformat] formatiert(http://go.microsoft.com/fwlink/?LinkID=90561). Die Felder "s-computername", "s-ip" und "cs-version" werden von Azure-Websites nicht verwendet.
...
5.	Geben Sie die URL für den Teil der Website ein, den Sie überwachen möchten. Zum Beispiel, [http://contoso.azurewebsites.net/archive](http://contoso.azurewebsites.net/archive). 
...
8.	Klicken Sie**Speichern**. Es kann einige Zeit dauern, bis die Daten der Web-Endpunktüberwachung auf den Registerkarten **Dashboard** und **Monitor** verfügbar sind.

	Gehen Sie folgendermaßen vor, um eine E-Mail-Regel zu erstellen:

9.	Klicken Sie im Dienstebalken links auf **Verwaltungsdienste**.
10.	Klicken Sie unten auf**Regel hinzufügen**.
11.	Wählen Sie in **Diensttyp** die Option **Website**, und wählen Sie dann die Website, für die Sie zuvor die Endpunktüberwachung konfiguriert haben. Klicken Sie **Weiter**.
...
13.	Wählen Sie **Eine E-Mail an Dienstadministrator oder Coadministratoren senden**. Klicken Sie auf **Fertig stellen**.
