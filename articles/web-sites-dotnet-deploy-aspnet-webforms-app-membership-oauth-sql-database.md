<properties pageTitle="Bereitstellen einer sicheren ASP.NET-Web Forms-App mit Mitgliedschaft, OAuth und SQL-Datenbank für eine Azure-Website" metaKeywords="Azure WebForms Hello World Lernprogramm, Lernprogramm für erste Schritte mit WebForms in Azure, Azure C# WebForms Hello World Lernprogramm, SQL Azure C# WebForms Lernprogramm" description="In diesem Lernprogramm wird gezeigt, wie Sie eine sichere ASP.NET 4.5 Web Forms-Anwendung erstellen, die eine SQL-Datenbank beinhaltet, und die Anwendung dann auf Azure bereitstellen." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Deploy a secure ASP.NET 4.5 Web Forms app to an Azure Website" authors="erikre"  solutions="" manager="wpickett" editor="mollybos"  />
...
9. Wählen Sie im Feld **Server** die Option **New SQL Database server**.
...
5. Wählen Sie im Dialogfeld **New ASP.NET Project** die Vorlage **Web Forms** aus. Deaktivieren Sie das Kontrollkästchen **Host in der Cloud**, wenn es aktiviert ist, und klicken Sie auf **OK**.  
...
1. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf *Modelle* und anschließend auf **Hinzufügen** -> **Klasse**.  
...
6. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf das *Projekt*, und wählen Sie dann **Hinzufügen** -> **Neu erstelltes Element** aus.  
...
3. Wählen Sie den Link **Contact Demo** oben auf der Seite aus, um die Kontaktlistenseite anzuzeigen.  
...
2. Entfernen Sie die Kommentarzeichen aus der app.UseGoogleAuthentication()-Methode so, dass die Methode folgedermaßen angezeigt wird.  
	<pre class="prettyprint">
...
2. Fügen Sie die folgenden using-Anweisungen im Namespace "ContactManger.Migrations" hinzu:  
	<pre class="prettyprint">
	using Microsoft.AspNet.Identity;
	using Microsoft.AspNet.Identity.EntityFramework;
	</pre>
...
4. Fügen Sie einen Aufruf der AddUserAndRole-Methode an den Anfang der Seed-Methode hinzu. Beachten Sie, dass nur der Anfang der Seed-Methode angezeigt wird.  
	<pre class="prettyprint">
        protected override void Seed(ContactManager.Models.ApplicationDbContext context)
        {
...
4. Aktualisieren Sie die sechs Verweise des Codes 'Response.Redirect("Default.aspx")' für die folgenden drei Dateien in 'Response.Redirect("~/Contacts/Default.aspx")':  
	- *Delete.aspx.cs*
	- *Edit.aspx.cs*
	- *Insert.aspx.cs*  
...
4. Geben Sie die neuen Daten für jedes Feld ein, und drücken Sie dann auf die Schaltfläche **Einfügen**.  
	...
	Die Seite *EditContactList.aspx* wird mit den neuen Eintrag angezeigt.  
...
3. Wenn das Dialogfeld **Add Firewall Rule** angezeigt wird, wählen Sie **Add Firewall Rule** aus.  
	...
	>[WACOM.NOTE]  
	Wenn Sie **SQL Databases** nicht erweitern und **ContactDB** in Visual Studio nicht anzeigen können, müssen Sie die folgenden Anweisungen befolgen, um einen Firewallport oder einen Portbereich zu öffnen. Befolgen Sie dazu die Anweisungen unter **Einrichten von Azure-Firewallregeln** am Ende des [MVC-Lernprogramms](http://azure.microsoft.com/de-de/documentation/articles/web-sites-dotnet-deploy-aspnet-mvc-app-membership-oauth-sql-database/). Alternativ können Sie auch die Daten der lokalen Datenbank überprüfen, indem Sie die Anwendung lokal erstellen, ausführen und Daten hinzufügen (**STRG+F5** in Visual Studio).  
