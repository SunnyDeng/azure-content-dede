...
<a href="/de-de/documentation/articles/mobile-services-xamarin-android-get-started-offline-data" title="Xamarin.Android" class="current">Xamarin.Android</a>
</div>
...
-   Visual Studio mit der [Xamarin-Erweiterung][Xamarin-Erweiterung] **oder** [Xamarin Studio][Xamarin Studio]
...
-   Fügen Sie die Deklarationen hinzu.

        using Microsoft.WindowsAzure.MobileServices.Sync;
        using Microsoft.WindowsAzure.MobileServices.SQLiteStore;
        using System.IO;

-   Ändern Sie den Mitgliedstyp für `ToDoActivity.toDoTable` von `IMobileServiceTable<>` in `IMobileServiceSyncTable<>`.

-   Fügen Sie in der Methode `OnCreate(Bundle)` nach der Zeile, die das Mitglied `client` initialisiert, den folgenden Code hinzu:
...
-   Ändern Sie in derselben Methode die Zeile, die `toDoTable` initialisiert, zur Verwendung der Methode `GetSyncTable<>` statt `GetTable<>`:

        toDoTable = client.GetSyncTable <ToDoItem> ();

-   Passen Sie die Methode `OnRefreshItemsSelected` an, um Aufrufe für `PushAsync` und `PullAsync` hinzuzufügen:
...
5.  Klicken Sie in der App auf die Schaltfläche **Refresh**. Dadurch ruft die App `MobileServiceClient.SyncContext.PushAsync` und `IMobileServiceSyncTable.PullAsync()` auf und dann `RefreshTodoItems`, um die App mit Elementen aus dem lokalen Speicher zu aktualisieren.
...
<!-- Anchors. --> <!-- Images --> <!-- URLs. -->
  ...
  [Azure Mobile Services SDK Version 1.3.0-alpha3]: http://www.nuget.org/packages/WindowsAzure.MobileServices/1.3.0-alpha3
