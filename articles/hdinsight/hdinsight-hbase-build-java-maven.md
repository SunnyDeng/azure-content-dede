...
4. Erstellen Sie ein neues Verzeichnis namens __conf__ im Verzeichnis __hbaseapp__. Erstellen Sie im Verzeichnis __conf__ eine neue Datei namens __hbase-site.xml__, und verwenden Sie Folgendes als Inhalt.
    ...
    > [AZURE.NOTE] Das ist eine sehr minimale Datei vom Typ "hbase-site.xml", und sie enthält Mindesteinstellungen für den HDInsight-Cluster. Für Linux-basierte Cluster müssen Sie die Kommentierung des Eintrags für `zookeeper.znode.parent` aufheben, um den Stamm-zNode für Zookeeper ordnungsgemäß festzulegen, der von HBase verwendet wird.
...
> [AZURE.NOTE] Es gibt viele Möglichkeiten, eine Datei in einen HDInsight-Cluster hochzuladen, wie unter [Hochladen von Daten für Hadoop-Aufträge in HDInsight](hdinsight-upload-data.md) beschrieben. Die folgenden Schritte geschehen mithilfe von [Azure PowerShell](../powershell-install-configure.md).
