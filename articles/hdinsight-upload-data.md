...
Azure HDInsight stellt über Azure Blob-Speicher ein Hadoop Distributed File System (HDFS) mit vollem Funktionsumfang zur Verfügung. Es wurde als eine HDFS-Erweiterung entwickelt, die Kunden eine reibungslose Leistung bietet, indem alle Komponenten im Hadoop-System direkt mit den von ihm verwalteten Daten arbeiten können. Azure Blob-Speicher und HDFS sind unterschiedliche Dateisysteme, die jeweils für die Datenspeicherung und Berechnungen dieser Daten optimiert sind. Die Vorteile der Verwendung von Azure Blob-Speicher werden unter [Verwenden von Azure-Blob-Speicher mit HDInsight][Verwenden von Azure-Blob-Speicher mit HDInsight].
...
Auf den Azure Blob-Speicher kann über [AzCopy][AzCopy], [Azure PowerShell][Azure PowerShell], [Azure-Speicherclientbibliothek für .NET][Azure-Speicherclientbibliothek für .NET] oder über Explorer-Tools zugegriffen werden. Hier sind einige der verfügbaren Tools:
...
-   Einen Azure HDInsight-Cluster. Anweisungen finden Sie unter [Erste Schritte mit Azure HDInsight][Erste Schritte mit Azure HDInsight] or [Bereitstellen eines HDInsight-Clusters][Bereitstellen eines HDInsight-Clusters].

## Themen in diesem Artikel

-   [Hochladen von Daten zum Blob-Speicher mit AzCopy][Hochladen von Daten zum Blob-Speicher mit AzCopy]
-   [Hochladen von Daten zum Blob-Speicher mit Azure PowerShell][Hochladen von Daten zum Blob-Speicher mit Azure PowerShell]
-   [Hochladen von Daten zum Blob-Speicher mit Azure-Speicher-Explorer][Hochladen von Daten zum Blob-Speicher mit Azure-Speicher-Explorer]
-   [Hochladen von Daten zum Blob-Speicher mit der Hadoop-Befehlszeile][Hochladen von Daten zum Blob-Speicher mit der Hadoop-Befehlszeile]
-   [Importieren von Daten aus der Azure-SQL-Datenbank in den Blob-Speicher mit Sqoop][Importieren von Daten aus der Azure-SQL-Datenbank in den Blob-Speicher mit Sqoop]

## <span id="azcopy"></span></a>Hochladen von Daten zum Blob-Speicher mit AzCopy

AzCopy ist ein Befehlszeilenprogramm, das das Übertragen von Daten in ein Azure-Speicherkonto und von einem Azure-Speicherkonto vereinfachen soll. Sie können dieses Dienstprogramm als eigenständiges Tool verwenden oder in eine vorhandene Anwendung integrieren. [AzCopy herunterladen][AzCopy herunterladen].
...
Azure PowerShell ist eine leistungsstarke Skriptumgebung, mit der Sie die Bereitstellung und Verwaltung Ihrer Arbeitsauslastungen in Azure steuern und automatisieren können. Mit Azure PowerShell können Sie Daten zum Blob-Speicher hochladen, sodass die Daten von MapReduce-Jobs verarbeitet werden können. Informationen zum Konfigurieren der Arbeitsstation für die Ausführung von Azure PowerShell finden Sie unter [Installieren und Konfigurieren von Azure PowerShell][Installieren und Konfigurieren von Azure PowerShell].

**So laden Sie eine lokale Datei zum Blob-Speicher hoch**

1.  Öffnen Sie das Azure PowerShell-Konsolenfenster entsprechend den Anweisungen unter [Installieren und Konfigurieren von Azure PowerShell][Installieren und Konfigurieren von Azure PowerShell].
...
Weitere Informationen über Sqoop finden Sie unter [Verwenden von Sqoop mit HDInsight][Verwenden von Sqoop mit HDInsight].
...
-   [Verwenden von Pig mit HDInsight][Verwenden von Pig mit HDInsight]
  ...
  [Azure-Speicherclientbibliothek für .NET]: /de-de/develop/net/how-to-guides/blob-storage/
