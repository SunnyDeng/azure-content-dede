...
4. *Hinzufügen eines neuen Endpunkts* Der Bewertungswebdienst, den wir oben in Schritt 2 veröffentlicht haben, wurde mit einem Standardendpunkt erstellt. Die Standardendpunkte werden mit dem ursprünglichen Experiment synchronisiert, und daher kann das trainierte Modell, das einem Standardendpunkt zugeordnet ist, nicht ersetzt werden. Zum Erstellen eines aktualisierbaren Endpunkts besuchen Sie das Azure-Portal, und klicken Sie auf "Add Endpoint" \(Endpunkt hinzufügen\) \(nähere Informationen finden Sie [hier](machine-learning-create-endpoint.md)\).	
