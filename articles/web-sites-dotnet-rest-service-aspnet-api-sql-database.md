<properties urlDisplayName="REST service using Web API" pageTitle=".NET REST-Dienst mit Web API - Azure-Lernprogramm" metaKeywords="Azure tutorial web site, ASP.NET API web site, Azure VS" description="In diesem Lernprogramm erfahren Sie, wie Sie eine App bereitstellen, die die ASP.NET-Web-API mithilfe von Visual Studio für eine Azure-Website verwendet." metaCanonical="" services="web-sites" documentationCenter=".NET" title="REST service using ASP.NET Web API and SQL Database" authors="riande" solutions="" manager="wpickett" editor="" />
...
***Von [Rick Anderson](https://twitter.com/RickAndMSFT) und Tom Dykstra.***

In diesem Lernprogramm wird die Bereitstellung einer ASP.NET-Webanwendung für eine Azure-Website mit dem Assistenten "Web veröffentlichen" in Visual Studio 2013 oder Visual Studio 2013 for Web Express gezeigt. 
...
1. Klicken Sie im [Azure-Verwaltungsportal](https://manage.windowsazure.com) auf **Websites** in der linken Registerkarte und dann auf **Neu**.
...
1. Wählen Sie in der Dropdownliste **Region** die am nächsten befindliche Region aus.

1. Wählen Sie in der Dropdownliste **Datenbank** die Option **Create a free 20 MB SQL database** aus.
...
3. Erweitern Sie im Dialogfeld **Neues Projekt** die Option **Visual C#** und wählen Sie **Web**. Wählen Sie dann **ASP.NET MVC 5-Webanwendung**. Behalten Sie die Standardeinstellung **.NET Framework 4.5** bei. Nennen Sie die Anwendung **ContactManager**, und klicken Sie auf **OK**.
	![New Project dialog box](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr4.PNG)]
1. Wählen Sie im Dialogfeld **Neues ASP.NET-Projekt** die Vorlage **MVC**, kreuzen Sie **Web API** an, und klicken Sie dann auf **Authentifizierung ändern**.
...
2. Klicken Sie auf der Registerkarte **Profil** im Assistenten **Web veröffentlichen** auf **Importieren**.
...
12. Klicken Sie**Veröffentlichen**.
...
1. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf die Modelle, und klicken Sie dann auf **Hinzufügen** und anschließend auf **Klasse**.

	![Add Class in Models folder context menu][adddb001]

2. Geben Sie der neuen Klassendatei im Dialogfeld **Neues Element hinzufügen** den Namen *Contact.cs* ein, und klicken Sie dann auf **Hinzufügen**.
...
Mit der Klasse **Contacts** werden die Daten definiert, die für die einzelnen Kontakte gespeichert werden, sowie ein Primärschlüssel ("ContactID", der von der Datenbank benötigt wird. Weitere Informationen zu Datenmodellen finden Sie unter [Nächste Schritte](#nextsteps) am Ende dieses Lernprogramms.
...
6. Wählen Sie als Bezeichnung für den Controller **HomeController**. Wählen Sie **Contact** als Modellklasse. Klicken Sie auf **Neuer Datenkontext** und bestätigen Sie die Standardeinstellung "ContactManager.Models.ContactManagerContext" für **Neuer Datenkontexttyp**. Klicken Sie**Hinzufügen**.

	![Add Controller dialog box](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr9.PNG)

	In einem Dialogfeld wird Folgendes angezeigt: "Es besteht bereits eine Datei mit der Bezeichnung "HomeController". Möchten Sie sie ersetzen?" Klicken Sie**Ja**. Der erstellte HomeController wird mit dem neuen Projekt überschrieben. Der neue HomeController wird für die Kontaktliste verwendet.
...
Die nächste Aufgabe besteht in der Aktivierung der Funktion [Code First Migrations](http://curah.microsoft.com/55220) zum Erstellen der Datenbank auf Basis des von Ihnen festgelegten Datenmodells.

1. Wählen Sie im Menü **Extras** den Eintrag **Library Package Manager** und danach **Package Manager-Konsole** aus.

	![Package Manager Console in Tools menu][addcode008]

2. Geben Sie im Fenster **Package Manager Console** den folgenden Befehl ein:
...
2. Geben Sie im Fenster **Package Manager Console** den folgenden Befehl ein:
	...
	In der **Initial**-Klasse wird mit der **Up**-Methode die Tabelle **Down**-Methode (wird verwendet, wenn Sie zum vorherigen Status zurückkehren möchten) wieder verworfen.
...
1. Öffnen Sie die Datei *Views\Home\Index.cshtml*. Im nächsten Schritt ersetzen wir das generierte Markup mit Code, der [jQuery](http://jquery.com/) und [Knockout.js] verwendet(http://knockoutjs.com/). Dieser neue Code ruft die Liste der Kontakte mithilfe von Web API und JSON ab und bindet die Kontaktdaten mithilfe von knockout.js an das UI. Weitere Informationen finden Sie im Abschnitte [Nächste Schritte](#nextsteps) am Ende dieses Lernprogramms. 
...
3. Klicken Sie mit der rechten Maustaste auf den Ordner "Content" und klicken Sie anschließend auf **Hinzufügen** und **Controller**.
...
7. Fügen Sie den folgenden Code hinzu, um [Knockout](http://knockoutjs.com/index.html "KO") zu registrieren.
...
1. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf die Modelle, und klicken Sie dann auf **Hinzufügen** und anschließend auf **Controller....** 

1. Geben Sie im Dialogfeld **Add Scaffold** die Option **Web API 2 Controller with actions, using Entity Framework** aus, und klicken Sie dann auf **Hinzufügen**.
...
1. Klicken Sie im **Projektmappen-Explorer** mit der rechten Maustaste auf das **ContactManager**-Projekt und dann auf **Hinzufügen**, und klicken Sie anschließend auf **Class**.
...
1. Fügen Sie den folgenden *Benutzervermerk* zum "Contracts Controller" hinzu, damit Sie Zugriff auf das Attribut **[ValidateHttpAntiForgeryToken]** haben.
...
1. Aktualisieren Sie den Abschnitt *Scripts* der Datei *Views\Home\Index.cshtml*, um Code für die XSRF-Tokens zu erhalten.
...
Dieses Lernprogramm und die Beispielanwendung wurden von [Rick Anderson](http://blogs.msdn.com/b/rickandy/) (Twitter [@RickAndMSFT](https://twitter.com/RickAndMSFT)) geschrieben, mit Unterstützung von Tom Dykstra und Barry Dorrans (Twitter [@blowdart](https://twitter.com/blowdart)). 
...
[dbcontext-link]: http://msdn.microsoft.com/de-de/library/system.data.entity.dbcontext(v=VS.103).aspx
