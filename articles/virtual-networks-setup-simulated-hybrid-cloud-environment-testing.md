<properties 
	pageTitle="Einrichten einer simulierten Hybrid Cloud-Umgebung zu Testzwecken" 
	description="Hier erhalten Sie Informationen zum Erstellen einer simulierten Hybrid Cloud-Umgebung für Tests durch IT-Spezialisten oder zu Entwicklungszwecken, die aus zwei virtuellen Azure-Netzwerken und einer VNet-zu-VNet-Verbindung besteht." 
...
In diesem Thema lernen Sie Schritt für Schritt, wie Sie eine simulierte Hybrid Cloud-Umgebung mit Microsoft Azure für Tests mithilfe zweier virtueller Azure-Netzwerke erstellen. Verwenden Sie diese Konfiguration als Alternative zu 
[Einrichten einer Hybrid Cloud-Umgebung zu Testzwecken](http://azure.microsoft.com/documentation/articles/virtual-networks-setup-hybrid-cloud-environment-testing/), wenn Sie nicht über eine direkte Internetverbindung und eine erreichbare öffentliche IP-Adresse verfügen. Die resultierende Konfiguration sieht folgendermaßen aus.
...
>[AZURE.NOTE] Durch die Ausführung von virtuellen Computern und virtuellen Netzwerkgateways in Azure entstehen laufende Kosten. Diese Kosten werden im Rahmen der kostenlosen Testversion, des MSDN-Abonnements oder des kostenpflichtigen Abonnements abgerechnet. Informationen zur Reduzierung der Kosten für diese Testumgebung, wenn sie nicht verwendet wird, finden Sie unter [Minimieren der laufenden Kosten dieser Umgebung](#costs) in diesem Thema.



##Phase 1: Konfigurieren des virtuellen Netzwerks TestLab

Befolgen Sie die Anweisungen in der Anleitung [Test Lab: Grundlegende Konfiguration in Azure](http://www.microsoft.com/download/details.aspx?id=41684), um die Computer DC1, APP1 und CLIENT1 in einem virtuellen Azure-Netzwerk namens TestLab zu konfigurieren. 

Stellen Sie im Azure-Verwaltungsportal auf Ihrem lokalen Computer eine Verbindung mit DC1 her. Verwenden Sie dafür die Anmeldeinformationen für CORP\User1. Führen Sie in einer Windows PowerShell-Eingabeaufforderung die folgenden Befehle auf Administratorebene aus, um die CORP-Domäne so zu konfigurieren, dass Computer und Benutzer zur Authentifizierung ihre lokalen Domänencontroller verwenden.
...
1.	Klicken Sie in der Taskleiste des Azure-Verwaltungsportals auf **Neu > Network Services > Virtuelles Netzwerk > Benutzerdefiniert erstellen**.
2.	Geben Sie auf der Seite "Details zum virtuellen Netzwerk" unter **Name** den Namen **TestVNET** ein.
3.	Wählen Sie unter **Speicherort** den entsprechenden Speicherort.
4.	Klicken Sie auf den Pfeil für "Weiter".
5.	Geben Sie auf der Seite "DNS-Server und VPN-Konnektivität" unter **DNS-Server** > **Namen eingeben oder auswählen** den Namen **DC1** ein , und klicken Sie dann auf den Pfeil für "Weiter".
6.	Gehen Sie auf der Seite "Virtuelle Netzwerkadressräume" folgendermaßen vor:
	- Wählen Sie unter **Adressraum** > **Start-IP** die IP **192.168.0.0** aus, oder geben Sie sie ein.
	- Klicken Sie unter **Subnetze** auf **Subnet-1**, und ersetzen Sie diesen Namen durch **TestSubnet**. 
	- Klicken Sie in der Spalte **CIDR (Anzahl Adressen)** für das TestSubnet auf **/24 (256)**.
7.	Klicken Sie auf das Symbol "Abgeschlossen". Warten Sie, bis das virtuelle Netzwerk erstellt ist, bevor Sie fortfahren.

Befolgen Sie anschließend die Anweisungen unter [Installieren und Konfigurieren von Azure PowerShell zum Installieren von Azure PowerShell auf dem lokalen Computer](../install-configure-powershell/).

Erstellen Sie anschließend einen neuen Cloud-Dienst für das virtuelle Netzwerk TestVNET. Sie müssen einen eindeutigen Namen auswählen. Sie können beispielsweise den Namen **TestVNET -***UniqueSequence* wählen, wobei *UniqueSequence* eine Abkürzung für Ihre Organisation ist. Wenn Ihre Organisation z. B. Tailspin Toys heißt, können Sie den Cloud-Dienst **TestVNET-Tailspin** nennen.
...
1.	Klicken Sie im Azure-Verwaltungsportal auf dem lokalen Computer auf **Neu > Network Services > Virtuelles Netzwerk > Lokales Netzwerk hinzufügen**.
...
3.	Geben Sie auf der Seite "Adressraum angeben" unter **Start-IP** die IP **10.0.0.0** ein.
4.	Klicken Sie unter **CIDR (Anzahl Adressen)** auf **/24 (256)**, und klicken Sie dann auf das Häkchen.
5.	Klicken Sie auf **Neu > Network Services > Virtuelles Netzwerk > Lokales Netzwerk hinzufügen**.
...
8.	Klicken Sie unter **CIDR (Anzahl Adressen)** auf **/24 (256)**, und klicken Sie dann auf das Häkchen.
...
1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf dem lokalen Computer auf **Netzwerke**, und überprüfen Sie, ob die Spalte **Status** für **TestLab** auf **Erstellt** gesetzt ist.
...
3.	Klicken Sie unter **LAN** auf **TestVNETLNet**.
4.	Klicken Sie in der Taskleiste auf **Speichern**.
5.	Klicken Sie im linken Bereich auf **Netzwerke**, und überprüfen Sie, ob die Spalte **Status** für TestVNET auf **Erstellt** gesetzt ist.
...
7.	Klicken Sie unter **LAN** auf **TestLabLNet**.
8.	Klicken Sie in der Taskleiste auf **Speichern**.

Erstellen Sie anschließend die virtuellen Netzwerkgateways für die beiden virtuellen Netzwerke.

1.	Klicken Sie im Azure-Verwaltungsportal auf der Seite **Netzwerke** auf **TestLab**. Auf der Seite "Dashboard" sollte nun der Status **Das Gateway wurde nicht erstellt** angezeigt werden.
2.	Klicken Sie in der Taskleiste auf **Gateway erstellen** und anschließend auf **Dynamisches Routing**. Klicken Sie auf **Ja**, wenn Sie dazu aufgefordert werden. Warten Sie, bis die Erstellung des Gateways abgeschlossen ist und sein Status in **Verbindung wird hergestellt** geändert wird. Dies kann einige Minuten dauern.
3.	Beachten Sie auf der Seite "Dashboard" die **Gateway-IP-Adresse**. Dies ist die öffentliche IP-Adresse des Azure-VPN-Gateways für das virtuelle Netzwerk TestLab. Notieren Sie diese IP-Adresse, da sie zum Konfigurieren der VNet-zu-VNet-Verbindung erforderlich sein wird.
4.	Klicken Sie in der Taskleiste auf **Schlüssel verwalten** und anschließend auf das Symbol "Kopieren" neben dem Schlüssel, um diesen in die Zwischenablage zu kopieren. Fügen Sie diesen Schlüsselwert in ein Dokument ein, und speichern Sie dieses. Sie benötigen diesen Schlüsselwert, um die VNet-zu-VNet-Verbindung zu konfigurieren.
5.	Klicken Sie auf der Seite "Netzwerke" auf **TestVNET**. Auf der Seite "Dashboard" sollte nun der Status **Das Gateway wurde nicht erstellt** angezeigt werden.
6.	Klicken Sie in der Taskleiste auf **Gateway erstellen** und anschließend auf **Dynamisches Routing**. Klicken Sie auf **Ja**, wenn Sie dazu aufgefordert werden. Warten Sie, bis die Erstellung des Gateways abgeschlossen ist und sein Status in **Verbindung wird hergestellt** geändert wird. Dies kann einige Minuten dauern.
7.	Beachten Sie auf der Seite "Dashboard" die **Gateway-IP-Adresse**. Dies ist die öffentliche IP-Adresse des Azure-VPN-Gateways für das virtuelle Netzwerk TestVNET. Notieren Sie diese IP-Adresse, da sie zum Konfigurieren der VNet-zu-VNet-Verbindung erforderlich sein wird.
...
6.	Geben Sie auf der Seite "Details zum lokalen Netzwerk angeben" unter **IP-Adresse des VPN-Geräts (optional)** die IP-Adresse des virtuellen Netzwerkgateways für das virtuelle Netzwerk TestVNET (aus Schritt 7 in der vorherigen Prozedur) ein, und klicken Sie dann auf den Pfeil nach rechts.
7.	Klicken Sie auf der Seite "Adressraum eingeben" auf das Häkchen.

Setzen Sie anschließend den vorinstallierten Schlüssel für beide Gateways auf denselben Wert. Dieser ist der im Azure-Verwaltungsportal festgelegte Schlüsselwert für das virtuelle Netzwerk TestLab. Führen Sie in einer Windows PowerShell-Eingabeaufforderung die folgenden Befehle auf Administratorebene aus, und tragen Sie den Wert des für TestLab vorinstallierten Schlüssels ein.
...
Klicken Sie anschließend im Azure-Verwaltungsportal des lokalen Computers auf der Seite "Netzwerk" auf das virtuelle Netzwerk **TestLab** und anschließend auf **Dashboard** sowie in der Taskleiste auf **Verbinden**. Warten Sie, bis angezeigt wird, dass das virtuelle Netzwerk TestLab verbunden ist.
...
Melden Sie sich anschließend auf dem neuen virtuellen Computer DC2 an.

1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf **Virtuelle Computer** und anschließend in der Spalte **Status** für DC2 auf **Wird ausgeführt**.
2.	Klicken Sie in der Taskleiste auf **Verbinden**. 
3.	Wenn Sie dazu aufgefordert werden, DC2.rdp zu öffnen, klicken Sie auf **Öffnen**.
...
Konfigurieren Sie anschließend DC2 als Replikat-Domänencontroller für die Domäne "corp.contoso.com". Führen Sie in der Windows PowerShell-Eingabeaufforderung von DC2 die folgenden Befehle aus.
...
Da nun das virtuelle Netzwerk TestNVET über seinen eigenen DNS-Server (DC2) verfügt, müssen Sie das virtuelle Netzwerk TestVNET für eine Verwendung dieses DNS-Servers konfigurieren.

1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportal auf **Netzwerke**, und anschließend auf **TestVNET**.
2.	Klicken Sie auf **Konfigurieren**.
3.	Entfernen Sie unter **DNS-Server** den Eintrag 10.0.0.4.
4.	Fügen Sie unter **DNS-Server** einen Eintrag mit dem Namen **DC2** und der IP-Adresse **192.168.0.4** hinzu. 
...
- [SharePoint-Intranetfarm](../virtual-networks-setup-sharepoint-hybrid-cloud-testing/)
- [Webbasierte Branchenanwendung](../virtual-networks-setup-lobapp-hybrid-cloud-testing/)
- [Office 365-Verzeichnissynchronisierungsserver (DirSync)](../virtual-networks-setup-dirsync-hybrid-cloud-testing/)

##Zusätzliche Ressourcen

[Einrichten einer Hybrid Cloud-Umgebung zu Testzwecken](../virtual-networks-setup-hybrid-cloud-environment-testing/)

[Konfigurieren einer Vnet-zu-VNet-Verbindung](http://msdn.microsoft.com/library/azure/dn690122.aspx)


##<a id="costs"></a>Minimieren der laufenden Kosten dieser Umgebung

Wenn Sie die Kosten für die Ausführung der virtuellen Computer in dieser Umgebung minimieren möchten, führen Sie die erforderlichen Tests und Demonstrationen so schnell wie möglich durch, und löschen Sie sie anschließend, oder fahren Sie die virtuellen Computer herunter, wenn Sie sie nicht verwenden. Sie können beispielsweise Azure Automation oder ein Runbook verwenden, um die virtuellen Computer in den virtuellen Netzwerken TestLab und TestVNET am Ende jedes Arbeitstages automatisch herunterzufahren. Weitere Informationen finden Sie unter [Erste Schritte mit Azure Automation](../automation-create-runbook-from-samples/). Wenn Sie die virtuellen Computer im Subnetz des Unternehmensnetzwerks erneut starten möchten, starten Sie zuerst DC1.

Ein Azure-VPN-Gateway wird als Gruppe von zwei virtuellen Computern in Azure implementiert, für die laufende Kosten anfallen. Informationen hierzu finden Sie unter [Preise - Virtuelles Netzwerk](http://azure.microsoft.com/pricing/details/virtual-network/). Wenn Sie die Kosten für die zwei VPN-Gateways (eines für TestLab und eines für TestVNET) minimieren möchten, erstellen sie die Testumgebung, und führen Sie die erforderlichen Test- und Demonstrationen so schnell wie möglich durch, oder löschen Sie die Gateways mit folgenden Schritten.
 
1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf dem lokalen Computer auf **Netzwerke** und anschließend auf **TestLab** sowie auf **Dashboard**.
...
3.	Klicken Sie im linken Bereich auf **Netzwerke** und anschließend auf **TestVNET** sowie auf **Dashboard**.
...
1.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf dem lokalen Computer auf **Netzwerke** und anschließend auf **TestLab**. Auf der Seite "Dashboard" sollte nun der Status **Das Gateway wurde nicht erstellt** angezeigt werden.
2.	Klicken Sie in der Taskleiste auf **Gateway erstellen** und anschließend auf **Dynamisches Routing**. Klicken Sie auf **Ja**, wenn Sie dazu aufgefordert werden. Warten Sie, bis die Erstellung des Gateways abgeschlossen ist und sein Status in **Verbindung wird hergestellt** geändert wird. Dies kann einige Minuten dauern.
3.	Beachten Sie auf der Seite "Dashboard" die **Gateway-IP-Adresse**. Dies ist die neue öffentliche IP-Adresse des Azure-VPN-Gateways für das virtuelle Netzwerk TestLab. Diese Adresse ist erforderlich, um das lokale Netzwerk TestLabLNet neu zu konfigurieren.
4.	Klicken Sie in der Taskleiste auf **Schlüssel verwalten** und anschließend auf das Symbol "Kopieren" neben dem Schlüssel, um diesen in die Zwischenablage zu kopieren. Fügen Sie diesen Schlüsselwert in ein Dokument ein, und speichern Sie dieses. Dieser Schlüsselwert ist erforderlich, um das VPN-Gateway für das virtuelle Netzwerk TestVNET neu zu konfigurieren.
5.	Klicken Sie im linken Bereich des Azure-Verwaltungsportals auf dem lokalen Computer auf **Netzwerke** und anschließend auf **TestVNET**. Auf der Seite "Dashboard" sollte nun der Status **Das Gateway wurde nicht erstellt** angezeigt werden.
6.	Klicken Sie in der Taskleiste auf **Gateway erstellen** und anschließend auf **Dynamisches Routing**. Klicken Sie auf **Ja**, wenn Sie dazu aufgefordert werden. Warten Sie, bis die Erstellung des Gateways abgeschlossen ist und sein Status in "Verbindung wird hergestellt" geändert wird. Dies kann einige Minuten dauern.
7.	Beachten Sie auf der Seite "Dashboard" die **Gateway-IP-Adresse**. Dies ist die neue öffentliche IP-Adresse des Azure-VPN-Gateways für das virtuelle Netzwerk TestVNET. Diese Adresse ist erforderlich, um das lokale Netzwerk TestVNETLNet neu zu konfigurieren.
