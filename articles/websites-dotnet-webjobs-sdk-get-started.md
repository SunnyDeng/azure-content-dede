<properties pageTitle="Erste Schritte mit dem Azure WebJobs-SDK" metaKeywords="Azure Lernprogramm, Azure WebJobs Lernprogramm, Azure mehrschichtig Lernprogramm, MVC Lernprogramm, Lernprogramm für Azure-Blobs, Azure-Warteschlangen-Lernprogramm, Azure-Speicher-Lernprogramm" description="Hier erfahren Sie, wie Sie mit ASP.NET MVC und Azure eine Anwendung mit mehreren Ebenen erstellen. Das Front-End wird auf einer Website und das Back-End als WebJob ausgeführt. Die App verwendet Entity Framework, SQL-Datenbank sowie Azure-Speicherwarteschlangen und -Blobs." metaCanonical="" services="web-sites,storage" documentationCenter=".NET" title="Get Started with the Azure WebJobs SDK" authors="tdykstra" solutions="" manager="wpickett" editor="mollybos" />
...
In diesem Lernprogramm erfahren Sie, wie Sie eine mehrschichtige ASP.NET MVC-Anwendung mit einem erstellen, die das WebJobs-SDK nutzt, um mit [Azure-Warteschlangen](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/queue-centric-work-pattern) und [Azure-Blobs](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/unstructured-blob-storage) auf einer [Azure-Website](/en-us/documentation/services/websites/) zu arbeiten. Die Anwendung verwendet auch [Azure SQL-Datenbank](http://msdn.microsoft.com/library/azure/ee336279). 
...
* Führen Sie das Programm in einer Azure-Cloud-Dienst-Workerrolle aus. Mit dieser Option können Sie das Front-End entweder in einer Cloud-Dienst-Webrolle oder in einer Website ausführen.

Dieses Lernprogramm beschreibt, wie Sie das Front-End in einer Website und das Back-End als WebJob in derselben Website ausführen können. Informationen zur Auswahl der optimalen Umgebung für Ihr Szenario finden Sie unter [Vergleich von Websites, Cloud-Diensten und virtuellen Computern in Azure](/en-us/documentation/articles/choose-web-site-cloud-service-vm/).

[WACOM.INCLUDE [install-sdk-2013-only](../includes/install-sdk-2013-only.md)]

Die Anleitungen in diesem Lernprogramm wurden mit der nächsten Vorabversion von [Visual Studio 2013 Update 4](http://go.microsoft.com/fwlink/?LinkID=510328) geschrieben. Der einzige Unterschied zu Visual Studio 2013 Update 3 befindet sich im Abschnitt zum Erstellen von Grund auf neu, in dem Sie das Projekt WebJob erstellen: mit Update 4 sind die WebJobs-SDK-Pakete automatisch im Projekt enthalten. Ohne Update 4 müssen Sie die Pakete manuell installieren.
...
Der Name muss eindeutig sein (kein anderes Azure-Speicherkonto kann den gleichen Namen haben). Wenn der eingegebene Name bereits verwendet wird, erhalten Sie Gelegenheit, ihn zu ändern.
...
1. Laden Sie die [vollständige Lösung][download] herunter, und entpacken Sie sie.
...
Die SQL-Verbindungszeichenfolge zeigt auf eine [SQL Server Express LocalDB](http://msdn.microsoft.com/en-us/library/hh510202.aspx)-Datenbank.
 
Die Speicher-Verbindungszeichenfolge ist ein Beispiel, das Platzhalter für den Namen des Speicherkontos und den Zugriffsschlüssel enthält. Sie müssen dies durch eine Verbindungszeichenfolge mit dem Namen und Schlüssel des Speicherkontos ersetzen.  
...
2. Klicken Sie im **Server-Explorer** in Ihrem Speicherkonto mit der rechten Maustaste im Knoten **Speicher** auf Ihre Website, und klicken Sie anschließend auf **Eigenschaften**.

	![Click Storage Account Properties](./media/websites-dotnet-webjobs-sdk-get-started/storppty.png)	

4. Klicken Sie im Fenster **Eigenschaften** auf **SSpeicherkontoschlüssel** und dann auf die Auslassungspunkte.
...
8. Ersetzen Sie die Speicher-Verbindungszeichenfolge in der Datei *Web.config* durch die Verbindungszeichenfolge, die Sie gerade kopiert haben. Stellen Sie sicher, dass Sie alles, was innerhalb der Anführungszeichen steht, aber nicht die Anführungszeichen selbst, vor dem Einfügen ausgewählt haben.
...
9. Geben Sie im Dialogfeld **Site auf Microsoft Azure erstellen** einen eindeutigen Namen in das Feld **Sitename** ein.
...
2. Klicken Sie im Schritt **Verbindung ** im Assistenten **Web veröffentlichen** auf **Weiter**.
...
Sie können die Warnungen auf dieser Seite ignorieren. 
	...
	* Für dieses Lernprogramm nehmen Sie keine Änderungen am Datenmodell für die ContosoAdsContext-Datenbank vor und müssen daher keine Entity Framework Code First Migrations für die Bereitstellung verwenden. Code First erstellt automatisch eine neue Datenbank, wenn die Anwendung zum ersten Mal versucht, auf SQL-Daten zuzugreifen.
...
Sie können die Warnung ignorieren, dass keine Datenbanken veröffentlicht werden. Die Datenbank wird von Entity Framework Code First erstellt und muss daher nicht veröffentlicht werden.
	...
	Im Vorschaufenster wird angezeigt, dass Binär- und Konfigurationsdateien aus dem WebJob-Projekt in den Ordner *app_data\jobs\continuous* der Website kopiert werden.
...
10. Klicken Sie im **Server Explorer** mit der rechten Maustaste auf die Website, und klicken Sie dann auf **Website anhalten**. 

12. Nachdem die Website beendet wurde, klicken Sie mit der rechten Maustaste erneut auf die Website, und klicken Sie auf **Website starten**.

Der WebJob wird bei der Veröffentlichung automatisch gestartet, wird jedoch bei Änderungen an der Konfiguration angehalten. Um ihn neu zu starten können Sie entweder die Website neu starten oder den WebJob im Azure-Verwaltungsportal neu starten. Nach einer Konfigurationsänderung sollten Sie jedoch im Allgemeinen die Website neu starten. 
...
Bei dieser Beispielanwendung gehen immer Websiteaktivitäten der Erstellung einer Warteschlangennachricht voraus. Es besteht also kein Problem, wenn die Website in den Ruhezustand wechselt und den WebJob aufgrund eines langen Zeitraums der Inaktivität beendet. Wenn eine Anforderung eingeht, wird die Website reaktiviert, und der WebJob wird neu gestartet.

Für WebJobs, die weiterhin ausgeführt werden sollen, auch wenn die Website für längere Zeit inaktiv ist, können Sie die Funktion [AlwaysOn](http://weblogs.asp.net/scottgu/archive/2014/01/16/windows-azure-staging-publishing-support-for-web-sites-monitoring-improvements-hyper-v-recovery-manager-ga-and-pci-compliance.aspx) von Azure-Websites verwenden.
...
10. Nennen Sie das Projekt *ContosoAdsCommon*, und klicken Sie auf **OK**.
...
Weitere Informationen zu diesen Änderungen finden Sie unter [Bereitstellen von WebJobs in Visual Studio](/en-us/documentation/articles/websites-dotnet-deploy-webjobs/).

### NuGet-Pakete hinzufügen

Die neue Projektvorlage für ein WebJob-Projekt installiert automatisch das WebJobs-SDK-NuGet-Paket [Microsoft.Azure.WebJobs](http://www.nuget.org/packages/Microsoft.Azure.WebJobs) und seine Abhängigkeiten. 

Eine WebJobs-SDK-Abhängigkeiten, die automatisch im WebJob-Projekt installiert werden, ist die Azure-Speicherclient-Bibliothek (Storage Client Library, SCL). Allerdings müssen Sie es zum Webprojekt hinzufügen, um mit Blobs und Warteschlangen arbeiten zu können.
...
11. Fügen Sie im Projekt ContosoAdsWebJob einen Verweis auf "System.Drawing" und "System.Configuration" ein.

### Hinzufügen von Code und Konfigurationsdateien

Dieses Lernprogramm befasst sich nicht mit dem [Erstellen von MVC-Controller und -Ansichten mithilfe eines Gerüsts](http://www.asp.net/mvc/tutorials/mvc-5/introduction/getting-started), dem [Schreiben von Entity Framework-Code für die Arbeit mit SQL Server-Datenbanken](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc) und den [Grundlagen asynchroner Programmierung in ASP.NET 4.5](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/web-development-best-practices#async). Daher müssen Sie nur noch den Code und die Konfigurationsdateien aus der heruntergeladenen Lösung in die neue Lösung kopieren. Anschließend finden Sie in den folgenden Abschnitten Hinweise zu den wichtigsten Bereichen des Codes.
...
3. Fügen Sie im Projekt ContosoAdsWeb die folgenden Dateien aus dem heruntergeladenen Projekt hinzu.
	...
	- Im Ordner *Views\Ad* (erstellen Sie den Ordner zunächst): fünf *.cshtml*-Dateien.<br/><br/>
...
Die Klasse "BlobInformation" speichert Informationen über ein Bild-Blob in einer Warteschlangen-Nachricht.
...
Der aus der Methode "Application_Start" aufgerufene Code erstellt einen *images*-Blob-Container und eine *images*-Warteschlange, falls diese noch nicht existieren. Damit wird sichergestellt, dass Blob-Container und Warteschlange immer automatisch erstellt werden, wenn Sie ein neues Speicherkonto verwenden.

Der Code verwendet die Speicher-Verbindungszeichenfolge aus der Datei *Web.config* oder aus der Azure-Laufzeitumgebung für den Zugriff auf das Speicherkonto.
...
In der Datei *AdController.cs* ruft der Konstruktor die "InitializeStorage"-Methode auf, um Objekte der Azure-Speicherclient-Bibliothek zu erstellen, die eine API für die Arbeit mit Blobs und Warteschlangen bereitstellen. 
...
Ein Großteil des Controller-Codes ist typisch für die Arbeit mit einem Entity Framework-Datenmodell und einer DbContext-Klasse. Eine Ausnahme ist die HttpPost Methode "Create", die eine Datei hochlädt und im Blobspeicher ablegt. Die Modellbindung stellt ein [HttpPostedFileBase](http://msdn.microsoft.com/en-us/library/system.web.httppostedfilebase.aspx)-Objekt für die Methode bereit.
...
>* Weitere Informationen zum Implementieren von ordnungsgemäßem Herunterfahren finden Sie unter [Ordnungsgemäßes Herunterfahren](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#graceful).   
...
### WebJobs-SDK vs. Cloud-Dienst-Worker-Rolle ohne WebJobs SDK

Wenn Sie die Menge an Code in der "GenerateThumbnails"-Methode in dieser Beispielanwendung mit dem Workerrollen-Code in der [Cloud-Dienst-Version der Anwendung](/en-us/documentation/articles/cloud-services-dotnet-get-started/) vergleichen, können Sie erkennen, wie viel Arbeit das WebJobs SDK Ihnen tatsächlich abnimmt. Der tatsächliche Vorteil ist sogar noch größer, da der Code der Cloud-Dienst-Beispielanwendung nicht alle Aufgaben erledigt (z. B. die Behandlung nicht verarbeitbarer Nachrichten), die sie in einer Produktionsanwendung implementieren würden, während das WebJobs SDK diese Aufgaben ausführt.

In der Cloud-Dienst-Version der Anwendung ist die Eintrags-ID die einzige Information in der Warteschlangen-Nachricht, und der Hintergrundprozess ruft die Bild-URL aus der Datenbank ab. In der WebJobs SDK-Version der Anwendung enthält die Warteschlangen-Nachricht die Bild-URL, die an die Blob-Attribute weitergegeben werden kann. Wenn die Warteschlangenmeldung nicht die Blob-URL enthält, können Sie [das Blob-Attribut im Text der Methode statt der Methodensignatur verwenden](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#blobbody).

### Verwenden des WebJobs-SDK außerhalb von WebJobs

Programme, die das WebJobs SDK verwenden, müssen nicht in einem WebJob in Azure ausgeführt werden. Sie können lokal ausgeführt werden und auch in anderen Umgebungen, wie z. B. einer Cloud-Dienst-Workerrolle oder einem Windows-Dienst. Sie erreichen das WebJobs SDK-Dashboard jedoch nur über eine Azure-Website. Um das Dashboard zu verwenden, müssen Sie die Website mit dem Speicherkonto verbinden, indem Sie "AzureWebJobsDashboard"-Verbindungszeichenfolge in der Registerkarte **Konfigurieren** im Verwaltungsportal setzen. Anschließend finden Sie das Dashboard unter der URL https://{websitename}.scm.azurewebsites.net/azurejobs/#/functions. Weitere Informationen finden Sie unter [Einrichten eines Dashboards für die lokale Entwicklung mit dem WebJobs SDK](http://blogs.msdn.com/b/jmstall/archive/2014/01/27/getting-a-dashboard-for-local-development-with-the-webjobs-sdk.aspx). Dieser Artikel enthält jedoch einen veralteten Namen für die Verbindungszeichenfolge. 

## Nächste Schritte

In diesem Lernprogramm haben Sie erfahren, wie Sie eine einfache Anwendung mit mehreren Ebenen erstellen können, die das WebJobs SDK für die Hintergrundverarbeitung verwendet. Die Anwendung wurde für dieses Lernprogramm einfach gehalten. Sie implementiert z. B. keine [Abhängigkeitsinjektion](http://www.asp.net/mvc/tutorials/hands-on-labs/aspnet-mvc-4-dependency-injection) oder [Repository und Arbeitseinheit](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/advanced-entity-framework-scenarios-for-an-mvc-web-application#repo), verwendet keine [Schnittstelle für die Protokollierung](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/monitoring-and-telemetry#log) und keine [EF Code First Migrations](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/migrations-and-deployment-with-the-entity-framework-in-an-asp-net-mvc-application) zur Verwaltung von Datenmodellen oder [EF-Verbindungsstabilität](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/connection-resiliency-and-command-interception-with-the-entity-framework-in-an-asp-net-mvc-application) ür vorübergehende Verbindungsfehler, usw.
